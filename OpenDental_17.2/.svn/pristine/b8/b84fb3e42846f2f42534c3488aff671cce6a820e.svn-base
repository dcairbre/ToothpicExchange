...
namespace OpenDentBusiness {
	public class AccountModules {
			...
			,Patient pat,Family fam,out decimal payPlanDue,out decimal balanceForward,Statement stmt=null,bool isComputeAging=true) 
		{
			...
			foreach(DataRow rowRp in rawPay.Rows) {//Each row is a payment
				...
				foreach(PaySplit split in listPaySplitMatches) {
					listPaySplitsAllocated.AddRange(listPaysplits.FindAll(x => x.PrepaymentNum==split.SplitNum));
				}
				...
				foreach(PaySplit split in listPaySplitsAllocated) {
					listPaySplitsProcs.AddRange(listPaysplits.FindAll(x => x.PayNum==split.PayNum && x.ProcNum>0 && x.SplitAmt==(0-split.SplitAmt)));//NOTE: THIS IS THE BEST COMPARISON WE CAN MAKE FOR THE INCOME TRANSFER!!! :(
				}
		...
		public static DataTable GetPaymentsForInvoice(List<long> listPatNums,List<long> listProcNums,DateTime dateTimeInvoice) {
			...
			string command = "SELECT tran.PriKey, patient.LName, patient.FName, tran.Date, tran.Amt, provider.Abbr Provider, "
			...
			command+=") tran "
				...
				+ "INNER JOIN provider ON provider.ProvNum = tran.ProvNum "
			...
			List<PaySplit> listSplitsCur = null/*,List<Adjustment> listAdjsCur = null,List<PayPlanCharge> listChargesCur = null*/) 
		{
			...
			if(calcType == CreditCalcType.ExcludeAll) {
				listProcs.ForEach(x => { listAccountEntries.Add(new AccountEntry(x)); });
			}
			else {
				List<ProcExtended> listProcExtendeds = Procedures.GetProcExtendedEntriesFromProcedures(listProcs,listAdjs,
					listPaySplits,listClaimProcs,listPayPlanCharges,listSplitsCur,ProcAttachTypes.InsEsts,ProcAttachTypes.WriteOffEsts);
				listProcExtendeds.ForEach(x => {
					listAccountEntries.Add(new AccountEntry(x.Proc,x.Proc.ProcDate,x.Proc.ProcNum,x.AmountOriginal,x.AmountStart,x.AmountEnd,x.Proc.ProvNum,x.Proc.ClinicNum,x.Proc.PatNum));
				});
			}
			listAccountEntries = listAccountEntries.OrderBy(x => x.Date).ToList();
			//get a list of unattached charges and add into account entry
			if(calcType == CreditCalcType.FIFO) {
				...
				listAccountEntries=listAccountEntries.OrderBy(x => x.Date).ToList(); //order by date. Credits get applied to oldest charges first.
				//get all unattached credits
				double creditTotal = 0;
				creditTotal-=listAdjs.Where(x => x.ProcNum == 0 && x.AdjAmt <0).Sum(x => x.AdjAmt); //negative adjustments are credits
				creditTotal+=listPaySplits.Where(x => x.ProcNum == 0).Sum(x => x.SplitAmt);
				creditTotal+=listInsPayTot.Where(x => x.ProcNum == 0).Sum(x => x.InsPayAmt);
				creditTotal+=listPayPlanCharges.Where(x => x.ProcNum == 0 && x.ChargeType == PayPlanChargeType.Credit).Sum(x => x.Principal);
				//apply unattached credits to account entries
				foreach(AccountEntry entryCur in listAccountEntries) {
					if(creditTotal==0) {
						break;
					}
					double amtApplied = entryCur.AmountStart - Math.Max(entryCur.AmountStart - creditTotal,0);
	...
	public class AccountEntry {
		...
		public AccountEntry(object tag,DateTime date,long priKey,double amtOrig,double amtStart,double amtEnd,long provNum,long clinicNum,long patNum) {
			Tag=tag;
			Date=date;
			PriKey=priKey;
			AmountOriginal=amtOrig;
			AmountStart=amtStart;
			AmountEnd=amtEnd;
			ProvNum=provNum;
			ClinicNum=clinicNum;
			PatNum=patNum;
		...
		public AccountEntry(Procedure proc) {
			...
			AmountStart=proc.ProcFee*Math.Max(1,proc.BaseUnits+proc.UnitQty);
			AmountEnd=AmountOriginal;
