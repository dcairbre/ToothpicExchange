...
namespace OpenDental{
	...
	public class FormFeeSchedTools :ODForm {
		...
		private ComboBox comboClinicGlobal;
		...
		public FormFeeSchedTools(long schedNum,List<FeeSched> listFeeScheds,List<Fee> listFees,List<Fee> listFeesOld,List<Provider> listProvs,List<Clinic> listClinics) {
		...
		private void InitializeComponent()
		{
			...
			this.comboClinicGlobal = new System.Windows.Forms.ComboBox();
			...
			this.groupBox2.Location = new System.Drawing.Point(236, 372);
			this.groupBox2.Name = "groupBox2";
			this.groupBox2.Size = new System.Drawing.Size(205, 47);
			...
			this.butClear.Location = new System.Drawing.Point(10, 17);
			...
			this.groupBox6.Controls.Add(this.comboClinicGlobal);
			...
			this.groupBox6.Size = new System.Drawing.Size(205, 80);
			...
			// labelClinic
			// 
			this.labelClinic.Location = new System.Drawing.Point(2, 23);
			...
			// comboClinicGU
			// 
			this.comboClinicGlobal.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList;
			this.comboClinicGlobal.FormattingEnabled = true;
			this.comboClinicGlobal.Location = new System.Drawing.Point(63, 20);
			this.comboClinicGlobal.Name = "comboClinicGlobal";
			this.comboClinicGlobal.Size = new System.Drawing.Size(136, 21);
			this.comboClinicGlobal.TabIndex = 6;
			this.comboClinicGlobal.SelectionChangeCommitted += new System.EventHandler(this.comboClinicGlobalUpdate_SelectionChangeCommitted);
			// 
			// label4
			// 
			...
			this.butUpdate.Location = new System.Drawing.Point(10, 49);
		...
		private void FillComboBoxes() {
			...
			int comboClinicGlobalIdx=comboClinicGlobal.SelectedIndex;
			...
			if(!PrefC.HasClinicsEnabled) {//No clinics
				...
				comboClinicTo.Items.Add(Lan.g(this,"None"));
				comboClinicGlobal.Items.Add(new ODBoxItem<Clinic>("None",new Clinic() { ClinicNum = 0 }));
				...
				comboClinicGlobal.Enabled=false;
				butPickClinic.Enabled=false;
				butPickClinicTo.Enabled=false;
			}
			else {
				comboClinic.Items.Add(Lan.g(this,"Default"));
				comboClinicTo.Items.Add(Lan.g(this,"Default"));
				for(int i=0;i<_listClinics.Count;i++) {
					comboClinic.Items.Add(_listClinics[i].Abbr);
					comboClinicTo.Items.Add(_listClinics[i].Abbr);
				}
				comboClinicGlobal.Items.Add(new ODBoxItem<Clinic>("Unassigned",new Clinic() { ClinicNum = 0}));
				List<Clinic> listClinicsGlobal=Clinics.GetForUserod(Security.CurUser);
				for(int i=0;i<listClinicsGlobal.Count;i++) {
					comboClinicGlobal.Items.Add(new ODBoxItem<Clinic>(listClinicsGlobal[i].Abbr,listClinicsGlobal[i]));
				}
			}
			...
			comboClinicGlobal.SelectedIndex=comboClinicGlobalIdx > -1 ? comboClinicGlobalIdx:0;//0 default to 'Unassigned'
		...
		private void butClear_Click(object sender, System.EventArgs e) {
			...
			Action actionCloseFeeSchedClearProgress=ODProgress.ShowProgressStatus("FeeSchedClear");
			long clinicNum=0;
		...
		private void butCopy_Click(object sender, System.EventArgs e) {
			...
			if(!PrefC.GetBool(PrefName.EasyNoClinics) && comboClinicTo.SelectedIndex!=0){
				toClinicNum=_listClinics[comboClinicTo.SelectedIndex-1].ClinicNum;
			}
			long toProvNum=0;
			...
			Action actionCloseFeeSchedCopyProgress=ODProgress.ShowProgressStatus("FeeSchedCopy",Lan.g(this,"Preparing to copy fees")+"...");
			...
			_listFees.RemoveAll(x => x.FeeSched==toFeeSchedNum && x.ProvNum==toProvNum && x.ClinicNum==toClinicNum);//clear current
			...
			foreach(Fee fee in listBestFees) {
				...
				newFee.ClinicNum=toClinicNum;
				newFee.FeeSched=toFeeSchedNum;
				_listFees.Add(newFee);
				ODEvent.Fire(new ODEventArgs("FeeSchedCopy",new ProgressBarHelper(Lan.g(this,"Copying fees, please wait")+"...","",rowNum,listBestFees.Count,ProgBarStyle.Continuous)));
				rowNum++;
			}
			...
			SecurityLogs.MakeLogEntry(Permissions.ProcFeeEdit,0,logText);
			_changed=true;
			actionCloseFeeSchedCopyProgress();
			DialogResult=DialogResult.OK;
		}
		...
		private void butIncrease_Click(object sender, System.EventArgs e) {
			...
			Action actionCloseFeeSchedIncreaseProgress=ODProgress.ShowProgressStatus("FeeSchedIncrease",Lan.g(this,"Preparing to modify fees")+"...");
			...
			for(int i=0;i<_listFees.Count;i++) {
				...
				try {
					procCode=ProcedureCodes.GetStringProcCode(_listFees[i].CodeNum);
				}
				...
				string logText=Lan.g(this,"Procedure")+": "+procCode+", "
					+Lan.g(this,"Fee")+": "+_listFees[i].Amount.ToString("c")+", "
					+Lan.g(this,"Fee Schedule")+": "+FeeScheds.GetDescription(_listFees[i].FeeSched);
				if(!PrefC.GetBool(PrefName.EasyNoClinics)) {
				...
				ODEvent.Fire(new ODEventArgs("FeeSchedIncrease",new ProgressBarHelper(Lan.g(this,"Modifying fees, please wait")+"...","",i,_listFees.Count,ProgBarStyle.Continuous)));
			}
			_changed=true;
      ...
      Dlg.FileName="Fees"+feeSchedDesc+".txt";
			...
			Action actionCloseFeeSchedExportProgress=ODProgress.ShowProgressStatus("FeeSchedExport",Lan.g(this,"Preparing to export fees")+"...");
		...
		private void butImport_Click(object sender,EventArgs e) {
			...
			Action actionCloseFeeSchedImportProgress=ODProgress.ShowProgressStatus("FeeSchedImport",Lan.g(this,"Importing fees, please wait")+"...");
			string[] fields;
			double feeAmt;
		...
		private void butImportEcw_Click(object sender,EventArgs e) {
			...
			Action actionCloseFeeSchedECWProgress=ODProgress.ShowProgressStatus("FeeSchedImportECW");
			int imported=0;
			int skippedCode=0;
			...
			if(formatQuotes) {//Original format - fields are surrounded by quotes (except first row, above)
				for(int i=1;i<lines.Length;i++) {
					...
					_listFees=Fees.Import(fields[0],feeAmt,feesched.FeeSchedNum,clinicNum,provNum,_listFees);
					ODEvent.Fire(new ODEventArgs("FeeSchedImportECW",new ProgressBarHelper(Lan.g(this,"Importing fees, please wait")+"...","",imported,lines.Length,ProgBarStyle.Continuous)));
					imported++;
				}
			}
				...
				for(int i=1;i<lines.Length;i++) {
					...
					if(fields.Count>10) {
						actionCloseFeeSchedECWProgress();
		...
		private void butImportCanada_Click(object sender,EventArgs e) {
			...
			if(formPick.IsFileChosenProtected) {
				...
				Action actionCloseFeeSchedImportCanadaProgress=ODProgress.ShowProgressStatus("FeeSchedImportCanada");
				...
				Action actionCloseFeeSchedImportCanadaProgress=ODProgress.ShowProgressStatus("FeeSchedImportCanada",Lan.g(this,"Downloading fee schedule")+"...");
		...
		private void butUpdate_Click(object sender,EventArgs e) {
			...
			Action actionCloseFeeSchedUpdateProgress=ODProgress.ShowProgressStatus("FeeSchedUpdate");
			Cursor=Cursors.WaitCursor;
			try {
				if(PrefC.HasClinicsEnabled) {
					rowsChanged=Procedures.GlobalUpdateFees(_listFees,((ODBoxItem<Clinic>)comboClinicGlobal.SelectedItem).Tag.ClinicNum);
				}
				else {
					rowsChanged=Procedures.GlobalUpdateFees(_listFees);
				}
			}
			catch(ApplicationException ex) {
				Cursor=Cursors.Default;
				actionCloseFeeSchedUpdateProgress?.Invoke();
			...
			Cursor=Cursors.Default;
			MessageBox.Show(Lan.g(this,"Treatment planned procedure fees changed")+": "+rowsChanged.ToString());
		}
		...
		private void FormFeeSchedTools_FormClosing(object sender,FormClosingEventArgs e) {
			if(DialogResult==DialogResult.OK && _changed) {
				...
				Fees.InvalidateFeeSchedules(schedNums);
				Cursor=Cursors.Default;
			}
		}
