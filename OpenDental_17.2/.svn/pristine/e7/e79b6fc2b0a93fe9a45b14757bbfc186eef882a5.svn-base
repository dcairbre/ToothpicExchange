...
namespace OpenDentBusiness {
    ...
    private List<X12Segment> _listSegments;
		...
		///If isSimple is true, then will skip pseudo attaching claims by claimnum (saves a query).</summary>
		public X835(string messageText,string tranSetId,List <Etrans835Attach> listAttached=null,bool isSimple=false):base(messageText) {//Parsing happens in the base class.
		...
		private void DiscoverSplitClaims() {
			foreach(Hx835_Claim claim in ListClaimsPaid) {
				if(claim.IsSplitClaim) {
					continue;//We already discoverd this claim is a split claim.  Skip.
				}
				foreach(Hx835_Claim claimOther in ListClaimsPaid) {
					if(claimOther!=claim && claimOther.ClaimTrackingNumber==claim.ClaimTrackingNumber) {
						claim.IsSplitClaim=true;
						claimOther.IsSplitClaim=true;
						break;
					}
				}
			}
		...
		///<summary>When listClaimNums is null this will run a query.  The listClaimNums count must match GetUnattached() count and be in the same order.</summary>
		public void SetClaimNumsForUnattached(List<long> listClaimNums=null) {
			List<Hx835_Claim> listUnattached=GetUnattached();
		...
		private Hx835_Claim ProcessCLP(int segNum,string npi) {
			...
			//"Amounts in CLP05 must have supporting adjustments relected in CAS segments at the 2100 (CLP) or 2110 (SVC) loop level with a
			//Claim Adjustment Group (CAS01) code or PR (Patient Responsibility)"
			List<Hx835_Adj> listPatRespAdjs=retVal.ListClaimAdjustments.FindAll(x => x.AdjCode=="PR");
			if(listPatRespAdjs.Count==0) {//No patient responsibility adjustments at claim level.  There "must" be some at the procedure level.
				foreach(Hx835_Proc proc in retVal.ListProcs) {
					retVal.PatientDeductAmt+=proc.DeductibleAmt;
				}
			}
			else {//Patient responsibility adjustments found at the claim level.
				listPatRespAdjs.FindAll(x => x.IsDeductible).ForEach(x => retVal.PatientDeductAmt+=x.AdjAmt);
			}
