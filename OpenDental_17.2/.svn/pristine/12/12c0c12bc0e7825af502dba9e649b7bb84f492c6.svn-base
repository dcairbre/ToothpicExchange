...
namespace OpenDental
{
	...
	public class ContrPerio : System.Windows.Forms.Control{
		...
		private static List<ProcedureCode> _listProcedureCodes=null;
		///<summary>Used to determine if a tooth had an implant proc.</summary>
		...
		*/
			...
			for(int seqI=0;seqI<PerioMeasures.List.GetLength(1);seqI++){
				for(int toothI=1;toothI<PerioMeasures.List.GetLength(2);toothI++){
						...
						|| HasChanged[seqI,toothI])
					{
						...
						if(PerioMeasures.List[selectedExam,seqI,toothI]==null || PerioExamCur.IsNew) {
		...
		public static bool IsImplant(int toothNum) {
			...
			if(_listProcedureCodes==null) {//Not initialized yet.
				_listProcedureCodes=ProcedureCodeC.GetListLong();
			}
			...
			for(int i = 0;i<listProcsForTooth.Count;i++) {
				ProcedureCode procCode=ProcedureCodes.GetProcCode(_listProcedureCodes,listProcsForTooth[i].CodeNum);
