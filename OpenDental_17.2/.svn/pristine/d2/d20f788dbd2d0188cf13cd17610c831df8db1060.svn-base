...
namespace OpenDental {
	public partial class FormMapSetup:ODForm {
		...
		private void FillSettings() {
			...
			if(_mapCur.ExtensionStartRange==-1) {
				textExtensionStart.Text="";
			}
			else {
				textExtensionStart.Text=_mapCur.ExtensionStartRange.ToString();
			}
			if(_mapCur.ExtensionEndRange==-1) {
				textExtensionEnd.Text="";
			}
			else {
				textExtensionEnd.Text=_mapCur.ExtensionEndRange.ToString();
			}
			FillMap();
		}
		...
		private void textExtensionStart_Leave(object sender,EventArgs e) {
			if(textExtensionStart.Text=="") {
				_mapCur.ExtensionStartRange=-1;
			}
			else {
				try {
					_mapCur.ExtensionStartRange=PIn.Int(textExtensionStart.Text);
				}
				catch(Exception ex) {
					_mapCur.ExtensionStartRange=-1;
					ex.DoNothing();
		...
		private void textExtensionEnd_Leave(object sender,EventArgs e) {
			if(textExtensionEnd.Text=="") {
				_mapCur.ExtensionEndRange=-1;
			}
			else {
				try {
					_mapCur.ExtensionEndRange=PIn.Int(textExtensionEnd.Text);
				}
				catch(Exception ex) {
					_mapCur.ExtensionEndRange=-1;
					ex.DoNothing();
		...
		private void butSave_Click(object sender,EventArgs e) {
			if(textExtensionStart.errorProvider1.GetError(textExtensionStart)!="" || textExtensionEnd.errorProvider1.GetError(textExtensionEnd)!="") {
