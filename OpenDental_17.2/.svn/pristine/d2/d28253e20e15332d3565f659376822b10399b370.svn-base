...
namespace OpenDental {
	public class PrefL{
		...
		public static bool CopyFromHereToUpdateFiles(Version versionCurrent,bool isSilent,bool hasAtoZ,bool hasConcatFiles) {
			...
			if(!isSilent) {
				actionCloseRecopyProgress=ODProgress.ShowProgressStatus("RecopyProgress");
			}
			#endregion
		...
		public static bool CheckProgramVersion(bool isSilent = false) {
			...
			if(storedVersion>currentVersion) {
				...
				return UpdateClientFromServerUpdateFiles(storedVersion,currentVersion); //Update client to server version
			}
				...
				&& !ODEnvironment.IdIsThisComputer(updateServerName.ToLower()))
			{
				...
				return UpdateClientFromServerUpdateFiles(storedVersion,currentVersion); //Downgrade client to server version
			}
				...
				if(!PrefC.GetBool(PrefName.EConnectorEnabled)) {
					...
					if(ODEnvironment.IdIsThisComputer(updateServerName.ToLower()) || needsEConnectorUpgrade) {
						...
						if(UpgradeOrInstallEConnector(isSilent,out isListening)) {
							...
							try {
								ListenerServiceType listenerType=WebServiceMainHQProxy.SetEConnectorOnOff(isListening);
		...
		///<summary>Inserts a generic alert with the text "Open Dental Service Error" when there are issues with installing the OD service.</summary>
		private static void CreateAlert() {
			AlertItem alert=new AlertItem();
			alert.Type=AlertType.Generic;
			alert.Actions=ActionType.Delete;
			alert.Description="Open Dental Service Error";
		...
		private static bool UpdateClientFromServerUpdateFiles(Version storedVersion, Version currentVersion) {
			...
			if(Directory.Exists(folderUpdate)) {
				try {
					...
					MessageBox.Show(Lan.g("Prefs","Unable to delete update files from local temp folder. Try closing and reopening the program."));
					ex.DoNothing();
