	...
	public class FormDisplayFieldsOrthoChart : ODForm {
		...
		private void LoadDisplayFields() {
			...
			List<DisplayField> listOrphanedFields=_listAllDisplayFields.FindAll(x => x.DisplayFieldNum==0
				|| !OrthoChartTabLinks.List.Exists(y => y.DisplayFieldNum==x.DisplayFieldNum));
		...
		private void FillGrids() {
			labelCategory.Text=OrthoChartTabs.Listt[0].TabName;//Placed here so that Up/Down buttons will affect the label text.			
		...
		private void butOK_Click(object sender,EventArgs e) {
			...
			List<OrthoChartTabLink> listOrthoChartTabLinks=OrthoChartTabLinks.GetList();
			...
			OrthoChartTabLinks.Sync(listOrthoChartTabLinks,OrthoChartTabLinks.List);
