...
namespace OpenDental {
	public partial class FormFHIRSetup:ODForm {
		...
		private List<APIKey> GetApiKeys() {
			...
			OpenDentBusiness.WebServiceMainHQ.WebServiceMainHQ service=WebServiceMainHQProxy.GetWebServiceMainHQInstance();
#if DEBUG
			...
			try {
				...
				result=service.GetFHIRAPIKeysForOffice(officeData);
			}
			catch(Exception ex) {
		...
		private APIKey GenerateNewKey() {
			...
			try {
				string officeData=WebServiceMainHQProxy.CreateWebServiceHQPayload(strbuild.ToString(),eServiceCode.FHIR);
				result=service.GenerateFHIRAPIKey(officeData);
			}
			catch(Exception ex) {
				MessageBox.Show(ex.Message);
				return null;
		...
		private bool UpdateKeysForOffice(List<APIKey> _listApiKeysLocal) {
			...
			try {
				string officeData=WebServiceMainHQProxy.CreateWebServiceHQPayload(strbuild.ToString(),eServiceCode.FHIR);
				result=service.UpdateFHIRAPIKeys(officeData);
			}
			catch(Exception ex) {
				MessageBox.Show(ex.Message);
				return false;
