...
namespace OpenDentBusiness{
	...
	public class ProcedureCodes {
		...
		public static List<ProcedureCode> GetProcCodeStartsWith(string codeStart,Hashtable hashProcedureCodes = null) {
			//No need to check RemotingRole; no call to db.
			List<ProcedureCode> listCodesMatchCodeStart=new List<ProcedureCode>();
			if(hashProcedureCodes==null) {
				hashProcedureCodes=ProcedureCodeC.GetHList();
			}
			foreach(DictionaryEntry entry in hashProcedureCodes) {
				if(entry.Key.ToString().StartsWith(codeStart)) {
					listCodesMatchCodeStart.Add((ProcedureCode)entry.Value);
				}
			}
			return listCodesMatchCodeStart;
		}
		...
		public static ProcedureCode GetProcCode(string myCode,Hashtable hashProcedureCodes=null) {
			//No need to check RemotingRole; no call to db.
		...
		public static List<ProcedureCode> GetProcCodes(List<string> listCodes) {
			...
			foreach(string code in listCodes) {
				if(hashProcedureCodes.Contains(code)) {
					listProcCodes.Add((ProcedureCode)hashProcedureCodes[code]);
				}
			}
			return listProcCodes;
		}
		...
		public static long GetCodeNum(string myCode,Hashtable hashProcedureCodes=null) {
			//No need to check RemotingRole; no call to db.
		...
		public static bool CanBypassLockDate(long codeNum,double procFee) {
			...
			foreach(ProcedureCode procCode in listProcCodes) {
				if(procCode.CodeNum==codeNum) {
					if(procCode.BypassGlobalLock==BypassLockStatus.BypassIfZero && procFee.IsZero()) {
						return true;
					}
					return false;
				}
		...
		public static bool DoAnyBypassLockDate() {
			return ProcedureCodeC.GetListLong().Any(x => x.BypassGlobalLock==BypassLockStatus.BypassIfZero);
		}
		...
		public static string GetStringProcCode(long codeNum,List<ProcedureCode> listProcedureCodes=null) {
			...
			ProcedureCode procedureCode=listProcedureCodes.FirstOrDefault(x => x.CodeNum==codeNum);
		...
		public static bool HasMissedCode() {
			return ProcedureCodeC.GetHList().Contains("D9986");
		}
		...
		public static bool HasCancelledCode() {
			return ProcedureCodeC.GetHList().Contains("D9987");
		}
		...
		public static List<ProcedureCode> GetCodesForCodeNums(List<long> listCodeNums) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<ProcedureCode>>(MethodBase.GetCurrentMethod(),listCodeNums);
			}
			if(listCodeNums.Count == 0) {
				return new List<ProcedureCode>();
			}
			string command = "SELECT * FROM procedurecode WHERE procedurecode.CodeNum IN (" +string.Join(",",listCodeNums)+")";
			return Crud.ProcedureCodeCrud.SelectMany(command);
		}
		...
		public static List<long> GetOrthoBandingCodeNums() {
			...
			if(strListOrthoNums!="") {
				...
				return ProcedureCodeC.Listt.Where(x => x.ProcCode.StartsWith("D8")).Select(x => x.CodeNum).ToList();
			}
		}
