...
namespace OpenDentBusiness{
	...
	public class SecurityLogs {
		...
		public static SecurityLog[] Refresh(DateTime dateFrom,DateTime dateTo,Permissions permType,long patNum,long userNum,int limit=0) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<SecurityLog[]>(MethodBase.GetCurrentMethod(),dateFrom,dateTo,permType,patNum,userNum,limit);
			}
			...
			return list.OrderBy(x => x.LogDateTime).ToArray();
		}
		...
		///<summary>Takes a foreign key to a table associated with that PermType.  PatNum can be 0.</summary>
		public static void MakeLogEntry(Permissions permType,long patNum,string logText,long fKey,LogSources logSource,long defNum,long defNumError=0) {
			//No need to check RemotingRole; no call to db.
			SecurityLog securityLog=MakeLogEntryNoInsert(permType,patNum,logText,fKey,logSource,defNum,defNumError);
		...
		public static SecurityLog MakeLogEntryNoInsert(Permissions permType,long patNum,string logText,long fKey,LogSources logSource=LogSources.None,long defNum=0,long defNumError=0) {
			//No need to check RemotingRole; no call to db.
