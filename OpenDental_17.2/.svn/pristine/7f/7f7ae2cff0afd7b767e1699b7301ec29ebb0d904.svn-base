	...
	public partial class FormSmsTextMessaging:ODForm {
		...
		private void FormSmsTextMessaging_Load(object sender,EventArgs e) {
			...
			if(PrefC.HasClinicsEnabled) {
				...
				comboClinic.SetSelected(true);//Select all options by default
			}
		...
		private void FillGridTextMessages(bool isRedrawOnly=false) {
			...
			if(listSmsFromStatuses.Count>0) {
				...
				for(int i=0;i<_listSmsFromMobile.Count;i++) {
					...
					row.Cells.Add(_listSmsFromMobile[i].PatNum==0?Lan.g(this,"Unassigned"):dictPatNames[_listSmsFromMobile[i].PatNum]);//Patient
			...
			if(selectedPatNum==0) {
				...
				Text=Lan.g(this,"Text Messaging - Patient:")+" "+dictPatNames[selectedPatNum];
			}
		}
		...
		///<summary>Since the grid is set to SelctionMode of "One", there will always be exactly 1 selection when this event occurs.
		...
		private void gridMessages_OnSelectionCommitted(object sender,EventArgs e) {
			if(gridMessages.GetSelectedIndex()!=-1) {//Verify that there is at least one selection, just in case.  There should always be one though.
				UI.ODGridRow row=gridMessages.Rows[gridMessages.GetSelectedIndex()];
				if(row.Tag is SmsFromMobile) {
					SmsFromMobile smsFromMobile=(SmsFromMobile)row.Tag;
		...
		private void BlockNumber() {
			if(gridMessages.SelectedIndices.Length!=1) {
				MsgBox.Show(this,"Please select one message to block the number.");
				return;
			}
			string numberToBlock;
			long patNum;
			if(gridMessages.Rows[gridMessages.SelectedIndices[0]].Tag.GetType()==typeof(SmsFromMobile)) {
				SmsFromMobile smsFromMobile=(SmsFromMobile)gridMessages.Rows[gridMessages.SelectedIndices[0]].Tag;
				numberToBlock=smsFromMobile.MobilePhoneNumber;
				patNum=smsFromMobile.PatNum;
			}
			else if(gridMessages.Rows[gridMessages.SelectedIndices[0]].Tag.GetType()==typeof(SmsToMobile)) {
				SmsToMobile smsToMobile=(SmsToMobile)gridMessages.Rows[gridMessages.SelectedIndices[0]].Tag;
				numberToBlock=smsToMobile.MobilePhoneNumber;
		...
		private void menuItemSelectPatient_Click(object sender,EventArgs e) {
			...
			FormOpenDental.S_Contr_PatientSelected(Patients.GetPat(patNum),true);
		}
		...
		private void butReply_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<gridMessages.Rows.Count;i++) {
				...
				if(row.ColorBackG!=_colorSelect) {
					continue;
				}
				if(row.Tag is SmsFromMobile) {
					SmsFromMobile smsFrom=(SmsFromMobile)row.Tag;
					if(smsFrom.PatNum!=patNum) {
        ...
        if(Plugins.HookMethod(this,"FormSmsTextMessaging.butReply_Click_sendSmsSingle",patNum,mobileNumber,textReply.Text,YN.Yes))
        {
          goto HookSkipSmsCall;
				...
				SmsToMobiles.SendSmsSingle(patNum,mobileNumber,textReply.Text,clinicNum,SmsMessageSource.DirectSms,user:Security.CurUser);
			}
			catch(Exception ex) {
