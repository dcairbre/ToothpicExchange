...
namespace OpenDental {
	public partial class FormProcEditAll:ODForm {
		...
		private void FormProcEditAll_Load(object sender,EventArgs e) {
			...
			for(int i=0;i<ProcList.Count;i++){
				if(ProcList[i].ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC)) {
					AnyAreC=true;
			...
			if(AnyAreC){
				if(areAllBypass) {
					if(!Security.IsAuthorized(Permissions.ProcComplEdit,oldestDateEntryC,ProcList[0].CodeNum,0)) {
						butOK.Enabled=false;
						butEditAnyway.Enabled=false;
					}
				}
				else if(!Security.IsAuthorized(Permissions.ProcComplEdit,oldestDateEntryC)){
					butOK.Enabled=false;
					butEditAnyway.Enabled=false;
				}
			}
