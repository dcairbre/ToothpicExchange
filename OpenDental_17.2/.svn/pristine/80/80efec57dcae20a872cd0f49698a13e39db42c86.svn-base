...
namespace OpenDental {
	public partial class FormEServicesSetup {
		...
		private void FillTabEConnector() {
			_listenerType=_singupOut.ListenerType;
		...
		private void AuthorizeTabEConnector(bool allowEdit) {
			...
			butSaveListenerPort.Enabled=allowEdit;
		}
		...
		private void butInstallEConnector_Click(object sender,EventArgs e) {
			...
			if(!PrefC.GetBool(PrefName.EConnectorEnabled)) {
				...
				try {
					_listenerType=WebServiceMainHQProxy.SetEConnectorOnOff(startListenerCommunications);
		...
		private void butSaveListenerPort_Click(object sender,EventArgs e) {			
			...
			try {
				_listenerType=WebServiceMainHQProxy.SetEConnectorOnOff(checkAllowEConnectorComm.Checked);
