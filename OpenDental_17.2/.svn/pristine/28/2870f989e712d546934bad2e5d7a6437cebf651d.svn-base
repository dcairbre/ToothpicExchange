	...
	public class FormOperatories : ODForm {
		...
		private void FormOperatories_Load(object sender,System.EventArgs e) {
			if(!PrefC.HasClinicsEnabled) {
				...
				for(int i = 0;i<listClinics.Count;i++) {
					comboClinic.Items.Add(new ODBoxItem<Clinic>(listClinics[i].Abbr,listClinics[i]));
					if(listClinics[i].ClinicNum==Clinics.ClinicNum) {
						comboClinic.SelectedIndex=i+1;//+1 because of All
					}
				}
			}
			RefreshList();
		...
		private void FillGrid(){
			...
			for(int i=0;i<_listOps.Count;i++){
				if(PrefC.HasClinicsEnabled
					&& comboClinic.SelectedIndex!=0 //'All' is not selected.
					&& _listOps[i].ClinicNum!=((ODBoxItem<Clinic>)comboClinic.SelectedItem).Tag.ClinicNum) 
				{
					continue;
				}
		...
		private void butPickClinic_Click(object sender,EventArgs e) {
			...
			ODBoxItem<Clinic> selectedBoxItem=comboClinic.Items.Cast<ODBoxItem<Clinic>>()
				.Where(x => x.Tag.ClinicNum == FormC.SelectedClinicNum)
				.FirstOrDefault();
			if(selectedBoxItem != null) {
				comboClinic.SelectedItem=selectedBoxItem;
			}
			FillGrid();
		}
		...
		private void butAdd_Click(object sender, System.EventArgs e) {
			...
			opCur.IsNew=true;
			if(PrefC.HasClinicsEnabled) {
				opCur.ClinicNum=((ODBoxItem<Clinic>)comboClinic.SelectedItem).Tag.ClinicNum;
			}
		...
		private void butCombine_Click(object sender,EventArgs e) {
			...
			List<Appointment> listApptsToMerge=Operatories.MergeApptCheck(masterOpNum,listSelectedOpNums.FindAll(x => x!=masterOpNum));
			ListConflictingAppts=listApptsToMerge.FindAll(x => (bool)x.TagOD);
		...
		private bool CanReorderOps(Operatory op1,Operatory op2,out string strErrorMsg) {
			...
			strErrorMsg=Lan.g(this,"You cannot change the order of the Operatories")+" '"+op1.Abbrev+"' "+Lan.g(this,"and")+" '"+op2.Abbrev+"' "
				+Lan.g(this,"with Clinic")+" '"+((ODBoxItem<Clinic>)comboClinic.SelectedItem).Tag.Abbr+"' "
