	...
	public class FormAdjust : ODForm {
		...
		private void FormAdjust_Load(object sender, System.EventArgs e) {
			if(IsNew){
				...
				if(DefC.GetValue(DefCat.AdjTypes,_adjustmentCur.AdjType)=="dp") {
					labelAdditions.Text=Lan.g(this,"Discount Plan")+": "+DefC.GetName(DefCat.AdjTypes,_adjustmentCur.AdjType);
			...
			if(DefC.GetValue(DefCat.AdjTypes,_adjustmentCur.AdjType)=="+"){//pos
				textAmount.Text=_adjustmentCur.AdjAmt.ToString("F");
			}
			else if(DefC.GetValue(DefCat.AdjTypes,_adjustmentCur.AdjType)=="-"){//neg
				textAmount.Text=(-_adjustmentCur.AdjAmt).ToString("F");//shows without the neg sign
			}
			else if(DefC.GetValue(DefCat.AdjTypes,_adjustmentCur.AdjType)=="dp") {//Discount Plan (neg)
				textAmount.Text=(-_adjustmentCur.AdjAmt).ToString("F");//shows without the neg sign
			}
		...
		private void FillProcedure(){
			...
			if(textAmount.errorProvider1.GetError(textAmount)==""){
				...
				else if(listTypeNeg.SelectedIndex>-1 || DefC.GetValue(DefCat.AdjTypes,_adjustmentCur.AdjType)=="dp"){//neg or discount plan
					procAdjCur=-PIn.Double(textAmount.Text);
				}
			}
		...
		private void butAttachProc_Click(object sender, System.EventArgs e) {
			FormProcSelect FormPS=new FormProcSelect(_adjustmentCur.PatNum,true);
			FormPS.ShowDialog();
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			bool isDiscountPlanAdj=(DefC.GetValue(DefCat.AdjTypes,_adjustmentCur.AdjType)=="dp");
			...
			if(PrefC.GetBool(PrefName.AdjustmentsForceAttachToProc) && _adjustmentCur.ProcNum==0) {
				MsgBox.Show(this,"You must attach a procedure to the adjustment.");
				return;
			}
			if(_adjRemAmt < 0) {
				if(!MsgBox.Show(this,MsgBoxButtons.YesNo,"You have attached more credits to this procedure than the procedure's fee. Continue anyway?")) {
					return;
				}
			}
			if(IsNew){
