...
namespace OpenDental {
	public class PhoneUI {
		...
		public static void Available(PhoneTile tile) {
			...
			if(Security.CurUser.EmployeeNum!=tile.PhoneCur.EmployeeNum) { //We are on someone else's tile. So Let's do some checks before we assume we can take over this extension.				
				...
				if(ClockEvents.IsClockedIn(tile.PhoneCur.EmployeeNum)) { //This tile is taken by an employee who is clocked in.					
					...
					PhoneAsterisks.SetToDefaultQueue(tile.PhoneCur.EmployeeNum);
					return;
				}
				if(tile.PhoneCur.ClockStatus!=ClockStatusEnum.None
			...
			PhoneEmpDefaults.SetAvailable(tile.PhoneCur.Extension,employeeNum,Environment.MachineName);
			PhoneAsterisks.SetToDefaultQueue(tile.PhoneCur.EmployeeNum);
		...
		public static void Training(PhoneTile tile) {
			...
			PhoneAsterisks.SetQueueForExtension(tile.PhoneCur.Extension,AsteriskQueues.None);
		}
		...
		public static void Backup(PhoneTile tile) {
			...
			PhoneAsterisks.SetQueueForExtension(tile.PhoneCur.Extension,AsteriskQueues.Backup);
		}
		...
		public static void QueueTech(PhoneTile tile) {
			...
			PhoneAsterisks.SetQueueForExtension(tile.PhoneCur.Extension,AsteriskQueues.Tech);
		}
