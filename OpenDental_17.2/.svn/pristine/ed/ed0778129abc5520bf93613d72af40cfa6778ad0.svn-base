...
namespace OpenDental.UI {
    ...
    private bool IsUpdating;
		...
		private int _noteLengthLimit=200000;//The grid will take about 3 seconds to draw at this length. Any much longer and the application will become
		                                    //unresponsive.
    ...
    private int _mouseClickCount=0;
		...
		private const string URL_REGEX_STRING=@"[-a-zA-Z0-9@:%._\\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=,()]*)";
    ...
    public ODGridRowCollection RowsFiltered {
      get {
		...
		///If true, add all of your rows to Rows like normal, but use RowsFiltered to access your list of rows (when registering click events, selectedIndices, etc).</summary>
				...
				for(int i=0;i<rows.Count;i++) {
					...
					if(wrapText) {
						...
						for(int j=0;j<rows[i].Cells.Count;j++) {
							...
							//  cellH=(int)Math.Ceiling(g.MeasureString(rows[i].Cells[j].Text,cellFont,columns[j].ColWidth).Height+1);
							//}
							//else {
							//  cellH=rows[i].Height;
							//}
					...
					GridH+=rows[i].RowHeight+rows[i].NoteHeight;
					if(_hasDropDowns) {
		...
		private void setHeaderHeightHelper() {
			...
			using(Graphics g = Graphics.FromImage(doubleBuffer)) {
				foreach(ODGridColumn column in columns) {
					headerHeight=Math.Max(headerHeight,Convert.ToInt32(g.MeasureString(column.Heading,_headerFont,column.ColWidth).Height+2));
				}
			}
			doubleBuffer.Dispose();
			doubleBuffer=null;
			...
			for(int i=0;i<rowsFiltered[rowI].Cells.Count;i++) {
				...
				using(SolidBrush backBrush = new SolidBrush(ColorCell)) {
					g.FillRectangle(backBrush,
						-hScroll.Value+listColumns.Take(i).Sum(x =>x.ColWidth)+1,
						-vScroll.Value+1+titleHeight+headerHeight+rowsFiltered[rowI].RowLoc+1,
						...
						if(i==0) { //only draw the dropdown arrow in the first column of the row.
							using(Pen trianglePen = new Pen(Color.Black)) {
								if(rowsFiltered[rowI].DropDownState==ODGridDropDownState.Up) {
									...
									textRect.X+=10; //keep it a consistent 10 so the text doesn't shift left-right when the dropdown is toggled.
								}
				...
				using(StringFormat format=new StringFormat())
				{
					for(int i=0;i<columns.Count;i++) {
						...
						if(_hasAutoWrappedHeaders) {
							RectangleF layoutRec=new RectangleF(ColPos[i]-hScroll.Value,titleHeight+2,columns[i].ColWidth,headerHeight);
    ...
    protected override void OnClick(EventArgs e) {
      ...
      if(_hasDropDowns
				&& MouseDownRow>-1 && MouseDownCol>-1 && RowsFiltered[MouseDownRow].DropDownState!=ODGridDropDownState.None
    ...
    private static void OpenWikiPage(string pageTitle) {
		...
		private static void OpenWebPage(string pageTitle) {
			try {
				Process.Start("http://"+pageTitle);//Web browser will handle if page is actually https://
			}
			catch {
      ...
      selectedIndices=new ArrayList();
		...
		public void PrintHeader(Graphics g,int x,int y) {
			...
			for(int i=0;i<columns.Count;i++) {
				...
				g.DrawString(columns[i].Heading,_headerFont,Brushes.Black,
					(float)x+(-hScroll.Value+ColPos[i]+columns[i].ColWidth/2-g.MeasureString(columns[i].Heading,_headerFont).Width/2),
					(float)y+1);
				if(sortedByColumnIdx==i) {
		...
		public int PrintPage(Graphics g,int pageNumber,Rectangle bounds,int marginTopFirstPage,bool HasHeaderSpaceOnEveryPage=false) {
			...
			try {
				...
				for(int i=0;i<columns.Count;i++) {
					g.DrawString(columns[i].Heading,_headerFont,Brushes.Black,
						xPos+(float)ColPos[i]+columns[i].ColWidth/2-g.MeasureString(columns[i].Heading,_headerFont).Width/2,
						yPos);
				}
				yPos+=headerHeight;
      ...
      _mouseClickCount=0;
		...
		private void CreateComboBox() {
			...
			comboBox.Size=new Size(Columns[selectedCell.X].ColWidth+1,rows[selectedCell.Y].RowHeight+1);
		...
		private void CreateEditBox() {
			...
			if(_hasEditableRTF) {
				...
				editRichBox.Size=new Size(Columns[selectedCell.X].ColWidth-1,rows[selectedCell.Y].RowHeight-1);
				...
				//Rich text boxes have strange borders (3D looking) and so we have to manipulate the size and location differently.
				editTextBox.Size=new Size(Columns[selectedCell.X].ColWidth+1,rows[selectedCell.Y].RowHeight+1);
