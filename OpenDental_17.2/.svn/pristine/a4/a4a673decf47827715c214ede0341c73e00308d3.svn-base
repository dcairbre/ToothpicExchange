...
//				}
		...
		public static string AddColumnEnd(string tableName,DbSchemaCol col,int tabInset,bool doRunQuery) {
			...
			strb.Append(tb+"if(DataConnection.DBtype==DatabaseType.MySql) {");
			command="ALTER TABLE "+tableName+" ADD "+col.ColumnName+" "+GetMySqlType(col)+(col.DataType==OdDbType.DateTimeStamp ? "" : " NOT NULL")+(col.DataType==OdDbType.Date ? " DEFAULT '0001-01-01'" : "");
