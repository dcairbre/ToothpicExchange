...
namespace OpenDental {
	public partial class UserControlTasks:UserControl {
		...
		public void LayoutToolBar() {
			...
			button.Text=Lan.g(this,"Block Subsc");
			...
			InboxButton.Text=Lan.g(this,"Block Inbox");
		...
		private void FillGrid(){
			...
			if(tabContr.SelectedTab==tabNew && !PrefC.GetBool(PrefName.TasksNewTrackedByUser,dictPrefs)) {//The old way
			...
			for(int i=0;i<_listTasks.Count;i++) {
				...
				if(_listTasks[i].ObjectType==TaskObjectType.Patient) {
					...
					if(_listTasks[i].KeyNum!=0) {
						Appointment AptCur=Appointments.GetOneApt(_listTasks[i].KeyNum);
						if(AptCur!=null) {
				...
				if(!_listTasks[i].Descript.StartsWith("==") && _listTasks[i].UserNum!=0) {
					objDesc+=Userods.GetName(_listTasks[i].UserNum,listUserods)+" - ";
				}
				notes="";
				...
				if(!_listExpandedTaskNums.Contains(_listTasks[i].TaskNum) && listNotesForTask.Count>1) {
					...
					notes+="\r\n\u22EE\r\n" //Vertical ellipse followed by last note. \u22EE - vertical ellipses
							+"=="+Userods.GetName(lastNote.UserNum,listUserods)+" - "
					...
					foreach(TaskNote note in listNotesForTask) {
						notes+="\r\n"//even on the first loop
							+"=="+Userods.GetName(note.UserNum,listUserods)+" - "
				...
				row=new ODGridRow();
				if(PrefC.GetBool(PrefName.TasksNewTrackedByUser,dictPrefs)) {//The new way
					if(_listTasks[i].TaskStatus==TaskStatusEnum.Done) {
						row.Cells.Add("1");
					}
					else {
				...
				row.ColorBackG=DefC.GetColor(DefCat.TaskPriorities,_listTasks[i].PriorityDefNum,arrayDefs);//No need to do any text detection for triage priorities, we'll just use the task priority colors.
		...
		private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(tabContr.SelectedTab==tabNew && e.Col==2 && PrefC.GetBool(PrefName.TasksNewTrackedByUser)) {
				return;//Don't double click on expand column
			}
			else if(tabContr.SelectedTab==tabNew && e.Col==3 && !PrefC.GetBool(PrefName.TasksNewTrackedByUser)) {//Extra column
				return;//Don't double click on expand column
			}
			else if(e.Col==1) {
				return;//Don't double click on expand column
