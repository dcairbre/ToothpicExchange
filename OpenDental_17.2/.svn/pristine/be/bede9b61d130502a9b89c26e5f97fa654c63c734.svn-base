...
namespace OpenDentBusiness{
	...
	public class Security{
		...
		public static bool IsAuthorized(Permissions perm,DateTime date,bool suppressMessage,bool suppressLockDateMessage,long userGroupNum) {
			return IsAuthorized(perm,date,suppressMessage,suppressLockDateMessage,userGroupNum,0,-1,0);
		}
			...
			double procCodeFee,long sheetDefNum,long fKey) 
		{
			...
			try {
				return IsAuthorized(perm,date,suppressMessage,suppressLockDateMessage,curUser.UserGroupNum,procCodeNum,procCodeFee,sheetDefNum,fKey);
			}
			catch(Exception ex) {
			...
			long procCodeNum,double procFee,long sheetDefNum,long fKey) 
		{
			...
			if(!GroupPermissions.HasPermission(userGroupNum,perm,fKey)){
				if(!suppressMessage){
		...
		public static bool IsGlobalDateLock(Permissions perm,DateTime date,bool isSilent=false,long codeNum=0,double procFee=-1,long sheetDefNum=0) {
			...
			if(!PrefC.GetBool(PrefName.SecurityLockIncludesAdmin) && GroupPermissions.HasPermission(Security.CurUser.UserGroupNum,Permissions.SecurityAdmin,0)) {
				return false;//admins are never affected by global date limitation when preference enabled.
			}
			if(perm.In(Permissions.ProcComplCreate,Permissions.ProcComplEdit) 
