		...
		*/
		...
		public static double GetClinicBalance(long clinicNum) {
			...
			if(PrefC.GetBool(PrefName.EasyNoClinics)) {
				...
				if(clinicNum==0 && Clinics.ListShort.Count>0) {//Sending text for "Unassigned" patient.  Use the first non-hidden clinic. (for now)
					clinicNum=Clinics.ListShort[0].ClinicNum;
				}
		...
		public static bool IsIntegratedTextingEnabled() {
			...
			foreach(Clinic clin in Clinics.ListLong) {
				if(clin.SmsContractDate.Year>1880) {
					return true;
				}
			} 
			return false;
