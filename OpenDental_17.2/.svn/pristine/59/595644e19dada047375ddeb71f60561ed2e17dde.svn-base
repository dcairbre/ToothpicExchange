...
namespace OpenDental{
	...
	public class FormApptViewEdit : ODForm {
		...
		private void FormApptViewEdit_Load(object sender, System.EventArgs e) {
			...
			_listApptViewItems=ApptViewItemC.List.ToList().FindAll(x => x.ApptViewNum==ApptViewCur.ApptViewNum);
			...
			for(int i=0;i<ProviderC.ListShort.Count;i++){
				...
				if(_listApptViewItems.Select(x => x.ProvNum).Contains(ProviderC.ListShort[i].ProvNum)) {
		...
		private void FillOperatories() {
			...
			for(int i=0;i<OperatoryC.ListShort.Count;i++) {
					...
					|| OperatoryC.ListShort[i].ClinicNum==ClinicNum) //or the operatory is assigned to same clinic as this view
				{
					...
					if(_listApptViewItems.Select(x => x.OpNum).Contains(OperatoryC.ListShort[i].OperatoryNum)) {
