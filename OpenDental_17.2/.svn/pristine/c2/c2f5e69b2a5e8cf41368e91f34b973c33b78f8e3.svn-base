...
namespace OpenDental {
	...
	public class ContrChart : System.Windows.Forms.UserControl	{
		...
		public void LayoutToolBar(){
			...
			button=new ODToolBarButton(OrthoChartTabs.Listt[0].TabName,-1,"","Ortho");
			if(OrthoChartTabs.Listt.Count>1) {
				button.Style=ODToolBarButtonStyle.DropDownButton;
...
//todo: optimize for Full mode:
		...
		private bool NewCropRefreshPrescriptions() {
			...
			foreach(XmlNode nodeTable in nodeNewDataSet.ChildNodes) {
				...
				if(erxLog!=null && erxLog.ProvNum!=0 && erxLog.DateTStamp.Date==rx.RxDate.Date) {
					Provider provErxLog=ProviderC.ListLong.FirstOrDefault(x => x.ProvNum==erxLog.ProvNum);
				...
				if(rxOld==null) {
					...
					//Preserve the pharmacy on the existing prescription, in case the user set the value manually.  We do not pull pharmacy back from eRx yet.
				...
				//In this special situation, we instead set the MedDescript, RxCui and NewCropGuid columns.
				long medicationPatNum=MedicationPats.InsertOrUpdateMedOrderForRx(rx,rxCui,isProv);
			...
			foreach(MedicationPat medication in listAllMedicationsForPatient) {
				if(String.IsNullOrEmpty(medication.NewCropGuid)) {
		...
		private void Tool_eRx_Click() {
			...
			Employee emp=null;
			bool isEmp=false;
			if(Security.CurUser.EmployeeNum==0) {//The current user does not have an employee associated.
				isEmp=false;
			}
			else if(Security.CurUser.ProvNum==0) {//The current user has an employee associated and no provider associated.
				isEmp=true;
			}
			...
			if(!prov.IsErxEnabled) {
				MessageBox.Show(Lan.g(this,"Erx is disabled for provider")+": "+prov.Abbr+".  "+Lan.g(this,"To enable, edit provider in Lists | Providers and acknowledge Electronic Prescription fees."));
				return;
			}
			if(prov.IsHidden) {
				MessageBox.Show(Lan.g(this,"Provider")+": "+prov.Abbr+" "+Lan.g(this,"is hidden")+".  "+Lan.g(this,"Unhide the provider to use Erx features."));
				return;
			}
			if(prov.IsNotPerson) {
			...
			if(Regex.Replace(fname,"[^A-Za-z\\- ]*","")!=fname) {
				MessageBox.Show(Lan.g(this,"Provider first name can only contain letters, dashes, or spaces.")+": "+prov.Abbr);
				return;
			}
			...
			if(Regex.Replace(lname,"[^A-Za-z\\- ]*","")!=lname) { //Will catch situations such as "Dale Jr. III" and "Ross DMD".
				MessageBox.Show(Lan.g(this,"Provider last name can only contain letters, dashes, or spaces.  Use the suffix box for I, II, III, Jr, or Sr")+": "+prov.Abbr);
				return;
			}
...
//			IE.Navigate(newCropUrl,null,null,PostDataBytes,additionalHeaders);
		...
		private void UpdateErxAccess(string npi) {
			...
			DateTime dateLastAccessMonth=PrefC.GetDate(PrefName.NewCropDateLastAccessCheck);
...
//todo: compare with: Bridges.ECW.AptNum, no need to generate PDF segment, pdfs only with eCW and this button not available with eCW integration
		...
		private void menuOrthoChart_Popup(object sender,EventArgs e) {
			...
			for(int i=1;i<OrthoChartTabs.Listt.Count;i++) {//Start i at 1 so tha we do not duplicate the first ortho tab (the main button)
				MenuItem menuItem=new MenuItem(OrthoChartTabs.Listt[i].TabName);
				menuItem.Tag=i;
		...
		public void FillPtInfo() {
			...
			for(int f=0;f<fields.Count;f++) {
				...
				switch(fieldCur.InternalName) {
					...
					case "PatFields":
						PatField field;
						List<FieldDefLink> listFieldDefLinks=FieldDefLinks.GetForLocation(FieldLocations.Chart)
					...
					case "Pat Restrictions":
						List<PatRestriction> listPatRestricts=PatRestrictions.GetAllForPat(PatCur.PatNum);
						...
						for(int i=0;i<listPatRestricts.Count;i++) {
							...
							row.ColorBackG=DefC.Short[(int)DefCat.MiscColors][10].ItemColor;//index 10 is Patient Restrictions (hard coded in convertdatabase4)
					...
					case "Patient Portal":
						...
						if(!Patients.HasPatientPortalAccess(PatCur.PatNum)) {
					...
					case "Referred From":
						List<RefAttach> RefAttachList=RefAttaches.Refresh(PatCur.PatNum).DistinctBy(x => x.ReferralNum).ToList();
						string referral="";
		...
		private bool ShouldDisplayProc(DataRow row,Hashtable hashPrograms) {
			...
			if(Programs.IsEnabled(ProgramName.Orion,hashPrograms)) {
		...
		public void FillProgNotes(bool retainSelection=false) {
			if(PatCur!=null && PatCur.PatNum>0) {
				...
				TreatPlans.AuditPlans(PatCur.PatNum,tpTypeCur);//consider moving this, to reduce calls to db
			}
			...
			if(checkShowTeeth.Checked) {
				...
				foreach(DataRow rowCur in table.Rows) {
					...
					if(ShouldDisplayProc(rowCur,hashPrograms)) {
			...
			foreach(DataRow rowCur in table.Rows) {
				...
				if(PrefC.GetBool(PrefName.UseProviderColorsInChart,dictPrefs)
						&& procNumCur>0
						&& provNum>0
		...
		private void FillToothChart(bool retainSelection,DateTime dateLimit){
			...
			foreach(DataRow row in _listProcsSkipped) {
				if(((ProcStat)PIn.Long(row["ProcStatus"].ToString())).In(ProcStat.C,ProcStat.EO,ProcStat.EC)) {
					ProcedureCode procCode=ProcedureCodes.GetProcCode(row["ProcCode"].ToString(),hashProcedureCodes);
			...
			DrawProcGraphics(dateLimit,hashProcedureCodes);
			toothChart.ResumeLayout();
			FillMovementsAndHidden();
			Cursor=Cursors.Default;
		}
		...
		private void FillTrackSlider() {
			...
			int trackIndex=trackToothProcDates.Value;
			...
			//Clear out the list of unique procedure dates and then fill them with the new values found in the ProgNotes data table.
			...
			_listProcDates=new List<DateTime>();
			foreach(DataRow rowCur in tableCur.Rows) {
				if(!_procListOrig.Select(x => x["ProcNum"].ToString()).Contains(rowCur["ProcNum"].ToString())) {
					continue;
				}
				if(DateTime.TryParse(rowCur["DateTP"].ToString(),out dateTP) && !_listProcDates.Contains(dateTP)) {
					_listProcDates.Add(dateTP);
				}
				if(DateTime.TryParse(rowCur["DateEntryC"].ToString(),out dateComplete) && !_listProcDates.Contains(dateComplete)) {
					_listProcDates.Add(dateComplete);
				}
				if(DateTime.TryParse(rowCur["ProcDate"].ToString(),out dateScheduled) && !_listProcDates.Contains(dateScheduled)) {
					_listProcDates.Add(dateScheduled);
				}
			}
			//Always make sure that the time bar contains today's date so that the user can get the tooth chart back to the "current" state.
			if(!_listProcDates.Contains(DateTime.Today)) {
				_listProcDates.Add(DateTime.Today);
			}
			//There is a possibility that one of the dates above has yet to be set or was never set (for any reason) and we do not want 01/01/01 available.
			_listProcDates.RemoveAll(x => x == DateTime.MinValue);
			_listProcDates.Sort();//Use the default DateTime comparer.
			trackToothProcDates.Maximum=_listProcDates.Count()-1;
			//Always default to today's date which is guaranteed to be in our track bar.
			trackToothProcDates.Value=_listProcDates.FindIndex(x => x==DateTime.Today);
			textToothProcDate.Text=_listProcDates[trackToothProcDates.Value].ToShortDateString();
		}

		private void DrawProcGraphics(DateTime dateLimit,Hashtable hashProcedureCodes){
			...
			for(int i=0;i<_procList.Count;i++) {
				...
				ProcedureCode procCode=ProcedureCodes.GetProcCode(_procList[i]["ProcCode"].ToString(),hashProcedureCodes);
				...
				if(procCode.GraphicColor.ToArgb()==Color.FromArgb(0).ToArgb()){
					switch((ProcStat)PIn.Long(_procList[i]["ProcStatus"].ToString())) {
						case ProcStat.C:
							cDark=arrayDefs[(int)DefCat.ChartGraphicColors][1].ItemColor;
							cLight=arrayDefs[(int)DefCat.ChartGraphicColors][6].ItemColor;
							break;
						case ProcStat.TP:
						case ProcStat.TPi:// TPi color should be the same as TP color.
							cDark=arrayDefs[(int)DefCat.ChartGraphicColors][0].ItemColor;
							cLight=arrayDefs[(int)DefCat.ChartGraphicColors][5].ItemColor;
							break;
						case ProcStat.EC:
							cDark=arrayDefs[(int)DefCat.ChartGraphicColors][2].ItemColor;
							cLight=arrayDefs[(int)DefCat.ChartGraphicColors][7].ItemColor;
							break;
						case ProcStat.EO:
							cDark=arrayDefs[(int)DefCat.ChartGraphicColors][3].ItemColor;
							cLight=arrayDefs[(int)DefCat.ChartGraphicColors][8].ItemColor;
							break;
						case ProcStat.R:
							cDark=arrayDefs[(int)DefCat.ChartGraphicColors][4].ItemColor;
							cLight=arrayDefs[(int)DefCat.ChartGraphicColors][9].ItemColor;
							break;
						case ProcStat.Cn:
							cDark=arrayDefs[(int)DefCat.ChartGraphicColors][16].ItemColor;
							cLight=arrayDefs[(int)DefCat.ChartGraphicColors][17].ItemColor;
							break;
					}
				}
				else{
		...
		private bool AddProcHelper(Procedure ProcCur) {
			...
			if(newStatus==ProcStat.C || newStatus==ProcStat.TP) {
				...
				if(!PrefC.GetBool(PrefName.ProcPromptForAutoNote)) {
					...
					while(ProcCur.Note.Contains("[[")) {
						int beginIdx=ProcCur.Note.IndexOf("[[");
						int endIdx=ProcCur.Note.IndexOf("]]");
						ProcCur.Note=ProcCur.Note.Remove(beginIdx,endIdx-beginIdx);
					}
				}
			}
			else {
			...
			if(newStatus==ProcStat.R || newStatus==ProcStat.EO || newStatus==ProcStat.EC) {
				...
				if(insPlanPrimary!=null && insPlanPrimary.PlanType=="p") {//PPO
					double ucrFee=Fees.GetAmount0(ProcCur.CodeNum,Providers.GetProv(Patients.GetProvNum(PatCur)).FeeSched,ProcCur.ClinicNum,ProcCur.ProvNum);
					if(ProcCur.ProcFee<ucrFee || PrefC.GetBool(PrefName.InsPpoAlwaysUseUcrFee)) {
						ProcCur.ProcFee=ucrFee;
					}
				}
			}
			if(newStatus==ProcStat.C 
			...
			for(int n=0;n==0 || n<toothChart.SelectedTeeth.Count;n++) {//always loops at least once.
				...
				if((tArea==TreatmentArea.Arch
					...
					if(toothChart.SelectedTeeth.Count==0) {
						...
						if(AutoCodeItemC.HList.ContainsKey(ProcCur.CodeNum)) { 
							autoCode=AutoCodeItems.GetListForCode((long)AutoCodeItemC.HList[ProcCur.CodeNum]).FirstOrDefault(x => x.CodeNum==ProcCur.CodeNum);
						}
		...
		private void butNew_Click(object sender, System.EventArgs e) {
			...
			if(AptCur.ProvHyg < 1) {
				...
				bool allProcsHyg=(myProcList.Count>0 && myProcList.Select(x=>ProcedureCodes.GetProcCode(x.CodeNum,listProcCodes)).ToList().All(x=>x.IsHygiene));
		...
		private void menuItemGroupSelected_Click(object sender,EventArgs e){
			...
			if(PrefC.GetBool(PrefName.ProcGroupNoteDoesAggregate)) {
				...
				if(!PrefC.GetBool(PrefName.ProcPromptForAutoNote)) {
					//Users do not want to be prompted for auto notes, so remove them all from the procedure note.
					while(group.Note.Contains("[[")) {
						int beginIdx=group.Note.IndexOf("[[");
						int endIdx=group.Note.IndexOf("]]");
						group.Note=group.Note.Remove(beginIdx,endIdx-beginIdx);
					}
				}
			}
			group.IsNew=true;
