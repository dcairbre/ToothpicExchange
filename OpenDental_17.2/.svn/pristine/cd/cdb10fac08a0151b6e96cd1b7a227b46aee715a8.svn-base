...
///<summary></summary>
	public class FormClaimPayBatch:ODForm {
		...
		private void FillClaimPayment() {
			...
			textPayGroup.Text=DefC.GetName(DefCat.ClaimPaymentGroups,ClaimPaymentCur.PayGroup);
		}
		...
		private void FillGrids(){
			...
			DateTime dateClaimUnpaid=DateTime.MinValue;
			if(!DateTime.TryParse(PrefC.GetDate(PrefName.ClaimPaymentNoShowZeroDate).ToShortDateString(),out dateClaimUnpaid)) {
				dateClaimUnpaid=PrefC.GetDate(PrefName.ClaimPaymentNoShowZeroDate);
			}
			ClaimsOutstanding=Claims.GetOutstandingClaims(textCarrier.Text,textLName.Text,textFName.Text,dateClaimUnpaid);
			gridOut.BeginUpdate();
