...
namespace OpenDentBusiness{
	...
	public class Tasks {
		...
		public static int TaskComparer(TaskCompareObj x,TaskCompareObj y) {
			...
			if(PrefC.GetBool(PrefName.TasksNewTrackedByUser,x.DictPrefs)) {
			...
			if(x.RowTask["PriorityDefNum"].ToString()!=y.RowTask["PriorityDefNum"].ToString()) {//we only care about task priority if they are different
				...
				return DefC.GetDef(DefCat.TaskPriorities,xTaskPriorityDefNum,x.ArrayDefs).ItemOrder.CompareTo(DefC.GetDef(DefCat.TaskPriorities,yTaskPriorityDefNum,x.ArrayDefs).ItemOrder);
			}
			//3)Sort by Date Time
		...
		public class TaskCompareObj {
			...
			public Dictionary<string,Pref> DictPrefs;
			public Def[][] ArrayDefs;
		}
