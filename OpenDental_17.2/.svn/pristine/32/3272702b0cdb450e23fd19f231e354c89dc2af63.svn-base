...
//Warning: this path is not handled properly if trailing slash is missing:
			...
			else if(insSub.AssignBen && clearinghouseClin.IsEraDownloadAllowed && (fieldInputter.MsgType=="21" || fieldInputter.MsgType=="23")) { //EOB and Predetermination EOB
				EOBImportHelper(fieldInputter,claimProcsClaim,procListAll,claimProcList,claim,false,showProviderTransferWindow);
		...
		public static void EOBImportHelper(CCDFieldInputter fieldInputter,List<ClaimProc> listClaimProcsForClaim,List<Procedure> listLabProcs,
			List<ClaimProc> listClaimProcs,Claim claim,bool isAutomatic,ShowProviderTransferWindowDelegate showProviderTransferWindow) {
			...
			foreach(int lineNumber in listMissingEOBProcs) {
				...
				ClaimProc cpByTotal=ByTotClaimProcHelper(claim,listPayPlans.Count==1?listPayPlans[0].PayPlanNum:0,isPreEob);
			...
			foreach(ClaimProc claimProcCur in listClaimProcsForClaim) {
				...
				//TODO: One day we might want to mark recieved automatically.
				//if(!isPreEob) {//Preauth claimproc retain the "Preauth" status.
		...
		private static ClaimProc ByTotClaimProcHelper(Claim claimCur,long payPlanNum,bool isPreEob) {
		...
		public static Clearinghouse GetCanadianClearinghouseHq(Carrier carrier){
			...
			for(int i=0;i<arrayClearinghousesHq.Length;i++) {
				if(PrefC.GetLong(PrefName.ClearinghouseDefaultDent)==arrayClearinghousesHq[i].ClearinghouseNum && arrayClearinghousesHq[i].Eformat==ElectronicClaimFormat.Canadian) {
					return arrayClearinghousesHq[i];
				}
			}
			//Return the first Canadian clearinghouse if one exists.
			for(int i=0;i<arrayClearinghousesHq.Length;i++) {
				if(arrayClearinghousesHq[i].Eformat==ElectronicClaimFormat.Canadian) {
					return arrayClearinghousesHq[i];
				}
			}
