...
namespace OpenDental {
	public partial class FormOrthoChart:ODForm {
		...
		private void FillGridPat() {
			...
			List<FieldDefLink> listFieldDefLinks=FieldDefLinks.GetForLocation(FieldLocations.OrthoChart)
			...
			for(int i=0;i<PatFieldDefs.ListShort.Count;i++) {
				...
				row.Tag=PatFieldDefs.ListShort[i];
		...
		private void gridMain_CellTextChanged(object sender,EventArgs e) {
			...
			if(Security.IsAuthorized(Permissions.OrthoChartEditUser,orthoDate,true) 
				&& !Security.IsAuthorized(Permissions.OrthoChartEditFull,orthoDate,true)) 
			{
				//User has permission to edit the ones that they have signed.
				if(!_showSigBox) {
					return;
				}
			...
			else if(!Security.IsAuthorized(Permissions.OrthoChartEditFull,orthoDate,true)) {
		...
		private void gridMain_CellLeave(object sender,ODGridClickEventArgs e) {
			...
			//User does not have full permission.  Check if the row was signed by them.  If so, allow changes.  If not, don't allow changes.
			OrthoChart sigChart=null;
			if(_showSigBox) {
				sigChart=_dictOrthoCharts[orthoDate].Find(x => x.FieldName==_listDisplayFieldNames[_sigTableOrthoColIdx]);
			}
			OrthoSignature sig=null;
			if(sigChart!=null) {
				sig=new OrthoSignature(sigChart.FieldValue);
			}
			if(Security.IsAuthorized(Permissions.OrthoChartEditUser,orthoDate,true) 
				&& ((sigChart==null || sig.SigString=="") || Security.CurUser.UserNum==sigChart.UserNum))//There is no signature or the signature is their own.
			{
				if(newText != oldText) {
					SetValueInDict(newText,orthoDate,(string)gridMain.Columns[e.Col].Tag);
					_hasChanged=true;//They had permission and they made a change.
		...
		private bool CanEditRow(DateTime orthoDate) {
			...
			else if(!Security.IsAuthorized(Permissions.OrthoChartEditUser,orthoDate,true)) {
			...
			return string.IsNullOrEmpty(sig.SigString);//User has partial permission and somebody else signed it.
		}
		...
		private void gridPat_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(field==null) {
				...
				field.FieldName=patFieldDef.FieldName;
		...
		private void butAudit_Click(object sender,EventArgs e) {
			...
			SecurityLog[] orthoChartLogs=SecurityLogs.Refresh(_patCur.PatNum,new List<Permissions> { Permissions.OrthoChartEditFull },null);
		...
		private void pd_PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
			...
			using(Font subHeadingFont = new Font("Arial",10,FontStyle.Bold)) {
				...
				#region printHeading
				//========== TITLE ==========
				string title=OrthoChartTabs.Listt[0].TabName;
