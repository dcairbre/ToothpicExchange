				...
				if(etranOriginal!=null) { //Null will happen when testing, but should not happen in production.
					...
					if(!exit) {
						...
						if(claim!=null) {
							...
							Claims.Update(claim);
							if((fieldA04.valuestr=="21" || fieldA04.valuestr=="23") && clearinghouseClin.IsEraDownloadAllowed && InsSubs.GetOne(claim.InsSubNum).AssignBen) { //EOB and Predetermination EOB
								...
								Canadian.EOBImportHelper(fieldInputter,listClaimProcsForClaim,listAllProcs,listAllClaimProcs,claim,true,null);
