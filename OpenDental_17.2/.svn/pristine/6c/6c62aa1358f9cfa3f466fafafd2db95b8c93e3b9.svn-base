    ...
    "";
		...
		private void FillManualRecall() {
			...
			for(int i=0;i<DefC.Short[(int)DefCat.RecallUnschedStatus].Length;i++) {
				...
				comboStatusTextedRecall.Items.Add(DefC.Short[(int)DefCat.RecallUnschedStatus][i].ItemName);
				comboStatusEmailTextRecall.Items.Add(DefC.Short[(int)DefCat.RecallUnschedStatus][i].ItemName);
				...
				if(DefC.Short[(int)DefCat.RecallUnschedStatus][i].DefNum==PrefC.GetLong(PrefName.RecallStatusTexted)) {
					comboStatusTextedRecall.SelectedIndex=i;
				}
				if(DefC.Short[(int)DefCat.RecallUnschedStatus][i].DefNum==PrefC.GetLong(PrefName.RecallStatusEmailedTexted)) {
			...
			listRecallCache=RecallTypeC.Listt.Where(x => x.Description!="Child Prophy").ToList();
		...
		private void FillGrid(){
			string webSchedMessageFields="[NameF], [DueDate], [ClinicName], [ClinicPhone], [PracticeName], [PracticePhone], [OfficePhone], [URL]";
			...
			row.Cells.Add("1");
			row.Cells.Add(Lan.g(this,"WebSched Email"));
			row.Cells.Add(Lan.g(this,"Email body.  Available variables")+": "+webSchedMessageFields);
			...
			row.Cells.Add(Lan.g(this,"Available variables")+": "+webSchedMessageFields);
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			if(comboStatusTextedRecall.SelectedIndex==-1) {
				...
				Prefs.UpdateLong(PrefName.RecallStatusTexted,DefC.Short[(int)DefCat.RecallUnschedStatus][comboStatusTextedRecall.SelectedIndex].DefNum);
			}
			if(comboStatusEmailTextRecall.SelectedIndex==-1) {
				Prefs.UpdateLong(PrefName.RecallStatusEmailedTexted,0);
			}
			else {
				Prefs.UpdateLong(PrefName.RecallStatusEmailedTexted,DefC.Short[(int)DefCat.RecallUnschedStatus][comboStatusEmailTextRecall.SelectedIndex].DefNum);
			}
