		...
		public static void Execute(long programNum,Patient pat) {
			...
			try{
				...
				if(pat!=null) {
					...
					cmdline=cmdline.Replace("[Gender]",pat.Gender.ToString());
					cmdline=cmdline.Replace("[ProvNum]",pat.PriProv.ToString());
					cmdline=cmdline.Replace("[ClinicNum]",pat.ClinicNum.ToString());
					...
					path=path.Replace("[Gender]",pat.Gender.ToString());
					path=path.Replace("[ProvNum]",pat.PriProv.ToString());
					path=path.Replace("[ClinicNum]",pat.ClinicNum.ToString());
