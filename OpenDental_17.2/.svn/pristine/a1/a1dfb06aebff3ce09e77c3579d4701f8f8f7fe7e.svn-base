	...
	public partial class ConvertDatabases {
		...
		private static void To16_2_14() {
			if(FromVersion<new Version("16.2.14.0")) {
				...
				try {
					if(DataConnection.DBtype==DatabaseType.MySql) {
						if(!IndexExists("procedurelog","ClinicNum")) {
		...
		private static void To16_2_20() {
			if(FromVersion<new Version("16.2.20.0")) {
				...
				try {
					if(DataConnection.DBtype==DatabaseType.MySql) {
						if(!IndexExists("claimproc","indexAcctCov")) {
				...
				try {
					if(DataConnection.DBtype==DatabaseType.MySql) {
						if(!IndexExists("adjustment","indexPNAmt")) {
				...
				try {
					if(DataConnection.DBtype==DatabaseType.MySql) {
						if(!IndexExists("paysplit","indexPNAmt")) {
		...
		private static void To16_2_26() {
			if(FromVersion<new Version("16.2.26.0")) {
				...
				try {
					if(DataConnection.DBtype==DatabaseType.MySql) {
						if(!IndexExists("document","PatNum,DocCategory")) {
		...
		private static void To16_2_29() {
			if(FromVersion<new Version("16.2.29.0")) {
				...
				try {
					if(DataConnection.DBtype==DatabaseType.MySql) {
						if(!IndexExists("patient","HmPhone")) {
				...
				try {
					if(DataConnection.DBtype==DatabaseType.MySql) {
						if(!IndexExists("patient","WirelessPhone")) {
				...
				try {
					if(DataConnection.DBtype==DatabaseType.MySql) {
						if(!IndexExists("patient","WkPhone")) {
		...
		private static void To16_2_32() {
			if(FromVersion<new Version("16.2.32.0")) {
				...
				try {
					if(DataConnection.DBtype==DatabaseType.MySql) {
						if(!IndexExists("insverify","VerifyType")) {
		...
		private static void To16_2_33() {
			if(FromVersion<new Version("16.2.33.0")) {
				...
				try {
					if(DataConnection.DBtype==DatabaseType.MySql) {
						if(!IndexExists("adjustment","AdjDate")) {
		...
		private static void To16_3_1() {
			if(FromVersion<new Version("16.3.1.0")) {
				...
				try {
					if(DataConnection.DBtype==DatabaseType.MySql) {
						if(!IndexExists("periomeasure","PerioExamNum")) {
				...
				try {
					if(DataConnection.DBtype==DatabaseType.MySql) {
						if(!IndexExists("perioexam","PatNum")) {
				...
				if(DataConnection.DBtype==DatabaseType.MySql) {
					...
					command="INSERT INTO program (ProgramNum,ProgName,ProgDesc,Enabled,FilePath) "
						+"VALUES((SELECT MAX(ProgramNum)+1 FROM program),'FHIR','RESTful API following the HL7 FHIR standard',0,'')";
					fhirProgNum=Db.NonQ(command,true);
				}
				if(DataConnection.DBtype==DatabaseType.MySql) {
					command="INSERT INTO programproperty (ProgramNum,PropertyDesc,PropertyValue,ComputerName,ClinicNum) "
				...
				try {
					if(DataConnection.DBtype==DatabaseType.MySql) {
						if(!IndexExists("procedurelog","PatNum,ProcStatus,ProcFee,UnitQty,BaseUnits,ProcDate")) {
				...
				try {
					if(DataConnection.DBtype==DatabaseType.MySql) {
						if(!IndexExists("claimproc","Status,PatNum,DateCp,PayPlanNum,InsPayAmt,WriteOff,InsPayEst,ProcDate")) {
        ...
        Db.NonQ(command);
				...
				if(DataConnection.DBtype==DatabaseType.MySql) {
					...
					command="INSERT INTO definition(DefNum,Category,ItemOrder,ItemName) VALUES((SELECT MAX(DefNum)+1 FROM definition),40,0,'Default')";
					insertID=Db.NonQ(command,true);
				}
				...
				try {
					if(DataConnection.DBtype==DatabaseType.MySql) {
						if(!IndexExists("xwebresponse","DateTUpdate")) {
				...
				if(eConfirmSentDefNum==0) {//Insert new def.
					if(DataConnection.DBtype==DatabaseType.MySql) {
						...
						command="INSERT INTO definition (DefNum,Category,ItemOrder,ItemName,ItemValue) VALUES ((SELECT MAX(DefNum)+1 from definition),2,"
							+POut.Long(itemOrder++)+",'eConfirmSent','eConfirmSent')";
					}
					eConfirmSentDefNum=Db.NonQ(command,true);
				}
				...
				if(eConfirmCallBackDefNum==0) {//Insert new def.
					...
					eConfirmCallBackDefNum=Db.NonQ(command,true);
				}
				...
				if(eConfirmFailureDefNum==0) {//Insert new def.
					...
					eConfirmFailureDefNum=Db.NonQ(command,true);
				}
				...
				if(eApptConfirmedDefNum==0) {//Insert new def.
					...
					eApptConfirmedDefNum=Db.NonQ(command,true);
				}
			...
			if(FromVersion<new Version("16.4.1.0")) {
				...
				if(defNum==0) {//Sales Tax adjustment definition doesn't exist.  Create one for them.
					if(DataConnection.DBtype==DatabaseType.MySql) {
						...
						command="INSERT INTO definition (DefNum,Category,ItemName,ItemValue) VALUES ((SELECT MAX(DefNum)+1 FROM definition),'1','Sales Tax','+')";
						defNum=Db.NonQ(command,true);
					}
				}
				if(DataConnection.DBtype==DatabaseType.MySql) {
		...
		private static void To16_4_15() {
			if(FromVersion<new Version("16.4.15.0")) {
				...
				try {
					if(DataConnection.DBtype==DatabaseType.MySql) {
						if(!IndexExists("claimproc","DateCP")) {
