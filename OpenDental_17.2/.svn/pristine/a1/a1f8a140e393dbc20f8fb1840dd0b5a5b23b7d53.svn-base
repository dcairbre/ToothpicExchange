...
namespace OpenDental{
	...
	public class FormApptEdit : ODForm {
		...
		public FormApptEdit(long aptNum,long patNum=0,bool useApptDrawingSettings=false) {
			//
		...
		private void FormApptEdit_Load(object sender, System.EventArgs e) {
			...
			_labCur=IsNew?null:LabCases.GetForApt(AptCur);
		...
		private void butDeleteProc_Click(object sender,EventArgs e) {
			...
			try{
				for(int i=gridProc.SelectedIndices.Length-1;i>=0;i--) {
					...
					if(!proc.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC)
						&& !Security.IsAuthorized(Permissions.ProcDelete,proc.DateEntryC,true)) 
					{
						skippedSecurity++;
						continue;
					}
					if(proc.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC)
						&& !Security.IsAuthorized(Permissions.ProcComplEdit,proc.DateEntryC,true)) 
					{
						skippedSecurity++;
						continue;
					}
		...
		private void butAdd_Click(object sender,EventArgs e) {
			...
			Procedure proc=Procedures.ConstructProcedureForAppt(FormP.SelectedCodeNum,AptCur,pat,_listPatPlans,PlanList,SubList,_listProcCodes,_listProvidersAll);
		...
		private void listQuickAdd_MouseDown(object sender,System.Windows.Forms.MouseEventArgs e) {
			...
			for(int i=0;i<codes.Length;i++) {
				...
				if(priplan!=null && priplan.PlanType=="p") {//PPO
					...
					if(proc.ProcFee<standardfee || PrefC.GetBool(PrefName.InsPpoAlwaysUseUcrFee)) {	//check if preference is enabled
		...
		private bool UpdateListAndDB(bool isClosing=true){
			...
			#endregion
			#region Save ApptCur changes to DB
			try {
			...
			#endregion
			#region Set and save Proc changes to DB 
			Procedures.ProcsAptNumHelper(_listProcsForAppt,AptCur,_listAppointments,gridProc.SelectedIndices.ToList(),_listProcNumsAttachedStart,_listProcCodes,_isPlanned);
			...
			if(AptCur.AptStatus!=ApptStatus.Complete && (_selectedProvNum!=AptOld.ProvNum || _selectedProvHygNum!=AptOld.ProvHyg)) {//Either the primary or hygienist changed.
				...
				string promptText="";
				updateProcFees=Procedures.FeeUpdatePromptHelper(listNewProcs,listOldProcs,ref promptText);
				if(updateProcFees && promptText!="" && !MsgBox.Show(this,MsgBoxButtons.YesNo,promptText)) {
					updateProcFees=false;
				}
			}
			#endregion
			if(Procedures.UpdateProcsInApptHelper(_listProcsForAppt,pat,AptCur,AptOld,PlanList,SubList,gridProc.SelectedIndices.ToList(),updateProcFees)){ 
				AutomationL.Trigger(AutomationTrigger.CompleteProcedure,_listProcsForAppt.FindAll(x => x.AptNum==AptCur.AptNum).Select(x => ProcedureCodes.GetStringProcCode(x.CodeNum)).ToList(),AptCur.PatNum);
			}
			#endregion
		...
		private bool AptTypeHelper() {
			...
			if(listAptTypeProcs.Count>0) {//AppointmentType is associated to procs.
				...
				Appointments.ApptTypeMissingProcHelper(AptCur,aptTypeCur,_listProcsForAppt,pat,true,_listPatPlans,SubList,PlanList,_listProvidersAll,_listProcCodes,_benefitList);
				FillProcedures();
				...
				foreach(ProcedureCode procCodeCur in listAptTypeProcs) {
					foreach(Procedure proc in _listProcsForAppt) {
						...
						if((_isPlanned && (proc.PlannedAptNum==0 || proc.PlannedAptNum == AptCur.AptNum))
							|| (!_isPlanned && (proc.AptNum==0 || proc.AptNum == AptCur.AptNum))) {
							for(int i = 0;i < gridProc.Rows.Count;i++) {
								Procedure rowProc = (Procedure)gridProc.Rows[i].Tag;
								if(rowProc.ProcNum == proc.ProcNum) {
