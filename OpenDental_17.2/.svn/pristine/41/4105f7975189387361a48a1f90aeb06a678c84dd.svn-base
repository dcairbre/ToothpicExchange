...
namespace OpenDentBusiness {
	...
	public class Recalls {
		...
		public static Recall GetRecallProphyOrPerio(long patNum,bool excludeScheduled=false) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<Recall>(MethodBase.GetCurrentMethod(),patNum,excludeScheduled);
			} 
			...
			if(excludeScheduled) {
				command+=" AND DateScheduled='0001-01-01'";//Exclude scheduled recalls
			}
				...
				&& patCur.Birthdate.AddYears(PrefC.GetInt(PrefName.RecallAgeAdult)) > ((recallCur.DateDue>DateTime.Today)?recallCur.DateDue:DateTime.Today)) 
			{
				for(int i=0;i<listRecallTypes.Count;i++) {
					if(listRecallTypes[i].RecallTypeNum==RecallTypes.ChildProphyType
						&& recallCur.RecallTypeNum == listRecallTypes[i].RecallTypeNum) 
					{
			...
			long siteNum,RecallListSort sortBy,RecallListShowNumberReminders showReminders) 
		{
			...
			//Create one deep copy of the preference cache that will be used throughout the looping in this entire method.
			...
			if(PrefC.GetString(PrefName.RecallTypesShowingInList,dictPrefs)!="") {
				command+="AND recall.RecallTypeNum IN("+PrefC.GetString(PrefName.RecallTypesShowingInList,dictPrefs)+") ";
			}
			if(PrefC.GetBool(PrefName.RecallExcludeIfAnyFutureAppt,dictPrefs)) {
			...
			//Check the commlog and webschedrecall tables to find if any reminders have been sent to these patients.
			command="SELECT PatNum,CommDateTime AS DateReminder,0 AS EmailSendStatus,0 AS SmsSendStatus,"//0 to indicate AutoCommStatus.Undefined
				+"'' AS WebSchedSendError,CommDateTime AS DateTimeEntry,0 AS WebSchedRecallNum,'0001-01-01' AS DateTimeSendFailed "
				+"FROM commlog "
				+"WHERE CommType="+POut.Long(Commlogs.GetTypeAuto(CommItemTypeAuto.RECALL))+" "
				+"AND PatNum IN ("+string.Join(",",dictPatientRows.Keys)+") "
				+"UNION ALL "
				+"SELECT PatNum,DateTimeReminderSent AS DateReminder,EmailSendStatus,SmsSendStatus,"
				+"ResponseDescript AS WebSchedSendError,DateTimeEntry,WebSchedRecallNum,DateTimeSendFailed "
				+"FROM webschedrecall "
				+"WHERE PatNum IN ("+string.Join(",",dictPatientRows.Keys)+")"; ;
			DataTable rawCommlogTable=Db.GetTable(command);
			//Create dictionary of key=PatNum, value=List of anonymous type containing DateReminder, EmailSendStatus, SmsSendStatus,WebSchedSendError,
			//DateTimeEntry,WebSchedRecallNum
			var dictRemindersSent=rawCommlogTable.Rows.OfType<DataRow>()
				.GroupBy(x => PIn.Long(x["PatNum"].ToString()),x => x)
				.ToDictionary(x => x.Key,x => x.Select(y => 
					new {
						DateReminder=PIn.Date(y["DateReminder"].ToString()),
						EmailSendStatus=PIn.Int(y["EmailSendStatus"].ToString()),
						SmsSendStatus=PIn.Int(y["SmsSendStatus"].ToString()),
						WebSchedSendError=PIn.String(y["WebSchedSendError"].ToString()),
						DateTimeEntry=PIn.Date(y["DateTimeEntry"].ToString()),
						WebSchedRecallNum=PIn.Long(y["WebSchedRecallNum"].ToString()),
						DateTimeSendFailed=PIn.DateT(y["DateTimeSendFailed"].ToString()),
					}));
			...
			DataRow row;
			long recallMaxNumberReminders=PrefC.GetLong(PrefName.RecallMaxNumberReminders,dictPrefs);
			...
			foreach(KeyValuePair<long,List<DataRow>> kvp in dictRecallRows) {
				...
				if(dictRemindersSent.ContainsKey(patNum)) {
					var listSentReminders=dictRemindersSent[patNum].Where(x => x.DateReminder.Year > 1880)
						.Where(x => x.EmailSendStatus.In((int)AutoCommStatus.Undefined,(int)AutoCommStatus.SendSuccessful)
							|| x.SmsSendStatus.In((int)AutoCommStatus.Undefined,(int)AutoCommStatus.SendSuccessful));
					foreach(var sentReminder in listSentReminders) {
						if(sentReminder.WebSchedRecallNum > 0 //This reminder is not a CommLog.
							&& listSentReminders.Where(x => x.WebSchedRecallNum==0)	//And there is a CommLog for that same day.
									.Any(x => x.DateReminder.Date==sentReminder.DateReminder.Date)) 
						{
							continue;//To avoid double counting a Web Sched reminder.
						}
						listDatesRemindersSent.Add(sentReminder.DateReminder.Date);
					}
				}
				familyBalance=PIn.Double(rowPat["BalTotal"].ToString());//from the guarantor's patient table
				if(!PrefC.GetBool(PrefName.BalancesDontSubtractIns,dictPrefs)) {//typical
				...
				foreach(DataRow rowCur in kvp.Value) {
					...
					if(numberOfReminders==1) {
						if(PrefC.GetInt(PrefName.RecallShowIfDaysFirstReminder,dictPrefs)==-1) {
							continue;
						}
						if(dateRemind.AddDays(PrefC.GetInt(PrefName.RecallShowIfDaysFirstReminder,dictPrefs)).Date>DateTime.Today) {
							continue;
						}
					}
					else if(numberOfReminders>1) {
						if(PrefC.GetInt(PrefName.RecallShowIfDaysSecondReminder,dictPrefs)==-1) {
							continue;
						}
						if(dateRemind.AddDays(PrefC.GetInt(PrefName.RecallShowIfDaysSecondReminder,dictPrefs)).Date>DateTime.Today) {
							continue;
						}
					}
					...
					switch(contmeth) {
						case ContactMethod.None:
							if(PrefC.GetBool(PrefName.RecallUseEmailIfHasEmailAddress,dictPrefs)) {//if user wants to use email if there is an email address
					...
					if(dictRemindersSent.ContainsKey(patNum)) {
						var webSchedSendInfo=dictRemindersSent[patNum]
							.Where(x => x.WebSchedRecallNum != 0)//0 means the entry came from a commlog, so it will not have fields from the webschedrecall table.
							.OrderByDescending(x => x.DateTimeEntry).FirstOrDefault();
						if(webSchedSendInfo != null) {
							row["webSchedDateTimeFailed"]=webSchedSendInfo.DateTimeSendFailed;
							row["webSchedEmailSendStatus"]=((int)webSchedSendInfo.EmailSendStatus).ToString();
							row["webSchedSmsSendStatus"]=((int)webSchedSendInfo.SmsSendStatus).ToString();
							row["WebSchedRecallNum"]=webSchedSendInfo.WebSchedRecallNum;
							if(webSchedSendInfo.EmailSendStatus==(int)AutoCommStatus.SendNotAttempted
								&& webSchedSendInfo.SmsSendStatus==(int)AutoCommStatus.SendNotAttempted) {
								row["webSchedSendDesc"]=Lans.g("FormRecallList","Sending");
							}
							else if(webSchedSendInfo.EmailSendStatus==(int)AutoCommStatus.SendFailed
								|| webSchedSendInfo.SmsSendStatus==(int)AutoCommStatus.SendFailed) {
								row["webSchedSendDesc"]=Lans.g("FormRecallList","Send Failed");
							}
							row["webSchedSendError"]=webSchedSendInfo.WebSchedSendError;
						}
					}
		...
		///<summary></summary>
		public static void Update(Recall recall,Recall recallOld) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),recall,recallOld);
				return;
			}
			Crud.RecallCrud.Update(recall,recallOld);
		}
		...
		public static List<string> PrepWebSchedNotifications(List<long> listRecallNums,bool isGroupFamily,RecallListSort sortBy,
			WebSchedRecallSource source,EmailAddress emailAddressOverride=null,Dictionary<long,long> dictWebSchedRecallNums=null,
			HashSet<long> hashSetDontSendEmail=null,HashSet<long> hashSetDontSendSms=null) 
		{
			//No need to check RemotingRole; no call to db.
			...
			List<long> listSpecialRecallTypeNums=new List<long>(){
				PrefC.GetLong(PrefName.RecallTypeSpecialChildProphy,dictPrefs),
				PrefC.GetLong(PrefName.RecallTypeSpecialPerio,dictPrefs),
				PrefC.GetLong(PrefName.RecallTypeSpecialProphy,dictPrefs),
			};
			List<RecallType> listRecallTypes=RecallTypeC.GetListt();
			...
			for(int i=0;i<addrTable.Rows.Count;i++) {
				...
				if(dictWebSchedRecallNums != null && dictWebSchedRecallNums.ContainsKey(wsRecall.RecallNum)
					&& dictWebSchedRecallNums[wsRecall.RecallNum] != 0) 
				{//The row already exists in webschedrecall. Set send statuses to NotAttempted so the eConnector will try to send them again.
					string command="UPDATE webschedrecall SET EmailSendStatus="+POut.Int((int)AutoCommStatus.SendNotAttempted)+", "
						+"SmsSendStatus="+POut.Int((int)AutoCommStatus.SendNotAttempted)+", "
						+"ClinicNum="+POut.Long(wsRecall.ClinicNum)+", "//In case this patient's clinic changed since the webschedrecall was inserted.
						+"DateTimeReminderSent="+POut.DateT(DateTime.MinValue)+", "
						+"Source="+POut.Int((int)source)+" "
						+"WHERE WebSchedRecallNum="+POut.Long(dictWebSchedRecallNums[wsRecall.RecallNum]);
					Db.NonQ(command);
					continue;//In order to not insert a WebSchedRecall.
				}
