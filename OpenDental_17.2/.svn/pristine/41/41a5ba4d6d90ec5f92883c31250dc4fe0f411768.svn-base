...
namespace OpenDentBusiness{
	...
	public class Benefit:TableBase, IComparable {
		...
		public int CompareTo(object obj) {
			...
			if(CodeNum!=ben.CodeNum) {
				...
				if(CovCatNum!=0 && CovCatNum==ben.CovCatNum) {//Both benefits are for same custom frequency group or the exam group.
					return ProcedureCodes.GetStringProcCode(CodeNum,listProcedureCodes)
						.CompareTo(ProcedureCodes.GetStringProcCode(ben.CodeNum,listProcedureCodes));
				}
				int frequencyGroup1=0;
				int frequencyGroup2=0;
				if(ProcedureCodeC.ListBWCodeNums.Contains(CodeNum)) {
					frequencyGroup1=1;
				}
				else if(ProcedureCodeC.ListExamCodeNums.Contains(CodeNum)) {
					frequencyGroup1=2;
				}
				else if(ProcedureCodeC.ListPanoFMXCodeNums.Contains(CodeNum)) {
					frequencyGroup1=3;
				}
				if(ProcedureCodeC.ListBWCodeNums.Contains(ben.CodeNum)) {
					frequencyGroup2=1;
				}
				else if(ProcedureCodeC.ListExamCodeNums.Contains(ben.CodeNum)) {
					frequencyGroup2=2;
				}
				else if(ProcedureCodeC.ListPanoFMXCodeNums.Contains(ben.CodeNum)) {
					frequencyGroup2=3;
				}
		...
		public override int GetHashCode() {
			...
			int prime=104743;//Arbitrary prime number
