    ...
    "t necessarily everyone.";
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			if(listProv.SelectedIndices[0]!=0){
				for(int i=0;i<listProv.SelectedIndices.Count;i++){
					...
					whereProv+="procedurelog.ProvNum = "
						+POut.Long(_listProviders[listProv.SelectedIndices[i]-1].ProvNum)+" ";
				}
				whereProv+=") ";
			}
			...
			if(listProv.SelectedIndices[0]==0){
				...
				report.SubTitle.Add(Lan.g(this,"Prov: ")+_listProviders[listProv.SelectedIndices[0]-1].GetLongDesc());
				report.SubTitle.Add(dateFrom.ToString("d")+" - "+dateTo.ToString("d"));
			}
			else{
