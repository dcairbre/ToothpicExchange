...
namespace SparksToothChart {
	...
	public partial class ToothChartDirectX:Control {
		...
		public void InitializeGraphics(){
			//The following flag prevents Microsoft.DirectX.Direct3D.Font.OnDeviceLost() from being called when device is disposed.
			//We can disable the flag since we handle font create/dispose manually.
		...
		private void DrawOcclusalView(ToothGraphic toothGraphic,Matrix defOrient) {
			...
			if(toothGraphic.Visible && toothGraphic.IsSealant) {//draw sealant
				...
				device.Transform.World=ToothTranslationMatrix(toothGraphic)*toothRot*toothTrans*defOrient;
				if(ToothGraphic.IsMaxillary(toothGraphic.ToothID)){
		...
		private void ToothChartDirectX_MouseMove(object sender,MouseEventArgs e) {
			if(TcData.CursorTool==CursorTool.Pointer) {
				...
				try {
					...
					//Just in case device is somehow lost in the middle of drawing.  Not sure if this is possible.
					//Worst case we fail to show some lines the user just drew, but better than showing broken tooth chart with a red X accross it.
				}
			} 
			else if(TcData.CursorTool==CursorTool.Eraser) {
				if(!MouseIsDown) {
