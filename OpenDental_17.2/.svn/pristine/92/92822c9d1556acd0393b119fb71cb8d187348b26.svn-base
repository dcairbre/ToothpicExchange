	...
	public class InsPlans {
		...
		public static long GenerateAllowedFeeSchedules(long userNum=0) {
			...
			for(int i=0;i<table.Rows.Count;i++){
				...
				command="SELECT PlanNum, CarrierNum FROM insplan "
					+"WHERE AllowedFeeSched = 0 "
					+"AND PlanType='' "
					+"AND IsHidden=0 "
					+"AND CarrierNum IN ("+string.Join(",",listCarrierNums)+")";
				DataTable tablePlanNums = Db.GetTable(command);
				command="UPDATE insplan "
					+"SET AllowedFeeSched="+POut.Long(sched.FeeSchedNum)+" "
					+"WHERE PlanNum IN ("+string.Join(",",tablePlanNums.Select().Select(x => x["PlanNum"]).ToList())+")";
				retVal+=Db.NonQ(command);
				//log updated InsPlan's AllowedFeeSched
				tablePlanNums.Select().ToList().ForEach(x => {
					InsEditLogs.MakeLogEntry("AllowedFeeSched",userNum,"0",POut.Long(sched.FeeSchedNum),InsEditLogType.InsPlan,PIn.Long(x["PlanNum"].ToString()),0);
				});
			}
			return retVal;
		}
		...
		public static void UnusedHideAll(long userNum=0) {
			...
			string command="SELECT PlanNum, CarrierNum FROM insplan "
				+"WHERE IsHidden=0 "
				+"AND NOT EXISTS (SELECT * FROM inssub WHERE inssub.PlanNum=insplan.PlanNum)";
			DataTable tablePlanNums = Db.GetTable(command);
			...
			tablePlanNums.Select().ToList().ForEach(x => {
				InsEditLogs.MakeLogEntry("IsHidden",userNum,"0","1",InsEditLogType.InsPlan,PIn.Long(x["PlanNum"].ToString()),0);
			});
		}
		...
		///Also deletes patplans, benefits, claimprocs, and possibly inssubs. No need to pass in userNum, 
		///it's set before remoting role check and passed to the server if necessary.</summary>
		public static void Delete(InsPlan plan, long userNum=0) {
			if(RemotingClient.RemotingRole!=RemotingRole.ServerWeb) {
				userNum=Security.CurUser.UserNum;//must be before normal remoting role check to get user at workstation
			}
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),plan,userNum);
				return;
			}
			#region Validation
			//first, check claims
		...
		///<summary>Deletes the insurance plan passed in and does not check any dependencies before doing so.
		///Used when a new plan is created and then is no longer needed.  Also used if all dependencies have already been fixed.  
		///Deletes any InsVerify dependencies after deleting the InsPlan.
		///No need to pass in userNum, it's set before remoting role check and passed to the server if necessary.</summary>
		public static void Delete(long planNum, long userNum=0) {
			if(RemotingClient.RemotingRole!=RemotingRole.ServerWeb) {
				userNum=Security.CurUser.UserNum;
			}
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
			...
			InsPlan planCur=Crud.InsPlanCrud.SelectOne(command);
			Crud.InsPlanCrud.Delete(planNum);
			if(planCur!=null) {
				InsEditLogs.MakeLogEntry(null,planCur,InsEditLogType.InsPlan,userNum);
			}
		...
		public static long SetAllPlansToShowUCR(long userNum=0) {
			...
			string command="SELECT PlanNum FROM insplan WHERE PlanType = '' AND ClaimsUseUCR = 0";
			List<long> listPlanNums = Db.GetListLong(command);
			command="UPDATE insplan SET ClaimsUseUCR=1 WHERE PlanType=''";
			Db.NonQ(command);
			listPlanNums.ForEach(x => { //log insplan ClaimsUseUCR change.
				InsEditLogs.MakeLogEntry("ClaimsUseUCR",userNum,"0","1",InsEditLogType.InsPlan,x,0);
			});
			return listPlanNums.Count;
		}
		...
		public static void UpdateCobRuleForAll(EnumCobRule cobRule, long userNum=0) {
			...
			string command = "SELECT PlanNum, CobRule FROM insplan WHERE CobRule != "+POut.Int((int)cobRule);
			DataTable tablePlans = Db.GetTable(command);
			command="UPDATE insplan SET CobRule="+POut.Int((int)cobRule);
			Db.NonQ(command);
			tablePlans.Select().ToList().ForEach(x => {
				InsEditLogs.MakeLogEntry("CobRule",userNum,x["CobRule"].ToString(),POut.Int((int)cobRule),InsEditLogType.InsPlan,
					PIn.Long(x["PlanNum"].ToString()),0);
			});
		}
