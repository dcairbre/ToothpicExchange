	...
	public class Userods {
		...
		public static List<Userod> GetUsers(List<long> listUserNums) {
			...
			for(int i=0;i<listUsersLong.Count;i++) {
				if(listUserNums.Contains(listUsersLong[i].UserNum)) {
					retVal.Add(listUsersLong[i].Copy());
				}
			}
			return retVal;
		}
		...
		///<summary>Returns null if not found.  Is not case sensitive.</summary>
		public static Userod GetUserByName(string userName,bool isEcwTight,List<Userod> listUserods=null) {
			//No need to check RemotingRole; no call to db.
			if(listUserods==null) {
				listUserods=UserodC.GetListt();
			}
			for(int i=0;i<listUserods.Count;i++) {
		...
		///<summary>Returns null if not found.  isEcwTight is not case sensitive.
		...
		public static Userod GetUserByNameNoCache(string userName,bool isEcwTight) {
			...
			return GetUserByName(userName,isEcwTight,listUserods);
		}
		...
		public static List<Userod> GetUsersByPermission(Permissions permission,bool showHidden) {
			...
			for(int i=0;i<listAllUsers.Count;i++) {
				if(GroupPermissions.HasPermission(listAllUsers[i].UserGroupNum,permission,0)) {
		...
		public static List<Userod> GetUsersByDomainUserName(string domainUser) {
			return UserodC.GetListShort().FindAll(x => x.DomainUser.Equals(domainUser,StringComparison.InvariantCultureIgnoreCase));
		}
		...
		///Once a user has been found, the failed login attempts is checked.
		///If the number of failed log in attempts exceeds the limit an exception is thrown with a message to display to the user.
		///Then the hash of the password (if usingEcw is true then the password needs to be hashed before passing into this method) is checked against
		///the password hash that is currently in the database.  Returns a user object if user and password are valid.
		///Throws an exception with a specific message to display to the user if anything goes wrong 
		///and then manipulates the appropriate log in failure columns in the db.</summary>
		public static Userod CheckUserAndPassword(string username,string password,bool isEcw) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<Userod>(MethodBase.GetCurrentMethod(),username,password,isEcw);
			}
			...
			if(userDb==null) {
				throw new ApplicationException(Lans.g("Userods","Invalid username or password."));
			}
			...
			if(CheckTypedPassword(password,userNew.Password,isEcw)) {
				//A successful login has been passed in, reset the failure columns.
		...
		///<summary>When RemotingRole.ClientWeb, this will hash the password on the server to utilize the server's MD5 hash algorithm instead of the
		///workstation's algorithm.  This is due to the group policy security option "System cryptography: Use FIPS compliant algorithms for encryption,
		...
		public static string HashPassword(string inputPass,bool useEcwAlgorithm) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetString(MethodBase.GetCurrentMethod(),inputPass,useEcwAlgorithm);
			}
			if(inputPass=="") {
				return "";
			}
		...
		public static void UpdatePassword(Userod userod,string newPassHashed,bool isPasswordStrong) {
			...
			Validate(false,userToUpdate,true);
