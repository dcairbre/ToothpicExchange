...
namespace UnitTests {
	public class AllTests {
		...
		public static string TestSeventyTwo(int specificTest) {
			...
			return "72: Passed.  Web Sched - Operatory priority worked as expected.\r\n";
		}
		...
		///<summary></summary>
		public static string TestSeventyFour(int specificTest) {
		...
		///<summary></summary>
		public static string TestSeventyFive(int specificTest) {
		...
		///<summary></summary>
		public static string TestSeventySix(int specificTest) {
		...
		///<summary>Web Sched - Magically tacking on any additional recall types if the current recall being scheduled is a "special type".</summary>
		public static string TestSeventySeven(int specificTest) {
			if(specificTest!=0 && specificTest!=77) {
				return "";
			}
			string suffix="77";
			return suffix+": Deprecated\r\n";
			//Operatory opDoc=null;
			//Operatory opHyg=null;
			//Recall recallProphy=null;
			//Recall recall4BW=null;
			//DateTime dateTimeSchedule=DateTime.MinValue;
			//WebSchedScenarioHelper(suffix,out opDoc,out opHyg,out recallProphy,out recall4BW,out dateTimeSchedule);
			////The open time slot returned should be for the first Web Sched operatory and not the second one due to item order.
			//List<TimeSlot> listTimeSlots=TimeSlots.GetAvailableWebSchedTimeSlots(recallProphy.RecallNum,dateTimeSchedule,dateTimeSchedule.AddDays(30));
			////There should be 1 time slot returned that spans from 09:00 - 10:10.
			//if(listTimeSlots.Count!=1) {
			//	throw new Exception("Incorrect number of time slots returned.  Expected 1, received "+listTimeSlots.Count+".\r\n");
			//}
			//if(listTimeSlots.Any(x => x.OperatoryNum!=opDoc.OperatoryNum)) {
			//	throw new Exception("Invalid operatory time slot returned.  Expected all time slots to be in operatory #"+opHyg.OperatoryNum+"\r\n");
			//}
			////First slot needs to be 70 mins long due to automagically tacking on the 4BW for this UNRELATED recall being scheduled (ugh).
			//if(listTimeSlots[0].DateTimeStart.Date!=dateTimeSchedule.Date
			//	|| listTimeSlots[0].DateTimeStart!=new DateTime(dateTimeSchedule.Year,dateTimeSchedule.Month,dateTimeSchedule.Day,9,0,0)
			//	|| listTimeSlots[0].DateTimeStop!=new DateTime(dateTimeSchedule.Year,dateTimeSchedule.Month,dateTimeSchedule.Day,10,10,0)) 
			//{
			//	throw new Exception("Invalid first time slot returned.  See test for expected values.\r\n");
			//}
			//return "77: Passed.  Web Sched - Additional recall types magically added for unrelated recall being scheduled.\r\n";
		}

		///<summary>This is a helper method that preps the database for Web Sched scenarios 77 and 78 (they require the same setup)</summary>
		private static void WebSchedScenarioHelper(string suffix,out Operatory opDoc,out Operatory opHyg,out Recall recallProphy
			,out Recall recall4BW,out DateTime dateTimeSchedule) 
		{
			//Start with fresh tables so that our time slots are extremely predictable.
			RecallTypeT.ClearRecallTypeTable();
			RecallT.ClearRecallTable();
			...
			ScheduleOpT.ClearScheduleOpTable();
			//Create a date that will always be in the future.  This date will be used for schedules and recalls.
			dateTimeSchedule=DateTime.Now.AddYears(1);
			long provNumDoc=ProviderT.CreateProvider("Doc-"+suffix);
			long provNumHyg=ProviderT.CreateProvider("Hyg-"+suffix,isSecondary:true);
			Patient pat=PatientT.CreatePatient(suffix,provNumDoc);
			//Make sure the that Appointment View time increment is set to 10 min.
			...
			Def defLunchBlockout=DefinitionT.CreateDefinition(DefCat.BlockoutTypes,"Lunch-"+suffix,itemColor:System.Drawing.Color.Azure);
			//Create a psudo prophy recall type that lasts 40 mins and has an interval of every 6 months and 1 day.
			RecallType recallTypeProphy=RecallTypeT.CreateRecallType("Prophy-"+suffix,"D3541,D1356","//X/",new Interval(1,0,6,0));
			//Set the prophy recall type as a "special" type.
			Prefs.UpdateLong(PrefName.RecallTypeSpecialProphy,recallTypeProphy.RecallTypeNum);
			//Create a recall for our patient.
			recallProphy=RecallT.CreateRecall(pat.PatNum,recallTypeProphy.RecallTypeNum,dateTimeSchedule,new Interval(1,0,6,0));
			//Now create a psudo 4BW recall type that lasts 30 mins that also has an interval of every 6 months and 1 day (so both share due date).
			RecallType recallType4BW=RecallTypeT.CreateRecallType("4BW-"+suffix,"D1698","/X/",new Interval(1,0,6,0));
			//Create a recall for our patient.
			recall4BW=RecallT.CreateRecall(pat.PatNum,recallType4BW.RecallTypeNum,dateTimeSchedule,new Interval(1,0,6,0));
			//Create operatories for the providers.
			opDoc=OperatoryT.CreateOperatory("1-"+suffix,"Doc Op - "+suffix,provNumDoc,provNumHyg,isWebSched:true,itemOrder:0);
			opHyg=OperatoryT.CreateOperatory("2-"+suffix,"Hyg Op - "+suffix,provNumDoc,provNumHyg,isWebSched:true,itemOrder:1,isHygiene:true);
			//Create a schedule for the doctor from 09:00 - 11:00
			Schedule schedDoc=ScheduleT.CreateSchedule(dateTimeSchedule,new TimeSpan(new DateTime(1,1,1,9,0,0).Ticks)
				,new TimeSpan(new DateTime(1,1,1,11,0,0).Ticks),schedType:ScheduleType.Provider,provNum:provNumDoc);
			//Now link up the schedule entries to the Web Sched operatory
			ScheduleOpT.CreateScheduleOp(opDoc.OperatoryNum,schedDoc.ScheduleNum);
			//Create the exact same schedule for the other provider.
			Schedule schedHyg=ScheduleT.CreateSchedule(dateTimeSchedule,new TimeSpan(new DateTime(1,1,1,9,0,0).Ticks)
				,new TimeSpan(new DateTime(1,1,1,11,0,0).Ticks),schedType:ScheduleType.Provider,provNum:provNumHyg);
			//Link the crazy schedule up to the non-Web Sched operatory.
			ScheduleOpT.CreateScheduleOp(opHyg.OperatoryNum,schedHyg.ScheduleNum);
		}

		///<summary>Unit test 77 MUST PASS before looking into why this one fails.
		///Web Sched - Magically tacking on any additional recall types if the current recall being scheduled is a "special type".
		///This specific test makes sure that all procedures from all "magical" recalls are added correctly to the appointment.</summary>
		public static string TestSeventyEight(int specificTest) {
			if(specificTest!=0 && specificTest!=78) {
				return "";
			}
			string suffix="78";
			return suffix+": Deprecated\r\n";
			...
			//List<TimeSlot> listTimeSlots=TimeSlots.GetAvailableWebSchedTimeSlots(recallProphy.RecallNum,dateTimeSchedule,dateTimeSchedule.AddDays(30));
			////There should be 1 time slot returned that spans from 09:00 - 10:10.  
			////Create an appointment for this recall and procedures from unrelated recalls should be attached to it.
			//List<Procedure> listProcedures=Recalls.CreateRecallApptForWebSched(recallProphy.RecallNum
			//	,listTimeSlots[0].DateTimeStart
			//	,listTimeSlots[0].DateTimeStop
			//	,listTimeSlots);
			////Unit test 77 MUST PASS before looking into why this one fails.--------------------------------------------------------------------------------
			//if(listProcedures.Count!=3) {
			//	throw new Exception("Incorrect number of procedures scheduled.  Expected 3, received "+listProcedures.Count+".\r\n");
			//}
			////Create the following procedure codes; D3541, D1356, D1698
			//if(!listProcedures.Exists(x => x.CodeNum==ProcedureCodes.GetCodeNum("D3541"))
			//	|| !listProcedures.Exists(x => x.CodeNum==ProcedureCodes.GetCodeNum("D1356"))
			//	|| !listProcedures.Exists(x => x.CodeNum==ProcedureCodes.GetCodeNum("D1698"))) 
			//{
			//	throw new Exception("Not all expected procedures were scheduled.\r\n");
			//}
			//return "78: Passed.  Web Sched - Additional recall type procedures magically added for unrelated recall being scheduled.\r\n";
		}

		///<summary>Testing appointment time pattern logic.</summary>
		public static string TestSeventyNine(int specificTest) {
		...
		public static string TestEighty(int specificTest) {
		...
		public static string TestEightyOne(int specificTest) {
		...
		///<summary>Makes sure that if two claimprocs are created for the same frequency limitation group and one is marked received, the 
		///estimate claimproc will show as 0 coverage because the limitation is met.</summary>
		public static string TestEightyTwo(int specificTest) {
		...
		/// <summary></summary>
		public static string TestEightyThree(int specificTest) {
		...
		public static string TestEightyFour(int specificTest) {
		...
		public static string TestEightyFive(int specificTest) {
		...
		private static void OpenFormInvisibly(Form form) {
