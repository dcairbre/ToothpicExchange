...
///<summary></summary>
	public class FormRpPaySheet : ODForm{
		...
		private void InitializeComponent()
		{
			...
			this.checkAllProv.Location = new System.Drawing.Point(494, 34);
		...
		private void FormPaymentSheet_Load(object sender, System.EventArgs e) {
			...
			_listProviders.Insert(0,ProviderC.GetUnearnedProv());
			...
			for(int i=0;i<DefC.Short[(int)DefCat.ClaimPaymentGroups].Length;i++) {
				listClaimPayGroups.Items.Add(DefC.Short[(int)DefCat.ClaimPaymentGroups][i].ItemName);
			}
		...
		private void butOK_Click(object sender,System.EventArgs e) {
			...
			List<Def> listClaimPayGroupDefs=new List<Def>(DefC.GetList(DefCat.ClaimPaymentGroups));
			if(checkAllProv.Checked) {
			...
			if(checkInsuranceTypes.Checked) {
				listInsTypes=listInsDefs.Select(x => x.DefNum).ToList();
			}
			else {
				listInsTypes=listInsuranceTypes.SelectedIndices.OfType<int>().Select(x => listInsDefs[x].DefNum).ToList();
			}
			if(checkPatientTypes.Checked) {
				listPatTypes=listPatDefs.Select(x => x.DefNum).ToList();
			}
			else {
				listPatTypes=listPatientTypes.SelectedIndices.OfType<int>().Select(x => listPatDefs[x].DefNum).ToList();
			}
			if(checkAllClaimPayGroups.Checked) {
				listSelectedClaimPayGroupNums=listClaimPayGroupDefs.Select(x => x.DefNum).ToList();
			}
			else {
				listSelectedClaimPayGroupNums=listClaimPayGroups.SelectedIndices.OfType<int>().Select(x => listClaimPayGroupDefs[x].DefNum).ToList();
			}
			...
			if(listProvNums.Count!=0) {
				tableIns=RpPaySheet.GetInsTable(date1.SelectionStart,date2.SelectionStart,listProvNums,listClinicNums,listInsTypes,
					listSelectedClaimPayGroupNums,checkAllProv.Checked,checkAllClin.Checked,checkInsuranceTypes.Checked,radioPatient.Checked,checkAllClaimPayGroups.Checked);
			}
			DataTable tablePat=RpPaySheet.GetPatTable(date1.SelectionStart,date2.SelectionStart,listProvNums,listClinicNums,listPatTypes,
				checkAllProv.Checked,checkAllClin.Checked,checkPatientTypes.Checked,radioPatient.Checked);
