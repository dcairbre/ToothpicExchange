...
namespace OpenDental {
	public partial class FormInsVerificationList:ODForm {
		...
		private void FormInsVerificationList_Load(object sender,EventArgs e) {
			...
			List<Def> listVerifyStatuses=DefC.Short[(int)DefCat.InsuranceVerificationStatus].ToList();
			...
			if(PrefC.HasClinicsEnabled) {
				...
				if(listRegionDefs.Count!=0) {
					...
					for(int i = 0;i<listRegionDefs.Count;i++) {
						listBoxVerifyRegions.Items.Add(listRegionDefs[i].ItemName);
						if(_listDefNumsVerifyRegionsFilter.Contains(listRegionDefs[i].DefNum)) {
		...
		private void listBoxVerifyRegions_MouseUp(object sender,MouseEventArgs e) {
			...
			foreach(int i in listBoxVerifyRegions.SelectedIndices) {
				if(i<1) {
					...
					_listDefNumsVerifyRegionsFilter.Add(DefC.Short[(int)DefCat.Regions][i-1].DefNum);
				}
			}
