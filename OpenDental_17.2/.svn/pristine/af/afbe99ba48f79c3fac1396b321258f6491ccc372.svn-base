...
///<summary></summary>
	public class FormClaimEdit : ODForm {
		...
		///<summary>Contains the claimprocs for the claim which are not associated to labs.</summary>
		...
		private Hashtable _hashProcCodes=ProcedureCodeC.GetHList();
		private List<ProcedureCode> _listProcCodes=ProcedureCodeC.Listt;
    ...
    "ed in the Chart module. ";
			...
			// textInsPayEst
			// 
			this.textInsPayEst.Location = new System.Drawing.Point(472, 363);
    ...
    "nt\" to finish this batch for daily reporting.";
			...
			// textWriteOff
			// 
			this.textWriteOff.Location = new System.Drawing.Point(596, 363);
			...
			// textInsPayAmt
			// 
			this.textInsPayAmt.Location = new System.Drawing.Point(534, 363);
			...
			// textDedApplied
			// 
			this.textDedApplied.Location = new System.Drawing.Point(410, 363);
			...
			this.textClaimIdOriginal.TabIndex = 168;
			// 
			// label62
			// 
			...
			this.label62.Text = "Claim Identifier Original";
		...
		private void FormClaimEdit_Load(object sender, System.EventArgs e) {
			...
			if(!IsNew && !Security.IsAuthorized(Permissions.ClaimDelete,ClaimCur.SecDateEntry,true)) {
      ...
      ClaimProcList=ClaimProcs.Refresh(PatCur.PatNum);
			...
			if(ClaimCur.ProvOrderOverride!=0) {
				...
				SetOrderingReferral(Referrals.GetReferral(ClaimCur.OrderingReferralNum));
			}
			...
			if(IsNew) {
				...
				if(listSiteProcs.Count > 0) {
					Site site=Sites.ListDeep.FirstOrDefault(x => x.SiteNum==listSiteProcs[0].SiteNum);
					if(site!=null) {
		...
		public void FillForm(){
			...
			textClaimIdOriginal.Text=ClaimCur.PatNum.ToString()+"/"+ClaimCur.ClaimNum.ToString();
		...
		private void FillGrids(){
			...
			gridProc.Rows.Clear();
			ODGridRow row;
			_listClaimProcsForClaim=ClaimProcs.RefreshForClaim(ClaimCur.ClaimNum,ProcList);//Excludes labs.
			...
			for(int i=0;i<_listClaimProcsForClaim.Count;i++){
				...
				if(CultureInfo.CurrentCulture.Name.EndsWith("CA") && _dictCanadianLabClaimProcs.ContainsKey(_listClaimProcsForClaim[i].ProcNum)) {//This proc has one or more labs attached.
					foreach(ClaimProc labClaimProc in _dictCanadianLabClaimProcs[_listClaimProcsForClaim[i].ProcNum]) {//Add one row for each attached proc, just below the parent proc.
						...
						ClaimProcRowHelper(row,labClaimProc);
						gridProc.Rows.Add(row);
					}
				}
			}
		...
		private void ClaimProcRowHelper(ODGridRow row,ClaimProc claimProcCur) {
			...
			if(claimProcCur.ProcNum==0) {
				...
				ProcedureCode procCodeCur=ProcedureCodes.GetProcCode(ProcCur.CodeNum,_listProcCodes);
				ProcedureCode procCodeSent=ProcedureCodes.GetProcCode(claimProcCur.CodeSent,_hashProcCodes);
			...
			if(claimProcCur.ClaimPaymentTracking!=0) {
				row.Cells.Add(DefC.GetDef(DefCat.ClaimPaymentTracking,claimProcCur.ClaimPaymentTracking).ItemName);//EOB Code
			}
			else {
				row.Cells.Add("");
			}
		...
		private void FillStatusHistory() {
			////status history
			...
			List<ClaimTracking> listCustomStatusEntries=ClaimTrackings.RefreshForClaim(ClaimTrackingType.StatusHistory,ClaimCur.ClaimNum)
				.OrderByDescending(x => x.DateTimeEntry).ToList();
			ODGridRow row;
			foreach(ClaimTracking claimTrackingEntry in listCustomStatusEntries) {
				row=new ODGridRow();
				row.Cells.Add(claimTrackingEntry.DateTimeEntry.ToShortDateString()+" "+claimTrackingEntry.DateTimeEntry.ToShortTimeString());
				String defValue=DefC.GetName(DefCat.ClaimCustomTracking,claimTrackingEntry.TrackingDefNum);//get definition Name
				row.Cells.Add(defValue);
				row.Cells.Add(claimTrackingEntry.Note);
				row.Cells.Add(DefC.GetName(DefCat.ClaimErrorCode,claimTrackingEntry.TrackingErrorDefNum));
		...
		private void gridProc_CellClick(object sender,ODGridClickEventArgs e) {
			if(ClaimCur.ClaimStatus=="S" || ClaimCur.ClaimStatus=="R") {//sent or received
				if(!Security.IsAuthorized(Permissions.ClaimSentEdit,ClaimCur.DateSent,true)) {
					return;
				}
			}
		...
		private void MakeSuppPayment() {
			...
			List<ClaimProc> cpList=new List<ClaimProc>();
			for(int i=0;i<gridProc.SelectedIndices.Length;i++){
				ClaimProc ClaimProcCur=_listClaimProcsForClaim[gridProc.SelectedIndices[i]];
				...
				ClaimProcCur.DateInsFinalized=DateTime.MinValue;
...
//fix: need to debug the recalculation feature to take this status into account.
		...
		private void butBatch_Click(object sender,EventArgs e) {
			...
			claimPayment.CheckDate=DateTimeOD.Today;//Today's date for easier tracking by the office and to avoid backdating before accounting lock dates.
			...
			double amt=ClaimProcs.AttachAllOutstandingToPayment(claimPayment.ClaimPaymentNum,ClaimCur.ClaimNum);
		...
		public static void FormFinalizePaymentHelper(ClaimPayment claimPayment,Claim claimCur,Patient patCur,Family famCur) {
			...
			ClaimProcs.DateInsFinalizedHelper(claimPayment.ClaimPaymentNum,claimCur.ClaimNum);
			//ClaimList=Claims.Refresh(PatCur.PatNum);
			//ClaimProcList=ClaimProcs.Refresh(PatCur.PatNum);
			//FillGrids();
		...
		private void butExport_Click(object sender,EventArgs e) {
			...
			if(!Directory.Exists(exportPath)){
				MessageBox.Show(Lan.g(this,"The claim export path no longer exists at:")+" "+exportPath);
				return;
			}
			for(int i=0;i<ClaimCur.Attachments.Count;i++){
      ...
      ClaimProcList=ClaimProcs.Refresh(PatCur.PatNum);
		...
		private void SendClaim() {
			...
			//string warnings;
			//string missingData=
			Eclaims.GetMissingData(clearinghouseClin,listQueue[0]);
		...
		private void butAdd_Click(object sender,EventArgs e) {
			FormClaimCustomTrackingUpdate FormCCTU = new FormClaimCustomTrackingUpdate(ClaimCur);
			FormCCTU.ShowDialog();
		...
		private void butDelete_Click(object sender, System.EventArgs e) {
			...
			//Loops through each of the procedures and checks that there is no payment, insurance amount attached
			...
			if(claimInsPayAttached){
				MsgBox.Show(this,"You cannot delete this claim while there are insurance payment amounts.  Set all Insurance Paid amounts to zero first.");
				return;
			}
      ...
      if(ClaimCur.ClaimType=="PreAuth"){
			...
			if(ClaimCur.ClaimType=="PreAuth"//all preauth claimprocs are just duplicates
				...
				for(int i=0;i<_listClaimProcsForClaim.Count;i++){
					...
					if(ClaimCur.ClaimType=="P" && PatPlanList.Count>0){
						ClaimProcs.ComputeBaseEst(_listClaimProcsForClaim[i],proc,plan,PatPlanList[0].PatPlanNum,benList,null,null,PatPlanList,0,0,PatCur.Age,0,PlanList,SubList,ListSubLinks);
					}
					else if(ClaimCur.ClaimType=="S" && PatPlanList.Count>1){
						ClaimProcs.ComputeBaseEst(_listClaimProcsForClaim[i],proc,plan,PatPlanList[1].PatPlanNum,benList,null,null,PatPlanList,paidOtherInsTotal,paidOtherInsTotal,PatCur.Age,writeOffOtherIns,PlanList,SubList,ListSubLinks);
					}
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			if(comboClaimStatus.SelectedIndex==2){//waiting to send
				...
				Clearinghouse clearinghouseHq=ClearinghouseL.GetClearinghouseHq(listQueue[0].ClearinghouseNum);
				Clearinghouse clearinghouseClin=Clearinghouses.OverrideFields(clearinghouseHq,Clinics.ClinicNum);
				Eclaims.GetMissingData(clearinghouseClin,listQueue[0]);//,out warnings);
				if(listQueue[0].MissingData!="") {
