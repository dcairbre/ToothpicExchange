			...
			get {
				...
				switch(PhaseCur) {
					...
					case JobPhase.Development:
						...
						if(ListJobReviews.Any(x => x.ReviewStatus!=JobReviewStatus.Done && x.ReviewStatus!=JobReviewStatus.NeedsAdditionalWork && x.ReviewStatus!=JobReviewStatus.NeedsAdditionalReview)){
							JobReview review=ListJobReviews.FirstOrDefault(x => x.ReviewStatus!=JobReviewStatus.Done && x.ReviewStatus!=JobReviewStatus.NeedsAdditionalWork && x.ReviewStatus!=JobReviewStatus.NeedsAdditionalReview);
							if(review!=null) {
				...
				switch(this.PhaseCur) {
					...
					case JobPhase.Development:
						...
						if(this.ListJobReviews.Any(x => x.ReviewStatus!=JobReviewStatus.Done 
							...
							&& x.ReviewStatus!=JobReviewStatus.NeedsAdditionalReview)) //.Any on empty list returns false.
						{
		...
		public JobAction ActionForUser(Userod user) {
			...
			switch(baseAction) {
				...
				case JobAction.ReviewCode:
					if(this.ListJobReviews.Any(x=> !x.ReviewStatus.In(JobReviewStatus.Done,JobReviewStatus.NeedsAdditionalReview,JobReviewStatus.NeedsAdditionalWork) && (x.ReviewerNum==user.UserNum || (x.ReviewerNum==0 && JobPermissions.IsAuthorized(JobPerm.Writeup,true,user.UserNum))))) {
						return baseAction;
					}
					if(this.UserNumEngineer==user.UserNum) {
