...
namespace OpenDental.InternalTools.Job_Manager {
	...
	public partial class UserControlJobEdit:UserControl {
		...
		public UserControlJobEdit() {
			...
			gridReview.ContextMenu.MenuItems.Add("Needs Additional Review",(o,arg) => { menuItemReviewNeedsAdditionalWork_Click(o,arg);
			});
			_listJobPrioritiesSorted.ForEach(x=>comboPriority.Items.Add(x.ToString()));
		...
		private void menuItemReviewComplete_Click(object sender,EventArgs e) {
			...
			if(Security.CurUser.UserNum==review.ReviewerNum) {
				...
				MsgBox.Show(this,"You are not authorized to mark this review as complete.");
			}
		}

		private void menuItemReviewNeedsAdditionalWork_Click(object sender,EventArgs e) {
			if(gridReview.SelectedIndices.Length<1) {
				return;
			}
			long FKey = ((JobReview)gridReview.Rows[gridReview.SelectedIndices[0]].Tag).JobReviewNum;
			...
			if(Security.CurUser.UserNum==review.ReviewerNum) {
				...
				MsgBox.Show(this,"You are not authorized to mark this review as needs additional review.");
			}
		}
		...
		public Job GetJob() {
			...
			job.Priority=_listJobPrioritiesSorted[comboPriority.SelectedIndex];
		...
		private void CheckPermissions() {
			...
			switch(_jobCur.PhaseCur) {
				case JobPhase.Concept:
					...
					break;
				case JobPhase.Quote:
					if((!JobPermissions.IsAuthorized(JobPerm.Writeup,true) && _jobCur.UserNumQuoter!=Security.CurUser.UserNum) 
						|| (!JobPermissions.IsAuthorized(JobPerm.NotifyCustomer,true)&& _jobCur.UserNumCustQuote!=Security.CurUser.UserNum) 
						|| (_jobCur.IsApprovalNeeded && !JobPermissions.IsAuthorized(JobPerm.Approval,true))) 
					{
						break;
					}
		...
		private bool ValidateJob(Job _jobCur) {
			...
			if(_jobCur.Category==JobCategory.Bug && _jobCur.ListJobLinks.Count(x => x.LinkType==JobLinkType.Bug)==0) {
		...
		private void actionMenu_ImplementedClick(object sender,EventArgs e) {
			...
			bool isHeadOnlyCommit=FormVP.IsHeadOnly;
			...
			if(!MsgBox.Show(this,MsgBoxButtons.YesNo,"Does this job need to be documented?")) {
				textEditorDocumentation.Append(Security.CurUser.UserName+" - "+DateTime.Now.ToString()+"\r\n"+"Documentation Not Needed");
			}
			...
			string logMsg = "";
			if(isHeadOnlyCommit) {
				logMsg=description+"\r\nCommitted to: "+textVersion.Text+"\r\nReviewed by: "+reviewers;
			}
			else {
				logMsg=description+"\r\nBackported to: "+textVersion.Text+"\r\nReviewed by: "+reviewers;
			}
		...
		private void SaveJob(Job job) {
			...
			if(job.ListJobLinks.Exists(x => x.LinkType==JobLinkType.Request) && job.PhaseCur==JobPhase.Development) {
				if(job.Priority==JobPriority.OnHold) {
		...
		private void comboPriority_SelectionChangeCommitted(object sender,EventArgs e) {
			...
			_jobCur.Priority=_listJobPrioritiesSorted[comboPriority.SelectedIndex];
			_jobOld.Priority=_listJobPrioritiesSorted[comboPriority.SelectedIndex];
			if(!IsNew) {
				Job job = Jobs.GetOne(_jobCur.JobNum);
				job.Priority=_listJobPrioritiesSorted[comboPriority.SelectedIndex];
				Jobs.Update(job);
