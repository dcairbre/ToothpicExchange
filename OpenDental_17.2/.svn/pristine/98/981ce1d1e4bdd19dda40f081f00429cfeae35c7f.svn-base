...
namespace OpenDentBusiness {
	...
	public class Signalods {
		...
		///<summary>Called in the constructor of a WPF window in order to subscribe for the signals.</summary>
		public static bool SubscribeWPF(Window win) {
			//No need to check RemotingRole; no call to db.
			ISignalProcessor sigProcessor = win as ISignalProcessor;
			if(sigProcessor==null) {
				return false;
			}
			_listISignalProcessors.Add(sigProcessor);
		...
		public static List<Signalod> SignalsTick(List<Signalod> listLocalSignals=null) {
			...
			try {
				...
				Cache.Refresh(getCacheFromDb,cacheRefreshArray);//refresh invalid data
