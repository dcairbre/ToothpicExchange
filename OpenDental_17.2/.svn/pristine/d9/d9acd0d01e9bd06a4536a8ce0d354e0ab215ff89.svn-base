	...
	public class CarrierCrud {
		...
		public static List<Carrier> TableToList(DataTable table){
			...
			foreach(DataRow row in table.Rows) {
				...
				carrier.CarrierGroupName        = PIn.String(row["CarrierGroupName"].ToString());
				retVal.Add(carrier);
			}
			return retVal;
		}
		...
		public static long Insert(Carrier carrier,bool useExistingPK){
			...
			command+="CarrierName,Address,Address2,City,State,Zip,Phone,ElectID,NoSendElect,IsCDA,CDAnetVersion,CanadianNetworkNum,IsHidden,CanadianEncryptionMethod,CanadianSupportedTypes,SecUserNumEntry,SecDateEntry,TIN,CarrierGroupName) VALUES(";
			if(useExistingPK || PrefC.RandomKeys) {
			...
			command+=
				...
				+"'"+POut.String(carrier.CarrierGroupName)+"')";
			if(useExistingPK || PrefC.RandomKeys) {
				Db.NonQ(command);
			}
			else {
		...
		public static void Update(Carrier carrier){
			string command="UPDATE carrier SET "
				...
				+"CarrierGroupName        = '"+POut.String(carrier.CarrierGroupName)+"' "
		...
		public static bool Update(Carrier carrier,Carrier oldCarrier){
			...
			if(carrier.CarrierGroupName != oldCarrier.CarrierGroupName) {
				if(command!=""){ command+=",";}
				command+="CarrierGroupName = '"+POut.String(carrier.CarrierGroupName)+"'";
			}
			if(command==""){
				return false;
			}
