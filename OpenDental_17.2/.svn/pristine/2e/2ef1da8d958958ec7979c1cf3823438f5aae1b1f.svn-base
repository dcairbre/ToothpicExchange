...
///<summary></summary>
	public class FormBilling : ODForm {
			...
			,ref int emailed,ref int printed,ref int sentElect,ref PdfDocument outputDocument,ref PrintDocument pd,ref int skippedDeleted) 
		{
			...
			//A dictionary of batches of statements.  The key is the batch num which is 0 based (helpful when displaying to the user).
			...
			for(int i = 0;i<gridBill.SelectedIndices.Length;i++) {
				listStatement.Add(Statements.GetStatement(PIn.Long(((DataRow)gridBill.Rows[gridBill.SelectedIndices[i]].Tag)["StatementNum"].ToString())));
				if((i+1) % maxStmtsPerBatch==0 || i+1==gridBill.SelectedIndices.Length) {
					dictStatementsForSend.Add(batchCount,listStatement);
					batchCount++;
