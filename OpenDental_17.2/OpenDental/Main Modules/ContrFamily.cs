...
namespace OpenDental{
	...
	public class ContrFamily : System.Windows.Forms.UserControl{
		...
		private void RefreshModuleData(long patNum) {
			...
			PatNoteCur=PatientNotes.Refresh(patNum,PatCur.Guarantor);
			...
			_dictCloneSpecialty=Patients.GetClonesAndSpecialties(PatCur.PatNum);
			...
			if(gridPat.Rows[e.Row].Tag!=null){
				if(gridPat.Rows[e.Row].Tag.ToString()=="Referral"){
					...
					if(field==null) {
						...
						if(PatFieldDefs.ListShort[index].FieldType==PatFieldType.InCaseOfEmergency) {
							//Deprecated
							//FormInCaseOfEmergency FormICE=new FormInCaseOfEmergency(field);
							//FormICE.IsNew=true;
							//FormICE.ShowDialog();
						}
					}
		...
		private void FillPatientData(){
			...
			for(int f=0;f<fields.Count;f++) {
				...
				switch(fieldCur.InternalName){
					...
					#region Billing Type
					case "Billing Type":
						string billingtype=DefC.GetName(DefCat.BillingTypes,PatCur.BillingType);
						if(DefC.GetHidden(DefCat.BillingTypes,PatCur.BillingType)) {
					...
					case "PatFields":
						PatField field;
						List<FieldDefLink> listFieldDefLinks=FieldDefLinks.GetForLocation(FieldLocations.Family)
						...
						for(int i=0;i<PatFieldDefs.ListShort.Count;i++) {
							...
							field=PatFieldList.FirstOrDefault(x => x.FieldName==PatFieldDefs.ListShort[i].FieldName);
							if(PatFieldDefs.ListShort[i].FieldType.ToString()=="InCaseOfEmergency") {
					...
					#region Referrals
					case "Referrals":
						List<RefAttach> listRefs=RefAttaches.Refresh(PatCur.PatNum);
		...
		private void FillGridPatientClones() {
			...
			if(PrefC.HasClinicsEnabled) {
				listClinics=Clinics.GetListLong();
			...
			foreach(KeyValuePair<Patient,Def> cloneAndSpecialty in _dictCloneSpecialty) {
				...
				if(PrefC.HasClinicsEnabled) {
					row.Cells.Add(Clinics.GetAbbr(cloneAndSpecialty.Key.ClinicNum,listClinics));
		...
		private void FillInsData(){
			if(PatCur!=null && PatCur.DiscountPlanNum!=0) {
				...
				DiscountPlan discountPlan=DiscountPlans.GetPlan(PatCur.DiscountPlanNum);
				Def adjType=DefC.GetDef(DefCat.AdjTypes,discountPlan.DefNum);
				...
				discountRow.ColorBackG=DefC.Long[(int)DefCat.MiscColors][0].ItemColor;
