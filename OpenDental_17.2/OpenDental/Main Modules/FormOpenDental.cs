...
namespace OpenDental{
    ...
    private FormSmsTextMessaging _formSmsTextMessaging;
		...
		private void InitializeComponent(){
			...
			// 
			this.menuItemConfirmations.Index = 6;
			...
			// 
			this.menuItemInsVerification.Index = 7;
			...
			// menuItemOperatories
			// 
			this.menuItemOperatories.Index = 8;
			...
			// 
			this.menuItemRequiredFields.Index = 23;
			...
			// 
			this.menuItemRequirementsNeeded.Index = 24;
			...
			// 
			this.menuItemMergeMedications.Index = 4;
			...
			// 
			this.menuItemMergePatients.Index = 5;
			...
			// 
			this.menuItemMergeProviders.Index = 6;
			...
			// 
			this.menuItemMergeReferrals.Index = 7;
			...
			// 
			this.menuItemMoveSubscribers.Index = 8;
			...
			// 
			this.menuItemProcLockTool.Index = 9;
			...
			// 
			this.menuItemServiceManager.Index = 10;
			...
			// menuItemShutdown
			// 
			this.menuItemShutdown.Index = 11;
			...
			// 
			this.menuItemXChargeReconcile.Index = 14;
		...
		private void EnableAdditionalFeatures() {
			...
			refreshNeeded|=SetAdvertisingForGroup(doc,ProgramName.DentalIntel,ProgramName.PracticeByNumbers);
			if(refreshNeeded) {
		...
		///<summary>Set advertising for a group of programs. If at least one program is enabled, set Disabled Advertising for all disabled programs to true. Otherwise set advertising normally.</summary>
		private bool SetAdvertisingForGroup(XmlDocument doc, params ProgramName[] arrProgNames) {
			bool hasChanged=false;
			ProgramProperty property;
			List<Program> listPrograms=new List<Program>();
			foreach(ProgramName progName in arrProgNames) {
				listPrograms.Add(Programs.GetCur(progName));
			}
			List<Program> listEnabledPrograms=listPrograms.FindAll(x => x.Enabled);
			if(listEnabledPrograms.Count>0) { //if any programs are enabled
				foreach(Program prog in listEnabledPrograms) {
					hasChanged|=SetAdvertising((ProgramName)Enum.Parse(typeof(ProgramName),prog.ProgName),doc); //set advertising according to HQ
				}
				foreach(Program prog in listPrograms.Where(x => !x.Enabled)) { //disable advertising for all other programs, regardless of what HQ says
					property=ProgramProperties.GetForProgram(prog.ProgramNum).FirstOrDefault(x => x.PropertyDesc=="Disable Advertising");
					if(property!=null) { //property exists, set it to true
						ProgramProperty propOld=property.Copy();
						property.PropertyValue="1";
						hasChanged|=ProgramProperties.Update(property,propOld);
					}
					else { //property doesn't exist, create it and set it to true
						hasChanged=true;
						property=new ProgramProperty();
						property.ProgramNum=prog.ProgramNum;
						property.PropertyDesc="Disable Advertising";
						property.PropertyValue="1";
						ProgramProperties.Insert(property);
					}
				}
			}
			else { //if no programs are enabled, set advertising as normal
				foreach(Program prog in listPrograms) {
					hasChanged|=SetAdvertising((ProgramName)Enum.Parse(typeof(ProgramName),prog.ProgName),doc);
				}
			}
			return hasChanged;
		}
		...
		private void OnEmail_Click() {
			//this button item will be disabled if no pat selected.
      ...
      message.FromAddress=string.IsNullOrEmpty(selectedAddress.SenderAddress)?selectedAddress.EmailUsername:selectedAddress.SenderAddress;
		...
		private void menuClick_OpenPickList(object sender,EventArgs e) {
			...
			Clinic clinicCur=Clinics.ListLong.FirstOrDefault(x => x.ClinicNum==FormC.SelectedClinicNum);
		...
		public static void S_RefreshCurrentModule(bool hasForceRefresh=false,bool isApptRefreshDataPat=true) {
			_formOpenDentalS.RefreshCurrentModule(hasForceRefresh,isApptRefreshDataPat);
		}
		...
		private void CheckAlerts() {
			...
			foreach(MenuItem menuItem in listMenuItem) {
				...
				if(_listAlertItems.Any(x => x.Equals((AlertItem)menuItem.Tag))) {//Still care about existing menuItem alert. So we will update later.
					continue;
				}
			...
			foreach(AlertItem alertItemCur in _listAlertItems) {
				...
				//This is safe since there is 1 row per AlertType per Severity. At least for now.
				MenuItem menuItem=listMenuItem.Where(x => x!=menuItemAlerts && x!=menuItemNoAlerts)
					.FirstOrDefault(x => alertItemCur.Type==((AlertItem)x.Tag).Type
					&& alertItemCur.Severity==((AlertItem)x.Tag).Severity);
				if(menuItem!=null) {//Menu already has AlertType with this Severity, so update text if needed.
					menuItem.Text=AlertMenuItemHelper(alertItemCur)+alertDescriptNew;
					continue;
				}
		...
		private void TaskPopupHelper(Task taskPopup) {
			...
			List<TaskNote> notesForThisTask=TaskNotes.GetForTask(taskPopup.TaskNum);
			...
			if(taskPopup.TaskListNum!=Security.CurUser.TaskListInBox//if not my inbox
				&& Security.CurUser.DefaultHidePopups)//and popups blocked
			{
				return;//no sound or popup
									//in other words, popups will always show for my inbox even if popups blocked.
			}
			if(taskPopup.TaskListNum==Security.CurUser.TaskListInBox//if my inbox
		...
		public void ProcessSignals(List<Signalod> listSignals) {
			...
			for(int i=0;i<tasksPopup.Count;i++) {
				TaskPopupHelper(tasksPopup[i]);//Reminders sent to a subscribed tasklist will pop up prior to the reminder date/time.
			}
			...
			bool doRefreshServerCache=true;
				...
				&& !listSignals.Any(x => x.RemoteRole==RemotingRole.ClientDirect)) 
			{
				...
				doRefreshServerCache=false;
			}
			RefreshLocalData(doRefreshServerCache,Signalods.GetInvalidTypes(listSignals));
			#endregion Refresh
		...
		private void ThreadClaimReportRetrieve(ODThread worker) {
			...
			Clearinghouses.RetrieveReportsAutomatic();
		}
		...
		public static void S_MenuItemSecurity_Click(object sender,EventArgs e) {
			_formOpenDentalS.menuItemSecurity_Click(sender,e);
		}
		...
		private void ProcessHqMetricsPhones() {
			...
			if(!this.IsDisposed) {
				Invoke(new ProcessHqMetricsResultsArgs(OnProcessHqMetricsResults),new object[] { listPED,_listPhonesCur,listSubGroups,listChatUsers,phone,isTriageOperator });
			}
		}
				...
				if(Programs.UsingEcwTightOrFullMode() && odUser!="") {//only leave it null if a user was passed in on the commandline.  If starting OD manually, it will jump into the else.
					...
					if(Security.CurUser==null) {
						...
						if(adminUser.Password=="") {
							...
							//All LDAP servers must expose a special entry, called the root DSE, whose Domain Name is the zero-length string.
							string domainName;
							try {
								DirectoryEntry root=new DirectoryEntry("LDAP://RootDSE");//This will fail with an exception if the local computer is not a domain member.
								domainName=root.Properties["defaultNamingContext"][0].ToString();	
								if(!loginPath.ToLower().Contains(domainName.ToLower())) {
