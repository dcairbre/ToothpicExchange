...
//		private System.Windows.Forms.PrintDialog printDialog2;
    ...
    private ArrayList ALPreAuth;
		...
		private void InitializeComponent(){
			...
			this.label1.Location = new System.Drawing.Point(755, 178);
			...
			// listSetPr
			// 
			this.listSetPr.Location = new System.Drawing.Point(757, 195);
			...
			this.gridMain.Size = new System.Drawing.Size(745, 469);
			...
			this.dateTimeTP.Location = new System.Drawing.Point(431, 103);
			...
			this.tabShowSort.Location = new System.Drawing.Point(512, 28);
			...
			this.butRefresh.Location = new System.Drawing.Point(431, 124);
			...
			this.labelCheckInsFrequency.Location = new System.Drawing.Point(428, 87);
		...
		private void RefreshModuleData(long patNum) {
			...
			if(patNum!=0) {
				TPModuleData tpData=TreatmentPlanModules.GetModuleData(patNum);
				FamCur=tpData.Fam;
				PatCur=tpData.Pat;
				TreatPlanType tpTypeCur=(PatCur.DiscountPlanNum==0?TreatPlanType.Insurance:TreatPlanType.Discount);
				TreatPlans.AuditPlans(PatCur.PatNum,tpTypeCur);
				SubList=tpData.SubList;
				InsPlanList=tpData.InsPlanList;
				_listSubLinks=SubstitutionLinks.GetAllForPlans(InsPlanList);
	...
	private void FillMainData() {
		...
		Dictionary<long,List<Fee>> dictFees=null;
		Procedure proc=null;
		Fee discountPlanFee=null;
		if(PatCur.DiscountPlanNum!=0) {
			discountPlan=DiscountPlans.GetPlan(PatCur.DiscountPlanNum);
			dictFees=Fees.GetByFeeSchedNum(discountPlan.FeeSchedNum);
		}
		TpRow row;
		...
		for(int i=0;i<ProcTPSelectList.Length;i++) {
			...
			if(PatCur.DiscountPlanNum!=0) {
				...
				discountPlanFee=Fees.GetFeeFromDict(dictFees,proc.CodeNum,discountPlan.FeeSchedNum,proc.ClinicNum,proc.ProvNum);
      ...
      for(int i=0;i<ALPreAuth.Count;i++){
		...
		private List<ProcTP> LoadActiveTP(ref TreatPlan treatPlan) {
			...
			for(int i=0;i<listProcForTP.Count;i++) {
				...
				if(PatCur.DiscountPlanNum!=0) {
					Fee procFee=Fees.GetFeeFromDict(dictFees,procCodeCur.CodeNum,discountPlan.FeeSchedNum,listProcForTP[i].ClinicNum,listProcForTP[i].ProvNum);
		...
		private void ToolBarMainUpdate_Click() {
			...
			for(int i=0;i<ProcListTP.Length;i++) {
				...
				if(priplan!=null && priplan.PlanType=="p") {//PPO
					ucrFee=Fees.GetAmount0(procCur.CodeNum,Providers.GetProv(Patients.GetProvNum(PatCur)).FeeSched,procCur.ClinicNum,procCur.ProvNum);
      ...
      ClaimCur.ClaimType="PreAuth";
			...
			foreach(Procedure procCur in listProcsSelected){
				...
				if(FormIPS.SelectedPlan.UseAltCode && (procCodeCur.AlternateCode1!="")){
					...
					ClaimProcCur.CodeSent=ProcedureCodes.GetStringProcCode(procCur.CodeNum,listProcCodes);
