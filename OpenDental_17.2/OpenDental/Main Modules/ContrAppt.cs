...
namespace OpenDental {
	...
	public class ContrAppt:System.Windows.Forms.UserControl {
		...
		private Dictionary<DateTime,List<Operatory>> _dictOpsForDay=new Dictionary<DateTime, List<Operatory>>();
		...
		private void InitializeComponent() {
			...
			this._menuOp.Size = new System.Drawing.Size(153, 26);
			// 
			// ContrAppt
			// 
		...
		private void OpPanelProv_RightClick(object sender,MouseEventArgs e) {
			...
			if(prov==null) {
				...
				Def defaultProvDef=DefC.GetDef(DefCat.ProviderSpecialties,prov.Specialty);
			...
			foreach(Schedule scheduleCur in listScheds) {
				...
				Def scheduledProvDef=DefC.GetDef(DefCat.ProviderSpecialties,scheduledProv.Specialty);
				ToolStripMenuItem scheduledStrip=new ToolStripMenuItem(("Scheduled "+scheduleCur.StartTime.ToShortTimeString()+"-"+scheduleCur.StopTime.ToShortTimeString()+": "+scheduledProv.GetFormalName()+", "+scheduledProvDef.ItemName));
		...
		private void FillWaitingRoom(bool refreshOpsForDay=false) {
			if(DS==null) {
				return;
			}
			...
			if(PrefC.GetBool(PrefName.WaitingRoomFilterByView)) {
				...
				//To save database calls we will only refresh the Ops for the current day if the user is forcing a refresh (clicks module button) or we don't
				//have the current day already on file (today not present in our dictionary).
				if(refreshOpsForDay || !_dictOpsForDay.TryGetValue(DateTime.Today,out listOpsForApptView)) {
					List<Schedule> listSchedulesForToday=Schedules.ConvertTableToList(Schedules.GetPeriodSchedule(DateTime.Today,DateTime.Today));
					ApptView viewCur=null;
					if(comboView.SelectedIndex>0) {
						viewCur=_listApptViews[comboView.SelectedIndex-1];
					}
					listOpsForApptView=ApptViewItemL.GetOpsForApptView(viewCur,ApptDrawing.IsWeeklyView,listSchedulesForToday);
					_dictOpsForDay[DateTime.Today]=listOpsForApptView;
				}
			}
		...
		void menuItemProv_Click(object sender,EventArgs e) {
			...
			if(procsForSingleApt.Exists(x => x.ProvNum!=listProcsNew.FirstOrDefault(y => y.ProcNum==x.ProcNum).ProvNum)) {//Either the primary or hygienist changed.
				string promptText="";
				isUpdatingFees=Procedures.FeeUpdatePromptHelper(listProcsNew,procsForSingleApt,ref promptText);
			...
			try {
				...
				if(pinBoard.SelectedAppt.AptStatus==ApptStatus.Planned//if Planned appt is on pinboard
					&& (!Security.IsAuthorized(Permissions.AppointmentCreate)//and no permission to create a new appt
						|| PatRestrictionL.IsRestricted(pinBoard.SelectedAppt.PatNum,PatRestrict.ApptSchedule))) 
				{
					return;
				}
				...
				if(AppointmentRuleC.List.Length>0) { //If no rules exist then don't bother checking.					
		...
		private void ContrApptSheet2_MouseUp(object sender,System.Windows.Forms.MouseEventArgs e) {
			...
			try {
				...
				if(ResizingAppt) {
					...
					Appointments.SetPattern(TempApptSingle.AptNum,pattern);
					SecurityLogs.MakeLogEntry(Permissions.AppointmentEdit,PatCur.PatNum,Lan.g(this,"Appointment resized from the appointment module."),
						TempApptSingle.AptNum);//Generate FKey to the appointment to show the audit entry in the ApptEdit window.
					RefreshModuleDataPatient(PatCur.PatNum);
					FormOpenDental.S_Contr_PatientSelected(PatCur,true,false);
					RefreshPeriod(true);
					Signalods.SetInvalidAppt(Appointments.GetOneApt(TempApptSingle.AptNum));
				...
				#endregion
				Operatory curOp=ApptDrawing.VisOps[ApptDrawing.ConvertToOp(TempApptSingle.Location.X-ContrApptSheet2.Location.X)];
				MoveAppointments(new List<Appointment>() { apt },new List<Appointment>() { aptOld },curOp,timeWasMoved,isOpChanged);//Apt's time has already been changed at this point.
				#region Update UI and cache
				RefreshModuleDataPatient(PatCur.PatNum);
				FormOpenDental.S_Contr_PatientSelected(PatCur,true,false);
				RefreshPeriod();				
				...
				for(int i=0;i<_aptBubbleDefs.Count;i++) {
					...
					switch(_aptBubbleDefs[i].InternalName) {
						...
						case "Confirmed":
							...
							continue;
						case "Appt Status":
							if(row.AptStatus==ApptStatus.ASAP) {
			...
			if(row.AptStatus==ApptStatus.UnschedList) {//unscheduled status
				if(row.AptDateTime.Year<1880) {//Indicates that this was a brand new appt
					...
					if(aptCur.AptDateTime.Year>1880){//if date is now present
						...
						Appointments.Delete(row.AptNum);
					}
				}
		...
		private void menuApt_Click(object sender,System.EventArgs e) {
			...
			switch(((MenuItem)sender).Name) {
				...
				case "Send Confirmation Text":
					...
					if(wasTextSent) {
						Appointments.SetConfirmed(appt.AptNum,PrefC.GetLong(PrefName.ConfirmStatusTextMessaged));
					}
					break;
			}
		}
		...
		private void OnBreak_Click() {
			...
			if(hasBrokenProcs) {//If true, user can not get here from right click 'Break Appointment' directly.
				FormApptBreak formAB=new FormApptBreak();
		...
		private void OnComplete_Click() {
			...
			if(apt.AptStatus == ApptStatus.PtNote) {
				...
				if(apt.AptStatus!=ApptStatus.Complete) { // seperate log entry for editing completed appointments.
					SecurityLogs.MakeLogEntry(Permissions.AppointmentEdit,apt.PatNum,
						ContrApptSheet2[GetIndex(apt.AptNum)].Procs+", "+ apt.AptDateTime.ToString()+", Set Complete",
		...
		private void MoveAppointments(List<Appointment> listAppts,List<Appointment> listApptsOld,Operatory curOp,bool timeWasMoved,bool isOpChanged,bool isOpUpdate=false) {
			for(int i=0;i<listAppts.Count;i++) {
				Appointment apt=listAppts[i];
				Appointment aptOld=listApptsOld[i];
				apt.Op=curOp.OperatoryNum;
				Patient patCur=Patients.GetPat(apt.PatNum);
				bool provChanged=false;
				bool hygChanged=false;
				long assignedDent=0;
				long assignedHyg=0;
				if(isOpUpdate) {
					assignedDent=Schedules.GetAssignedProvNumForSpot(Schedules.GetSchedsForOp(curOp),curOp,false,apt.AptDateTime);
					assignedHyg=Schedules.GetAssignedProvNumForSpot(Schedules.GetSchedsForOp(curOp),curOp,true,apt.AptDateTime);
				}
				else { 
				...
				try {
					if(isOpUpdate) {
						Appointments.MoveValidatedAppointment(apt,aptOld,patCur,curOp,Schedules.GetSchedsForOp(curOp),ApptDrawing.VisOps,provChanged,hygChanged,timeWasMoved,isOpChanged,isOpUpdate);
					}
					else { 
		...
		private void ShowSearch() {
			...
			for(int i=0;i<ProviderC.ListShort.Count;i++) {
				if(pinBoard.SelectedAppt.IsHygiene
					&& ProviderC.ListShort[i].ProvNum==pinBoard.SelectedAppt.ProvHyg) {
					...
					&& ProviderC.ListShort[i].ProvNum==pinBoard.SelectedAppt.ProvNum) {
		...
		private class OpPanel {
			...
			public void ResetOpPanel(ContrAppt apptModule,Operatory op,Provider prov,int panelHeight,Point location) {
				...
				_backPanel.MouseDown-=new MouseEventHandler(apptModule.OpPanelProv_RightClick); //ensures that the same event doesn't get added twice.
				_backPanel.MouseDown+=new MouseEventHandler(apptModule.OpPanelProv_RightClick);
				...
				_labelOpName.MouseDown-=new MouseEventHandler(apptModule.OpPanelProv_RightClick); //ensures that the same event doesn't get added twice.
				_labelOpName.MouseDown+=new MouseEventHandler(apptModule.OpPanelProv_RightClick);
