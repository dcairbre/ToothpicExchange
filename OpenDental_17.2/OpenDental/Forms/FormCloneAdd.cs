...
namespace OpenDental {
	public partial class FormCloneAdd:ODForm {
		...
		private void FillClinicComboBoxes() {
			...
			List<Def> listSpecialtyDefs=DefC.GetList(DefCat.ClinicSpecialty).ToList();
			//If there are specialties present, we need to know which clinics have no specialty set so that the user can always make clones for that clinic.
			if(listSpecialtyDefs.Count > 0) {
				listClinicsNoSpecialty.RemoveAll(x => x.Specialty > 0);
			}
			//Remove all clinics that do not have a specialty from the original list of clinics for the user.
			listClinicsForUser.RemoveAll(x => x.Specialty < 1);
			//Filter out any specialties that are not associated to any available clinics for this user.
			listSpecialtyDefs.RemoveAll(x => !listClinicsForUser.Any(y => y.Specialty==x.DefNum));
			//Get all clinics that are associated to any current clones of this patient.
			List<Clinic> listClinicsForClones=Clinics.GetClinicsForClones(_patientMaster.PatNum);
			//Filter out any specialties that are currently in use by clones of this patient.
			listSpecialtyDefs.RemoveAll(x => listClinicsForClones.Any(y => y.Specialty==x.DefNum));
			...
			foreach(Def specialty in listSpecialtyDefs) {
				...
				_dictSpecialtyClinics[specialty.DefNum]=listClinicsForUser.FindAll(x => x.Specialty==specialty.DefNum);
			}
		...
		private void butPickPrimary_Click(object sender,EventArgs e) {
			...
			comboPriProv.SelectedIndex=Providers.GetIndexLong(FormPP.SelectedProvNum,_listProviders);
