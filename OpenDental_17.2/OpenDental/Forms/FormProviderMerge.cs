...
namespace OpenDental {
	public partial class FormProviderMerge:ODForm {
		...
		public FormProviderMerge() {
			InitializeComponent();
			Lan.F(this);
			_listActiveProvs=ProviderC.GetListShort().FindAll(x => x.ProvStatus != ProviderStatus.Deleted);
		}
		...
		private void butChangeProvFrom_Click(object sender,EventArgs e) {
			FormProviderPick FormPP=new FormProviderPick(checkDeletedProvs.Checked ? ProviderC.GetListLong() : _listActiveProvs);
			FormPP.ShowDialog();
			if(FormPP.DialogResult==DialogResult.OK) {
				Provider selectedProv=Providers.GetProv(FormPP.SelectedProvNum);
