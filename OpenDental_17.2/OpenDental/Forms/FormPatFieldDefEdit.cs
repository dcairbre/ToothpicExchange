	...
	public class FormPatFieldDefEdit : ODForm {
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			if(OldFieldName!=textName.Text && !IsNew) {
				if(!MsgBox.Show(this,MsgBoxButtons.YesNo,"Changing the field name will cause existing information for this field to be hidden.  Continue?")) {
					textName.Text=OldFieldName;
					return;
				}
				List<PatFieldDef> listPatFieldDefs=PatFieldDefs.GetListLong();
				for(int i=0;i<listPatFieldDefs.Count;i++) {
					if(listPatFieldDefs[i].FieldName==textName.Text) {
