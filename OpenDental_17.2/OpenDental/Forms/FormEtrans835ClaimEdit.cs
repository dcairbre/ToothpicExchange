...
namespace OpenDental {
	public partial class FormEtrans835ClaimEdit:ODForm {
		...
		private void FillProcedureBreakdown() {
			...
			gridProcedureBreakdown.Columns.Add(new ODGridColumn("PatResp",colWidthAdjAmt,HorizontalAlignment.Right));
			...
			for(int i=0;i<_claimPaid.ListProcs.Count;i++) {
				...
				row.Cells.Add(new ODGridCell(patRespProc.ToString("f2")));//PatResp
		...
		private void butPrint_Click(object sender,EventArgs e) {
			FormEtrans835Print form=new FormEtrans835Print(null,_claimPaid);
			form.Show(this);//This window is just used to display information.
		}
