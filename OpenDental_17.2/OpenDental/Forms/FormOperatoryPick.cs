...
namespace OpenDental {
	public partial class FormOperatoryPick:ODForm {
		...
		private void FillGrid(){
			...
			gridMain.Rows.Clear();
			ODGridRow row;
			List<Clinic> listClinics=Clinics.GetListLong();
			...
			foreach(Operatory opCur in _listOps) {
				...
				if(PrefC.HasClinicsEnabled) {
					row.Cells.Add(Clinics.GetAbbr(opCur.ClinicNum,listClinics));
				}
				row.Cells.Add(Providers.GetAbbr(opCur.ProvDentist,listProvs));
				row.Cells.Add(Providers.GetAbbr(opCur.ProvHygienist,listProvs));
				if(opCur.IsHygiene){
					row.Cells.Add("X");
				}
				else{
