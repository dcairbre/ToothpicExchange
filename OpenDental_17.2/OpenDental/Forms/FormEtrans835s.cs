...
namespace OpenDental {
	public partial class FormEtrans835s:ODForm {
		...
		private void FormEtrans835s_Load(object sender,EventArgs e) {
			...
			#endregion
			_listClinics=Clinics.GetListShort();
			...
			string carrierName,string checkTraceNum,string amountMin,string amountMax)
		{
			...
			if(isRefreshNeeded) {
				actionCloseProgress=ODProgress.ShowProgressStatus("Etrans835",Lan.g(this,"Gathering data")+"...",false);
				...
				foreach(Etrans etrans in _listEtranss) {
					...
					X835 x835=new X835(dictEtransMessages[etrans.EtransMessageTextNum],etrans.TranSetId835,listAttachedTo835,true);
				...
				if(PrefC.HasClinicsEnabled) {	
					...
					if(listClinicNums.Count==1) {
						if(listClinicNums[0]==0) {
							...
							clinicAbbr=Clinics.GetAbbr(listClinicNums[0],_listClinics);
						}
					}
