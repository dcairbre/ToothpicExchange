...
namespace OpenDental {
	public partial class FormOrthoSetup:ODForm {
		...
		private void FormOrthoSetup_Load(object sender,EventArgs e) {
			...
			checkPatClone.Checked=PrefC.GetBool(PrefName.ShowFeaturePatientClone,dictPrefs);
			checkApptModuleShowOrthoChartItem.Checked=PrefC.GetBool(PrefName.ApptModuleShowOrthoChartItem,dictPrefs);
			checkOrthoEnabled.Checked=PrefC.GetBool(PrefName.OrthoEnabled,dictPrefs);
			checkOrthoFinancialInfoInChart.Checked=PrefC.GetBool(PrefName.OrthoCaseInfoInOrthoChart,dictPrefs);
			checkOrthoClaimMarkAsOrtho.Checked=PrefC.GetBool(PrefName.OrthoClaimMarkAsOrtho,dictPrefs);
			checkOrthoClaimUseDatePlacement.Checked=PrefC.GetBool(PrefName.OrthoClaimUseDatePlacement,dictPrefs);
			textOrthoMonthsTreat.Text=PrefC.GetByte(PrefName.OrthoDefaultMonthsTreat,dictPrefs).ToString();
			_orthoAutoProcCodeNum=PrefC.GetLong(PrefName.OrthoAutoProcCodeNum,dictPrefs);
			textOrthoAutoProc.Text=ProcedureCodes.GetStringProcCode(_orthoAutoProcCodeNum);
			checkConsolidateInsPayment.Checked=PrefC.GetBool(PrefName.OrthoInsPayConsolidated,dictPrefs);
