...
namespace OpenDental {
	public partial class FormCCDPrint:ODForm {
		...
		private void Init(){
			...
			if(etrans.PatNum!=0) { //Some transactions are not patient specific.
				...
				if(claim==null) {//for eligibility or when the claim was deleted by the customer before receiving the response from the mailbox.
					...
					else if(PrefC.HasClinicsEnabled && Clinics.ListLong.Count>0) {
						clinic=Clinics.ListLong[0];
					}
				}
				if(provTreat==null) {
		...
		private void butOverride_Click(object sender,EventArgs e) {
			...
			Canadian.EOBImportHelper(formData,claimprocs,listAllProcs,listAllClaimProcs,claim,false,FormClaimEdit.ShowProviderTransferWindow);
