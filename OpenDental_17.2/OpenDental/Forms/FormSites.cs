...
namespace OpenDental{
	...
	public class FormSites:ODForm {
		...
		private void FormSites_Load(object sender, System.EventArgs e) {
			...
			if(SelectedSiteNum!=0){
				for(int i=0;i<Sites.ListShallow.Count;i++){
					if(Sites.ListShallow[i].SiteNum==SelectedSiteNum){
		...
		private void FillGrid(){
			...
			gridMain.Rows.Clear();
			ODGridRow row;
			foreach(Site site in Sites.ListDeep) {
				row=new ODGridRow();
		...
		private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			if(IsSelectionMode){
				SelectedSiteNum=Sites.ListShallow[e.Row].SiteNum;
				DialogResult=DialogResult.OK;
				return;
			}
			else{
				FormSiteEdit FormS=new FormSiteEdit();
				FormS.SiteCur=Sites.ListShallow[e.Row];
				FormS.ShowDialog();
				FillGrid();
				changed=true;
			}
		...
		private void butOK_Click(object sender,EventArgs e) {
			...
			//	return;
				...
				SelectedSiteNum=Sites.ListShallow[gridMain.GetSelectedIndex()].SiteNum;
			}
			DialogResult=DialogResult.OK;
		}
