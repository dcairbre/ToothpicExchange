...
namespace OpenDental {
	public partial class FormSheetDefEdit:ODForm {
		...
		private void DrawGridHelper(Graphics g,int i) {
			...
			if(SheetDefCur.SheetFieldDefs[i].FieldName=="StatementInvoicePayment") {
				SizeF sSize=g.MeasureString("Payments",new Font(FontFamily.GenericSansSerif,10,FontStyle.Bold));
				g.FillRectangle(Brushes.White,SheetDefCur.SheetFieldDefs[i].XPos,yPosGrid,odGrid.Width,odGrid.TitleHeight);
				g.DrawString("Payments",new Font(FontFamily.GenericSansSerif,10,FontStyle.Bold),new SolidBrush(Color.Black),SheetDefCur.SheetFieldDefs[i].XPos,yPosGrid);
				yPosGrid+=odGrid.TitleHeight;
			}
		...
		private void DrawImagesHelper(Graphics g,int i) {
			...
			Image img=null;
			if(SheetDefCur.SheetFieldDefs[i].ImageField!=null) {//The image has already been downloaded.
				img=new Bitmap(SheetDefCur.SheetFieldDefs[i].ImageField);
			}
				...
				FormP.TickMS=1000;
				OpenDentalCloud.Core.TaskStateDownload state=CloudStorage.DownloadAsync(SheetUtil.GetImagePath(),SheetDefCur.SheetFieldDefs[i].FieldName,
				...
				if(state==null || state.FileContent==null) {
					...
					using(MemoryStream stream=new MemoryStream(state.FileContent)) {
						...
						SheetDefCur.SheetFieldDefs[i].ImageField=new Bitmap(Image.FromStream(stream));//So it doesn't have to be downloaded again the next time.
					}
				}
			}
			else {
