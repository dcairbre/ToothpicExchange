...
namespace OpenDental {
	public partial class FormReqFieldCondEdit:ODForm {
		...
		private void FormReqFieldCondEdit_Load(object sender,EventArgs e) {
			...
			_listClinics=Clinics.ListLong.ToList();
			...
			switch(selectedField) {
				...
				case RequiredFieldName.BillingType:
					...
					for(int i=0;i<DefC.Short[(int)DefCat.BillingTypes].Length;i++) {
						listConditionValues.Items.Add(DefC.Short[(int)DefCat.BillingTypes][i].ItemName);
					}
					ListValuesSetIndices();
					break;
		...
		private void ListValuesSetIndices() {
			...
			for(int i=0;i<listConditionValues.Items.Count;i++) {
				if(selectedType==RequiredFieldName.BillingType
					&& _listReqFieldConds.Exists(x => x.ConditionValue==DefC.Short[(int)DefCat.BillingTypes][i].DefNum.ToString()))
				{
		...
		private void butPickProv_Click(object sender,EventArgs e) {
			...
			listConditionValues.SelectedIndices.Add(Providers.GetIndexLong(FormP.SelectedProvNum,_listProvs));
		}
			...
			switch(selectedField) {
				...
				case RequiredFieldName.PrimaryProvider:
					...
					if(selectedField==RequiredFieldName.PrimaryProvider) {
						...
						for(int i=0;i<listConditionValues.SelectedIndices.Count;i++) {
							listFkNums.Add(DefC.Short[(int)DefCat.BillingTypes][listConditionValues.SelectedIndices[i]].DefNum);
						}
					}
