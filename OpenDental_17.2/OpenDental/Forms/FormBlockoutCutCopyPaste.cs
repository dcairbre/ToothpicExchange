		...
		private void butPaste_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<SchedList.Count;i++) {
				...
				if(Schedules.Overlaps(sched)) {
					string opNames = " ";
					if(sched.Ops.Count>1) {
						opNames="(s) ";
					}
					foreach(long opNum in sched.Ops) {
						opNames+=Operatories.GetOpName(opNum)+",";
					}
					opNames.TrimEnd(',');
					MessageBox.Show(Lans.g("Schedule","A blockout already exists in op")+opNames+" "
						+Lans.g("Schedule","on")+" "+sched.SchedDate.ToShortDateString()+" "+sched.StartTime.ToShortTimeString());
			...
			foreach(Schedule schedule in SchedList) {
