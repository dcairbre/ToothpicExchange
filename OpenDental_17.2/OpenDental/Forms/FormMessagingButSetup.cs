	...
	public class FormMessagingButSetup : ODForm {
		...
		private void listComputers_Click(object sender,EventArgs e) {
			...
			if(listComputers.SelectedIndex==0) {
				...
				_arraySigButDefs=SigButDefs.GetByComputer(Computers.List[listComputers.SelectedIndex-1].CompName);
			}
			FillList();
		}
		...
		private void listButtons_DoubleClick(object sender,EventArgs e) {
			...
			string computerNameSelected=(listComputers.SelectedIndex > 0) ? Computers.List[listComputers.SelectedIndex-1].CompName : "";
			...
			if(sigButDef==null) {//Add
				...
				if(listComputers.SelectedIndex!=0) {
					computerNameSigButDef=Computers.List[listComputers.SelectedIndex-1].CompName;
				}
				...
				sigButDef.ComputerName=Computers.List[listComputers.SelectedIndex-1].CompName;
				FormSigButDefEdit FormS=new FormSigButDefEdit(sigButDef.Copy());
				FormS.IsNew=true;
				FormS.ShowDialog();
			}
