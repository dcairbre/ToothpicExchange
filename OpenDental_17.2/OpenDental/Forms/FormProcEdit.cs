...
///<summary></summary>
    ...
    "ou should reprint the claim if any significant changes are made.";
			...
			this.textOrderingProviderOverride.QuickPasteType = OpenDentBusiness.QuickPasteType.None;
    ...
    " to edit.";
		...
		private void FormProcInfo_Load(object sender,System.EventArgs e) {
			...
			if(ProcCur.ProvOrderOverride!=0) {
				...
				SetOrderingReferral(Referrals.GetReferral(ProcCur.OrderingReferralNum));
			}
		...
		private void PromptForAutoNotes() {
			while(textNotes.Text.Contains("[[")) {
				int beginIdx=textNotes.Text.IndexOf("[[");
				int endIdx=textNotes.Text.IndexOf("]]");
				if(beginIdx > endIdx) {
					return;
				}
				string autoNoteTitle=textNotes.Text.Substring(beginIdx+2,endIdx-beginIdx-2);
				string note=AutoNotes.GetByTitle(autoNoteTitle);
				StringBuilder autoNote=new StringBuilder(note);
				List<AutoNoteControl> listAutoNotePrompts=new List<AutoNoteControl>();
				...
				for(int i=0;i<matches.Count;i++) {
					matchIndex=autoNote.ToString().IndexOf(matches[i].Value);
					autoNoteDescript=matches[i].Value.Substring(9,matches[i].Value.Length-11);
					autoNoteControl=AutoNoteControls.GetByDescript(autoNoteDescript);
					if(autoNoteControl==null) {
						continue;//couldn't find a prompt with that name, so just ignore it.
					}
					promptResponse="";
					if(autoNoteControl.ControlType=="Text") {
						FormAutoNotePromptText FormT=new FormAutoNotePromptText();
						FormT.PromptText=autoNoteControl.ControlLabel;
						FormT.ResultText=autoNoteControl.ControlOptions;
						FormT.ShowDialog();
						...
						FormOR.PromptText=autoNoteControl.ControlLabel;
						FormOR.PromptOptions=autoNoteControl.ControlOptions;
						FormOR.ShowDialog();
						...
						FormMR.PromptText=autoNoteControl.ControlLabel;
						FormMR.PromptOptions=autoNoteControl.ControlOptions;
						FormMR.ShowDialog();
					...
					autoNote.Replace(matches[i].Value,promptResponse,matchIndex,matches[i].Value.Length);
				}
				textNotes.Text=textNotes.Text.Replace("[["+autoNoteTitle+"]]",autoNote.ToString());
			}
		}
		...
		private void SetControlsEnabled(bool isSilent) {
			//Return if the current procedure is considered complete (C, EC, EO).
			...
			//Use DateEntryC to compare to the date/days newer restriction.
			bool hasDateLock=Security.IsGlobalDateLock(Permissions.ProcComplEdit,ProcCur.DateEntryC,isSilent);//really only used to silence other security messages.
			bool hasProcEditLim = Security.IsAuthorized(Permissions.ProcComplEditLimited,ProcCur.DateEntryC,isSilent);
			bool hasProcEditComp = Security.IsAuthorized(Permissions.ProcComplEdit,ProcCur.DateEntryC,isSilent||hasDateLock||!hasProcEditLim,hasDateLock);
			...
			if(!hasProcEditComp) {//always silent. All permissions messages generated at top of method.
				...
				List<Control> listControlsEnabled=new List<Control>() {
					...
					textNotes,signatureBoxWrapper,buttonUseAutoNote,butChangeUser,butAppend,//proc note controls
			...
			if(!Security.IsAuthorized(Permissions.ProcComplEdit,ProcCur.DateEntryC,true,true)
				&& Security.IsAuthorized(Permissions.ProcComplEdit,ProcCur.DateEntryC,true,true,ProcCur.CodeNum,ProcCur.ProcFee,0,0)) 
			{
		...
		private void gridIns_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(ProcCur.IsLocked || (ProcCur.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC)
					&& !Security.IsAuthorized(Permissions.ProcComplEdit,ProcCur.DateEntryC))) 
			{
      ...
      SetControlsUpperLeft();
		...
		private void butSetComplete_Click(object sender, System.EventArgs e) {
			...
			if(!PrefC.GetBool(PrefName.ProcPromptForAutoNote)) {
				...
				while(textNotes.Text.Contains("[[")) {
					int beginIdx=textNotes.Text.IndexOf("[[");
					int endIdx=textNotes.Text.IndexOf("]]");
					textNotes.Text=textNotes.Text.Remove(beginIdx,endIdx-beginIdx);
				}
			}
		...
		private void butDelete_Click(object sender, System.EventArgs e) {
			...
			if(!ProcOld.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC)
				&& !Security.IsAuthorized(Permissions.ProcDelete,ProcCur.DateEntryC)) //This should be a much more lenient permission since completed procedures are already safeguarded.
			{
				return;
			}
			if(ProcOld.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC) 
				&& !Security.IsAuthorized(Permissions.ProcComplEdit,ProcOld.DateEntryC,ProcOld.CodeNum,ProcOld.ProcFee)) 
			{
				return;
			}
			...
			try{
				...
				if(ProcOld.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC)) 
				{
					SecurityLogs.MakeLogEntry(Permissions.ProcComplEdit,ProcOld.PatNum,
						ProcedureCodes.GetProcCode(ProcOld.CodeNum).ProcCode+" ("+ProcOld.ProcStatus+"), "+ProcOld.ProcFee.ToString("c")+", Deleted");
				}
				else {
					SecurityLogs.MakeLogEntry(Permissions.ProcDelete,ProcOld.PatNum,
						ProcedureCodes.GetProcCode(ProcOld.CodeNum).ProcCode+" ("+ProcOld.ProcStatus+"), "+ProcOld.ProcFee.ToString("c"));
				}
				DialogResult=DialogResult.OK;
		...
		private bool EntriesAreValid(){
			...
			if(ProcOld.ProcStatus!=ProcStat.C && ProcCur.ProcStatus==ProcStat.C){//if status was changed to complete
				...
				if(ProcOld.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC)) {//that was already complete
					...
					if(ProcOld.ProcDate!=PIn.Date(textDate.Text) //If user changed the procedure date
					...
					{						
						//We need to check to make sure they are not trying to backdate before the global lock date a procedure that can't be backdated.
						if(!Security.IsAuthorized(Permissions.ProcComplEdit,PIn.Date(textDate.Text),ProcCur.CodeNum,PIn.Double(textProcFee.Text))) {
							return false;
						}
					}
				}
		...
		private void SaveAndClose(){
			...
			if(ProcCur.ProvNum!=ProcOld.ProvNum) {
				if(PaySplits.IsPaySplitAttached(ProcCur.ProcNum)) {
					...
					if(hasProvChanged && !MsgBox.Show(this,MsgBoxButtons.OKCancel,"The provider for the associated payment split will be changed along with the provider on the procedure.")) {
						return;
					}
				}
			}
          ...
          string range="";
			...
			if(hasProvChanged) {
				PaySplits.UpdateAttachedPaySplits(ProcCur);//only update the attached paysplits.
			}
			#endregion
			...
			{
				string promptText="";
				bool isUpdatingFee=Procedures.FeeUpdatePromptHelper(new List<Procedure>() { ProcCur.Copy() },new List<Procedure>() { ProcOld.Copy() },ref promptText);
			...
			//Autocodes----------------------------------------------------------------------------------------------------------------------------------------
			if(!ProcOld.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC)
				|| Security.IsAuthorized(Permissions.ProcComplEdit,ProcCur.DateEntryC,true)) {
			...
			if(ProcOld.ProcStatus!=ProcStat.C && ProcCur.ProcStatus==ProcStat.C){
				...
				if(ProcOld.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC)) {//If was complete before the window loaded.
