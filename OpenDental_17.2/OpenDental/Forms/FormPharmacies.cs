	...
	public class FormPharmacies:ODForm {
		...
		private void FormPharmacies_Load(object sender, System.EventArgs e) {
			...
			if(SelectedPharmacyNum!=0){
				for(int i=0;i<Pharmacies.ListShallow.Count;i++){
					if(Pharmacies.ListShallow[i].PharmacyNum==SelectedPharmacyNum){
		...
		private void FillGrid(){
			...
			for(int i=0;i<Pharmacies.ListShallow.Count;i++){
				row=new ODGridRow();
				row.Cells.Add(Pharmacies.ListShallow[i].StoreName);
				row.Cells.Add(Pharmacies.ListShallow[i].Phone);
				...
				row.Cells.Add(Pharmacies.ListShallow[i].Fax);
				txt=Pharmacies.ListShallow[i].Address;
				if(Pharmacies.ListShallow[i].Address2!=""){
					txt+="\r\n"+Pharmacies.ListShallow[i].Address2;
				}
				row.Cells.Add(txt);
				row.Cells.Add(Pharmacies.ListShallow[i].City);
				row.Cells.Add(Pharmacies.ListShallow[i].Note);
		...
		private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			if(IsSelectionMode){
				SelectedPharmacyNum=Pharmacies.ListShallow[e.Row].PharmacyNum;
				DialogResult=DialogResult.OK;
				return;
			}
			else{
				FormPharmacyEdit FormP=new FormPharmacyEdit();
				FormP.PharmCur=Pharmacies.ListShallow[e.Row];
				FormP.ShowDialog();
				FillGrid();
				changed=true;
			}
		...
		private void butOK_Click(object sender,EventArgs e) {
			...
			//	return;
				...
				SelectedPharmacyNum=Pharmacies.ListShallow[gridMain.GetSelectedIndex()].PharmacyNum;
			}
			DialogResult=DialogResult.OK;
		}
