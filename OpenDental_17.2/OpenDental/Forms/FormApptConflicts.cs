	...
	public partial class FormApptConflicts:ODForm {
		...
		private void FillGrid(){
			...
			foreach(Appointment apptCur in _listAppts) {
				...
				row.Cells.Add(Operatories.GetAbbrev(apptCur.Op,listOps));
				if(apptCur.IsHygiene) {
					row.Cells.Add(Providers.GetAbbr(apptCur.ProvHyg,listProvs));
				}
				else {
					row.Cells.Add(Providers.GetAbbr(apptCur.ProvNum,listProvs));
				}
