...
///<summary></summary>
    ...
    "ot delete the plan.";
			...
			// textDivisionNo
			// 
			this.textDivisionNo.Location = new System.Drawing.Point(296, 233);
			...
			// labelDivisionDash
			// 
			this.labelDivisionDash.Location = new System.Drawing.Point(242, 236);
    ...
    "t.\n5 lines in 72 vert\n6 in 85";
		...
		private void FormInsPlan_Load(object sender,System.EventArgs e) {
			Cursor=Cursors.WaitCursor;
			PlanNumOriginal=_planCur.PlanNum;
			PlanCurOriginal=_planCur.Copy();
			_listInsFilingCodes=InsFilingCodeC.Listt.Select(x => x.Clone()).ToList();
			if(_subCur!=null) {
		...
		private void FillFormWithPlanCur(bool isPicked) {
			...
			for(int i=0;i<ClaimForms.ListShort.Length;i++) {
				...
				if(ClaimForms.ListShort[i].ClaimFormNum==_planCur.ClaimFormNum) {
			...
			for(int i=0;i<FeeSchedsCopay.Count;i++) {
				...
				if(FeeSchedsCopay[i].FeeSchedNum==_planCur.CopayFeeSched)
			...
			List<Def> listDefs=new List<Def>() { new Def() { ItemName=Lans.g(this,"None")} };
			listDefs.AddRange(DefC.Short[(int)DefCat.BillingTypes].ToList());
			listDefs.ForEach(x => comboBillType.Items.Add(x.ItemName));
			comboBillType.IndexSelectOrSetText(listDefs.FindIndex(x => x.DefNum==_planCur.BillingType),() => { 
					return DefC.GetName(DefCat.BillingTypes,_planCur.BillingType)+" "+Lans.g(this,"(hidden)");
			});
		...
		private void butPick_Click(object sender,EventArgs e) {
			...
			if(FormIP.SelectedPlan.PlanNum==0) {//user clicked Blank
				...
				_planCur.PlanNum=PlanNumOriginal;
			}
		...
		private void comboBillType_SelectionChangeCommitted(object sender,EventArgs e) {
			if(comboBillType.SelectedIndex==0) {//'None' is selected.
				_planCur.BillingType=0;
				return;
			}
			_planCur.BillingType=DefC.Long[(int)DefCat.BillingTypes][comboBillType.SelectedIndex-1].DefNum;//-1 for 'None'
		}
		...
		private void FillBenefits() {
			...
			for(int i=0;i<benefitList.Count;i++) {
				...
				if(((Benefit)benefitList[i]).CodeNum==0) {
					//EXAMS-------------------------------------------------------------------------------------------------------------------------------------
					if(benefitList[i].BenefitType==InsBenefitType.Limitations
						&& CovCats.GetForEbenCat(EbenefitCategory.RoutinePreventive)!=null
						&& benefitList[i].CovCatNum==CovCats.GetForEbenCat(EbenefitCategory.RoutinePreventive).CovCatNum
						&& benefitList[i].MonetaryAmt==-1
						&& benefitList[i].PatPlanNum==0
					...
					if(proccode.ProcCode=="D0274"//4BW
						&& benefitList[i].BenefitType==InsBenefitType.Limitations
						&& benefitList[i].MonetaryAmt==-1
						&& benefitList[i].PatPlanNum==0
						&& benefitList[i].Percent==-1
						&& (benefitList[i].QuantityQualifier==BenefitQuantity.Months 
		...
		private void gridBenefits_DoubleClick(object sender,EventArgs e) {
			if(IsNewPlan && _planCur.PlanNum != PlanNumOriginal) {  //If adding a new plan and picked existing plan from list
		...
		private void GetEmployerNum() {
			if(_planCur.EmployerNum==0) {//no employer was previously entered.
				if(textEmployer.Text=="") {
					...
					if(PatPlanCur!=null && _employerNameOrig=="" && _planCur.PlanNum==PlanCurOriginal.PlanNum) {
						...
						PlanCurOriginal.EmployerNum=insPlanDB.EmployerNum;
					}
				}
				else {
		...
		private void butGetElectronic_Click(object sender,EventArgs e) {
			...
			try {
				...
				if(etrans != null) {
					...
					FormEtrans270Edit formE=new FormEtrans270Edit(PatPlanCur.PatPlanNum,_planCur.PlanNum,_subCur.InsSubNum);
		...
		private void butHistoryElect_Click(object sender,EventArgs e) {
			//button not visible if SubCur is null
			FormBenefitElectHistory formB=new FormBenefitElectHistory(_planCur.PlanNum,PatPlanCur.PatPlanNum,_subCur.InsSubNum);
		...
		private bool IsCarrierValid() {
			...
			else if(PlanNumOriginal!=_planCur.PlanNum) {//Plan was picked from list
				if(Security.IsAuthorized(Permissions.InsPlanEdit,true)) {
		...
		private bool FillPlanCurFromForm(){
			...
			if(Security.IsAuthorized(Permissions.InsPlanEdit,true)) {//User has the ability to edit carrier information.  Check for matches, create new Carrier if applicable.
				...
				if(_carrierCur.CarrierNum==_carrierNumOrig && Carriers.Compare(carrierForm,_carrierCur) && _planCur.PlanNum==PlanNumOriginal) {
					...
					if(PatPlanCur!=null) {
						...
						PlanCurOriginal.CarrierNum=_carrierCur.CarrierNum;
					}
					else {
				...
				if(Carriers.GetCarrier(_carrierCur.CarrierNum).CarrierName=="" && _planCur.PlanNum!=PlanNumOriginal) {//Carrier not found, it must have been deleted or combined
					MsgBox.Show(this,"Selected carrier has been combined or deleted.  Please choose another insurance plan.");
					return false;
				}
				else if(_planCur.PlanNum==PlanNumOriginal) {//Didn't switch insplan, they were only viewing.
					long planNumDb=PlanNumOriginal;
					if(PatPlanCur!=null) {
			...
			if(comboClaimForm.SelectedIndex!=-1){
				_planCur.ClaimFormNum=ClaimForms.ListShort[comboClaimForm.SelectedIndex].ClaimFormNum;
			}
			...
			if(comboFeeSched.SelectedIndex==0) {
				...
				_planCur.FeeSched = PlanCurOriginal.FeeSched;
			}
			else{
				_planCur.FeeSched=FeeSchedsStandard[comboFeeSched.SelectedIndex-1].FeeSchedNum;
			}
			if(comboCopay.SelectedIndex==0){
				_planCur.CopayFeeSched=0;
			}
			else{
				_planCur.CopayFeeSched=FeeSchedsCopay[comboCopay.SelectedIndex-1].FeeSchedNum;
			}
		...
		private void butAudit_Click(object sender,EventArgs e) {
			...
			FormAuditOneType FormA=new FormAuditOneType(0,perms,Lan.g(this,"All changes for InsPlan #")+" "+_planCur.PlanNum,_planCur.PlanNum);
			FormA.ShowDialog();
		}
		...
		private void butOK_Click(object sender,System.EventArgs e) {
			...
			if(_planCur.CarrierNum!=PlanCurOriginal.CarrierNum) {
				long patNum=0;
				if(PatPlanCur!=null) {//PatPlanCur will be null if editing insurance plans from Lists > Insurance Plans.
					patNum=PatPlanCur.PatNum;
				}
			...
			try {
				...
				if(PatPlanCur!=null) {
					...
					if(PIn.Date(textDateLastVerifiedPatPlan.Text)!=_datePatPlanLastVerified) {
			...
			if(Security.IsAuthorized(Permissions.InsPlanEdit,true)) {
				...
				if(_subCur==null) {//editing from big list.  No subscriber.  'pick from list' button not visible, making logic easier.
					...
					try {
						...
						if(InsPlans.AreEqualValue(_planCur,PlanCurOriginal)) {//If no changes
					...
					if(IsNewPlan) {
						if(InsPlans.AreEqualValue(_planCur,PlanCurOriginal)) {//New plan, no changes
							#region 5 - If the logic in this region changes, then also change region 5a below.
							try {
								if(_planCur.PlanNum != PlanNumOriginal) {//clicked 'pick from list' button
									...
									SecurityLogs.MakeLogEntry(Permissions.InsPlanEdit,(PatPlanCur==null)?0:PatPlanCur.PatNum
										,Lan.g(this,"FormInsPlan region 5 delete without checking dependencies first."),PlanNumOriginal);
									removeLogs=true;
									_subCur.PlanNum=_planCur.PlanNum;
							...
							if(_planCur.PlanNum != PlanNumOriginal) {//clicked 'pick from list' button, and then made changes
								...
								try {
									if(radioChangeAll.Checked) {
										...
										SecurityLogs.MakeLogEntry(Permissions.InsPlanEdit,(PatPlanCur==null)?0:PatPlanCur.PatNum
											,Lan.g(this,"FormInsPlan region 6 delete without checking dependencies first."),PlanNumOriginal);
										removeLogs=true;
										_subCur.PlanNum=_planCur.PlanNum;
										//PatPlanCur.PlanNum=PlanCur.PlanNum;
										//PatPlans.Update(PatPlanCur);
						...
						if(InsPlans.AreEqualValue(_planCur,PlanCurOriginal)) {//If no changes
							#region 7
							try {
								if(_planCur.PlanNum != PlanNumOriginal) {//clicked 'pick from list' button, then made no changes
																												 //do not need to update PlanCur because no changes were made.
									_subCur.PlanNum=_planCur.PlanNum;
									//PatPlanCur.PlanNum=PlanCur.PlanNum;
									//PatPlans.Update(PatPlanCur);
				...
				//Skipping the large section above for users without the InsPlanEdit permission was causing mass amounts of orphaned ins plans to stay around.
				//The safest thing to do is to do EXACTLY what the section of code above does for cleaning up our pre-inserted insurance plan as needed.
				if(_subCur!=null) {
					if(IsNewPlan) {
						if(InsPlans.AreEqualValue(_planCur,PlanCurOriginal)) {
							#region 5a - If the logic in this region changes, then also change region 5 above.
							try {
								if(_planCur.PlanNum != PlanNumOriginal) {
									InsPlans.Delete(PlanNumOriginal);
									SecurityLogs.MakeLogEntry(Permissions.InsPlanEdit,(PatPlanCur==null) ? 0 : PatPlanCur.PatNum
										,Lan.g(this,"FormInsPlan region 5a delete without checking dependencies first."),PlanNumOriginal);
								}
							}
							catch(Exception ex) {
							...
							if(_planCur.PlanNum != PlanNumOriginal) {
								#region 6a - If the logic in this region changes, then also change region 6 above.
								try {
									//Technically this radio button doesn't even show up when the user doesn't have permissions to edit ins plans.
									//It just feels better to have the code look identical to the code above.
									if(radioChangeAll.Checked) {
										InsPlans.Delete(PlanNumOriginal);
										SecurityLogs.MakeLogEntry(Permissions.InsPlanEdit,(PatPlanCur==null) ? 0 : PatPlanCur.PatNum
											,Lan.g(this,"FormInsPlan region 6a delete without checking dependencies first."),PlanNumOriginal);
									}
								}
								catch(Exception ex) {
									MessageBox.Show(Lan.g(this,"Error Code 6a")+".  "+Lan.g(this,"Please contact support")+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
									return;
								}
								#endregion
							}
			...
			#region 11
			try {
			if(PIn.Date(textDateLastVerifiedBenefits.Text)!=_dateInsPlanLastVerified) {
			...
			if(removeLogs) {
				InsEditLogs.DeletePreInsertedLogsForPlanNum(PlanNumOriginal);
			}
			...
			if(_planCur.CarrierNum!=PlanCurOriginal.CarrierNum) {
				...
				if(carrierNameOrig!=carrierNameNew) {//The CarrierNum could have changed but the CarrierName might not have changed.  Only make an audit entry if the name changed.
					...
					+carrierNameOrig+" "+Lan.g(this,"to")+" "+carrierNameNew,_planCur.PlanNum);
				}
			}
			}
			catch(Exception ex) {
			...
			if(PlanCurOriginal.FeeSched!=0 && PlanCurOriginal.FeeSched!=_planCur.FeeSched) {
			...
			if(InsPlanCrud.UpdateComparison(PlanCurOriginal,_planCur)) {
		...
		private void FormInsPlan_Closing(object sender, System.ComponentModel.CancelEventArgs e) {
			...
			if(IsNewPlan){//this would also be new coverage
				...
				SecurityLogs.MakeLogEntry(Permissions.InsPlanEdit,(PatPlanCur==null)?0:PatPlanCur.PatNum
					,Lan.g(this,"FormInsPlan_Closing delete without checking dependencies first."),PlanNumOriginal);
