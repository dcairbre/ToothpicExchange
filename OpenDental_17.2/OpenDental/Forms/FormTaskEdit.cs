...
namespace OpenDental {
	...
	public class FormTaskEdit:ODForm {
		...
		private void InitializeComponent() {
			...
			this.label8.Location = new System.Drawing.Point(625, 60);
			...
			this.checkDone.Location = new System.Drawing.Point(134, 2);
			...
			// labelDoneAffectsAll
			// 
			this.labelDoneAffectsAll.Location = new System.Drawing.Point(217, 2);
			...
			this.checkNew.Location = new System.Drawing.Point(12, 2);
			...
			// 
			this.textDateTimeFinished.Location = new System.Drawing.Point(107, 64);
			...
			// textDateTimeEntry
			// 
			this.textDateTimeEntry.Location = new System.Drawing.Point(107, 42);
			...
			// 
			this.textBoxDateTimeCreated.Location = new System.Drawing.Point(107, 20);
			...
			this.textBoxDateTimeCreated.TabIndex = 173;
			// 
			// label3
			// 
			...
			this.textDescript.Location = new System.Drawing.Point(91, 4);
			...
			this.textDescript.Size = new System.Drawing.Size(841, 99);
		...
		private void FormTaskListEdit_Load(object sender,System.EventArgs e) {
			...
			if(textBoxDateTimeCreated.Text==DateTime.MinValue.ToString()) {
		...
		private void FillComboJobs() {
			...
			if(_listJobs.Count==0) {
				...
				if(JobPermissions.GetForUser(Security.CurUser.UserNum).Count>0) {
		...
		private string CreateCopyTask() {
			string taskText=_taskCur.DateTimeEntry.ToShortDateString()+" "+_taskCur.DateTimeEntry.ToShortTimeString()+(textObjectDesc.Visible?" - "+textObjectDesc.Text:"")+" - "+textUser.Text+" - "+textDescript.Text;
			for(int i=0;i<_listTaskNotes.Count;i++) {
				taskText+="\r\n--------------------------------------------------\r\n";
		...
		private void butCreateJob_Click(object sender,EventArgs e) {
			...
			jobNew.Category=(JobCategory)categoryChoose.comboSelection.SelectedIndex;
			...
			jobNew.Description=CreateCopyTask();
				jobNew.Priority=JobPriority.Normal;
			if(jobNew.Category==JobCategory.Bug) {
				jobNew.Priority=JobPriority.High;
			}
			...
			if(jobNew.Category==JobCategory.Bug) {
				...
				Bug bugNew=Bugs.GetNewBugForUser();
				InputBox bugDescription=new InputBox("Provide a brief description for the bug. This will appear in the bug tracker.");
		...
		private bool SaveCur() {
			...
			//If user changed reminder type from NoReminder to some other type,
			//then we assume the date/time entered was intended for the reminder copy, not the current task.
			if(taskReminderType==TaskReminderType.Once) {
				dateTimeEntry=new DateTime(datePickerReminder.Value.Date.Year,datePickerReminder.Value.Date.Month,datePickerReminder.Value.Date.Day,
					timePickerReminder.Value.TimeOfDay.Hours,timePickerReminder.Value.TimeOfDay.Minutes,timePickerReminder.Value.TimeOfDay.Seconds);
			}
			else {
				dateTimeEntry=PIn.DateT(textDateTimeEntry.Text);
			}
			try {
				if(IsNew) {
					_taskCur.IsNew=true;
		...
		private void butSend_Click(object sender,EventArgs e) {
			...
			if(listObjectType.SelectedIndex==(int)TaskObjectType.Patient) {
				...
				_taskCur.TaskListNum=FormT.SelectedTaskListNum;
				...
				_taskCur.TaskListNum=FormT.TaskListNum;
				_taskListCur=TaskLists.GetOne(_taskCur.TaskListNum);
				textTaskList.Text=_taskListCur.Descript;
				if(!SaveCur()) {
					return;
		...
		private void butOK_Click(object sender,System.EventArgs e) {
			...
			if(!NotesChanged && _taskCur.Equals(_taskOld) && !_statusChanged) {//if there were no changes, then don't bother with the signal
				DialogResult=DialogResult.OK;
				Close();
				return;
			}
			...
			else if(NotesChanged || textDescript.Text!=_taskCur.Descript) {//notes or descript changed
				DataValid.SetInvalidTask(_taskCur.TaskNum,true);//popup
			}
			else {
				DataValid.SetInvalidTask(_taskCur.TaskNum,false);//no popup
