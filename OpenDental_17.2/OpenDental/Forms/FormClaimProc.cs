...
namespace OpenDental
{
	...
	public class FormClaimProc : ODForm
	{
		...
		public void Initialize() {
			...
			_listPayTrackDefs=new List<Def>();
			_listPayTrackDefs.AddRange(DefC.GetList(DefCat.ClaimPaymentTracking));
		...
		private void FillAllowed(){
			if(IsProc){
				...
				CarrierAllowedAmount=InsPlans.GetAllowed(ProcedureCodes.GetStringProcCode(proc.CodeNum),plan.FeeSched,plan.AllowedFeeSched,
					!SubstitutionLinks.HasSubstCodeForPlan(plan,proc.CodeNum,ListSubLinks),plan.PlanType,proc.ToothNum,proc.ProvNum,proc.ClinicNum);
		...
		private void butOK_Click(object sender, System.EventArgs e) {
				...
				&& ClaimProcCur.ProvNum != ClaimProcOld.ProvNum) 
			{
				string strSecLog;
				if(proc == null) {
					strSecLog = Lan.g(this,"Total Payment for")+" "+textInsPlan.Text+". "+Lan.g(this,"Provider changed from")+" "
