...
namespace OpenDental {
	public partial class FormEmailInbox:ODForm,ISignalProcessor {
    ...
    public FormEmailInbox() {
      ...
      if(_listEmailAddresses!=null) {//Since Resize() is called once before Load(), we must ensure FillComboEmail() has been called at least once.
        switch(ActiveMailbox) {
          case MailboxType.Inbox:
            FillGridInbox();//To resize the columnns in the grid as needed.
            break;
          case MailboxType.Sent:
            FillGridSent();
            break;
    ...
    ///<summary>This function is called on load and also when signals are received indicating a refresh is needed for email addresses.</summary>
      ...
      _listEmailAddresses=new List<EmailAddress>();
			List<EmailAddress> listAddresses=EmailAddresses.GetListt();//Does not include user specific email addresses.
    ...
    private void gridInboxSent_CellClick(object sender,UI.ODGridClickEventArgs e) {
          ...
          || emailMessage.SentOrReceived==EmailSentOrReceived.WebMailSentRead) 
      {
        ...
        FormEmailMessageEdit FormEME=new FormEmailMessageEdit(emailMessage,false);
        FormEME.EmailPreviewAddress=AddressCur;
    ...
    private void RefreshInboxEmailList() {
      _listInboxEmails=EmailMessages.GetMailboxForAddress(AddressCur,MailboxType.Inbox);
      _isRefreshInbox=false;
    }

    ///<summary>Refreshes the local list of sent emails.</summary>
    private void RefreshSentEmailList() {
      _listSentEmails=EmailMessages.GetMailboxForAddress(AddressCur,MailboxType.Sent);
      _isRefreshSent=false;
    }

    private void RefreshLists() {
      List<EmailMessage> listEmailsForSelection = EmailMessages.GetMailboxForAddress(AddressCur,MailboxType.Inbox,MailboxType.Sent);
    ...
    private void butCompose_Click(object sender,EventArgs e) {
      ...
      message.FromAddress=string.IsNullOrEmpty(AddressCur.SenderAddress)?AddressCur.EmailUsername:AddressCur.SenderAddress;
    ...
    private void butReply_Click(object sender,EventArgs e) {
      ...
      FormEmailMessageEdit FormE=new FormEmailMessageEdit(EmailMessages.CreateReply(selectedMessage,AddressCur));
      FormE.IsNew=true;
