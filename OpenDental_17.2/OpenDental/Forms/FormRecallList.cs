...
///<summary></summary>
	public class FormRecallList:ODForm,ISignalProcessor {
		...
		private void InitializeComponent()
		{
			...
			this.groupBox3.Location = new System.Drawing.Point(654, 1);
			...
			this.groupBox2.Location = new System.Drawing.Point(654, 44);
		...
		private void CheckClinicsSignedUpForWebSched() {
			...
			_threadWebSchedSignups=new ODThread(new ODThread.WorkerDelegate((ODThread o) => {
				_listClinicNumsWebSched=WebServiceMainHQProxy.GetEServiceClinicsAllowed(
					Clinics.ListLong.Select(x => x.ClinicNum).ToList(),
		...
		private void FillMain(){
			...
			_tableRecalls=Recalls.GetRecallList(fromDate,toDate,checkGroupFamilies.Checked,provNum,clinicNum,siteNum,sortBy,showReminders);
		...
		private void SendWebSched() {
			...
			Dictionary<long,long> dictWebSchedRecallNums=new Dictionary<long, long>();
			...
			for(int i=0;i<gridMain.SelectedIndices.Length;i++) {
				...
				dictWebSchedRecallNums.Add(PIn.Long(_tableRecalls.Rows[gridMain.SelectedIndices[i]]["RecallNum"].ToString()),
					PIn.Long(_tableRecalls.Rows[gridMain.SelectedIndices[i]]["WebSchedRecallNum"].ToString()));
			}
