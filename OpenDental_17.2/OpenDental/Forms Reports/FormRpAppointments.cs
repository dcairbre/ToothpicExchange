...
namespace OpenDental
{
	...
	public class FormRpAppointments : ODForm {
		...
		private void InitializeComponent()
		{
			...
			this.groupBox2.Location = new System.Drawing.Point(264, 174);
		...
		private void butOK_Click(object sender,System.EventArgs e) {
			...
			if(checkWebSchedNewPat.Checked) {
				listSources.Add(LogSources.WebSchedNewPatAppt);
			}
			if(checkWebSchedRecall.Checked) {
			...
			if(radioDateAptCreated.Checked) {
				command+="appointment.SecDateEntry,";
			}
			command += @"appointment.AptDateTime,"
				+"patient.PatNum,TRIM(CONCAT(CONCAT(CONCAT(CONCAT(CONCAT(patient.LName,', '),"
				+"CASE WHEN LENGTH(patient.Preferred) > 0 THEN CONCAT(CONCAT('(',patient.Preferred),') ') ELSE '' END)"
				...
				+",COALESCE(clinic.Description,'"+POut.String(Lans.g("FormRpAppointments","Unassigned"))+"') ClinicDesc"
				+" FROM appointment"
				...
				+whereClinics;
			if(radioDateAptCreated.Checked) {
				command+=" AND appointment.SecDateEntry BETWEEN "+POut.Date(dateFrom)+" AND "+POut.Date(dateTo.AddDays(1))
				+" ORDER BY appointment.ClinicNum,appointment.SecDateEntry,PatName";
			}
			else {
				command+=" AND appointment.AptDateTime BETWEEN "+POut.Date(dateFrom)+" AND "+POut.Date(dateTo.AddDays(1))
