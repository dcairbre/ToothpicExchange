...
namespace OpenDental {
	public partial class FormJobManager2:ODForm, ISignalProcessor {
		...
		///<summary>The colors that correspond to the job.JobPriority enum.  They need to be in the same order as that enum.</summary>
		private static Color[] statusColors = new Color[]{
					Color.FromArgb(254,179,179), //High Priority
					Color.White, //Normal Priority
					Color.FromArgb(233,233,233), //Low Priority
					Color.FromArgb(200,200,200),//On Hold
					Color.FromArgb(255,43,43), //Urgent Priority
					Color.FromArgb(255,209,179), //Medium-High Priority
				};
		...
		private void RefreshAndFillThreaded() {
			ODThread thread = new ODThread((o) => {
				...
				this.Invoke((Action)FillExtraGrids);
		...
		private void FillGridActions() {
			...
			if(userFilter==null 
				|| (!JobPermissions.IsAuthorized(JobPerm.Documentation,true,userFilter.UserNum) 
				&& !JobPermissions.IsAuthorized(JobPerm.NotifyCustomer,true,userFilter.UserNum))) 
			{
				#region Non-Documentation
				...
				foreach(KeyValuePair<JobAction,List<Job>> kvp in dictActions) {
					...
					List<Job> listJobsSorted = kvp.Value//filter
						...
						.ThenBy(x => userControlJobEdit._listJobPrioritiesSorted.IndexOf(x.Priority)).ToList();
							...
							new ODGridCell(x.Priority.ToString())
								{
									...
									ColorText=(x.Priority==JobPriority.Urgent) ? Color.White : Color.Black,
								},
							new ODGridCell(FlagHelper(x,gridAction.Rows.Count))
								{
									ColorText=x.TagOD!=null ? (Color)x.TagOD:Color.Black//Set in FlagCellHelper(...), tag is reset everytime FillGridActions() is called.
								},
				...
				if(JobPermissions.IsAuthorized(JobPerm.NotifyCustomer,true,userFilter.UserNum)) {
					...
					gridAction.Title="Jobs To Document";
					tabAction.Text="Needs Documentation";
				}
				checkShowUnassigned.Checked=true;
				checkShowUnassigned.Enabled=false;
				long selectedJobNum = 0;
				if(userControlJobEdit.GetJob()!=null) {
					selectedJobNum=userControlJobEdit.GetJob().JobNum;
				}
				...
				gridAction.Columns.Add(new ODGridColumn("Priority",50) { TextAlign=HorizontalAlignment.Center });
				gridAction.Columns.Add(new ODGridColumn("Owner",55) { TextAlign=HorizontalAlignment.Center });// X for yes, - for unassigned
				gridAction.Columns.Add(new ODGridColumn("Version",55) { TextAlign=HorizontalAlignment.Center });// X for yes, - for unassigned
				gridAction.Columns.Add(new ODGridColumn("",245));
				gridAction.Rows.Clear();
				//Sort jobs into action dictionary
				Dictionary<JobAction,List<Job>> dictActions = new Dictionary<JobAction,List<Job>>();
		...
		private void FillExtraGrids() {
			...
			foreach(KeyValuePair<JobCategory,List<Job>> kvp in dictCategories) {
				List<Job> listJobsSorted = kvp.Value.OrderBy(x => x.Priority).ToList();
				listJobsSorted.RemoveAll(x => x.Priority==JobPriority.OnHold || x.OwnerAction!=JobAction.AssignExpert);
				if(listJobsSorted.Count==0) {
					continue;
				}
				gridAvailableJobsExpert.Rows.Add(new ODGridRow("",kvp.Key.ToString()) { ColorBackG=gridAvailableJobsExpert.HeaderColor,Bold=true });
				listJobsSorted.ForEach(x => gridAvailableJobsExpert.Rows.Add(
					new ODGridRow(
						new ODGridCell(x.Priority.ToString()) { CellColor=statusColors[(int)x.Priority] },
						new ODGridCell(x.ToString()) { CellColor=(x.ToString().ToLower().Contains(textSearch.Text.ToLower())&&!string.IsNullOrWhiteSpace(textSearch.Text) ? Color.LightYellow : Color.Empty) }
						) { Tag=x }
			...
			foreach(KeyValuePair<JobCategory,List<Job>> kvp in dictCategories) {
				List<Job> listJobsSorted = kvp.Value.OrderBy(x => x.Priority).ToList();
				listJobsSorted.RemoveAll(x => x.Priority!=JobPriority.OnHold || x.PhaseCur==JobPhase.Complete || x.PhaseCur==JobPhase.Documentation || x.PhaseCur==JobPhase.Cancelled);
				if(listJobsSorted.Count==0) {
					continue;
				}
				gridJobsOnHold.Rows.Add(new ODGridRow("","",kvp.Key.ToString()) { ColorBackG=gridJobsOnHold.HeaderColor,Bold=true });
				listJobsSorted.ForEach(x => gridJobsOnHold.Rows.Add(
					new ODGridRow(
						new ODGridCell(x.Priority.ToString()) { CellColor=statusColors[(int)x.Priority] },
						new ODGridCell(x.OwnerNum==0 ? "-" : Userods.GetName(x.OwnerNum)),
						new ODGridCell(x.ToString()) { CellColor=(x.ToString().ToLower().Contains(textSearch.Text.ToLower())&&!string.IsNullOrWhiteSpace(textSearch.Text) ? Color.LightYellow : Color.Empty) }
