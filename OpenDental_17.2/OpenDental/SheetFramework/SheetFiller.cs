	...
	public class SheetFiller {
		...
		private static void FillFieldsInStaticText(Sheet sheet,Patient pat) {
			...
			if(pat!=null) {
				...
				List<long> listIntraoralComplete=ProcedureCodes.GetProcCodeStartsWith("D0210",hashProcedureCodes).Select(x => x.CodeNum).ToList();//intraoral - complete series (including bitewings)
				listProcCodeNums.AddRange(listIntraoralComplete);
				listIntraoralAndBiteWings.AddRange(listIntraoralComplete);
				List<long> listBiteWingSingle=ProcedureCodes.GetProcCodeStartsWith("D0270",hashProcedureCodes).Select(x => x.CodeNum).ToList();//bitewing - single film
				listProcCodeNums.AddRange(listBiteWingSingle);
				listIntraoralAndBiteWings.AddRange(listBiteWingSingle);
				List<long> listBiteWingsTwo=ProcedureCodes.GetProcCodeStartsWith("D0272",hashProcedureCodes).Select(x => x.CodeNum).ToList();//bitewings - two films
				listProcCodeNums.AddRange(listBiteWingsTwo);
				listIntraoralAndBiteWings.AddRange(listBiteWingsTwo);
				List<long> listBiteWingsFour=ProcedureCodes.GetProcCodeStartsWith("D0274",hashProcedureCodes).Select(x => x.CodeNum).ToList();//bitewings - four films
				listProcCodeNums.AddRange(listBiteWingsFour);
				listIntraoralAndBiteWings.AddRange(listBiteWingsFour);
				List<long> listVertBiteWings7to8=ProcedureCodes.GetProcCodeStartsWith("D0277",hashProcedureCodes).Select(x => x.CodeNum).ToList();//vertical bitewings - 7 to 8 films
				listProcCodeNums.AddRange(listVertBiteWings7to8);
				listIntraoralAndBiteWings.AddRange(listVertBiteWings7to8);
				List<long> listBiteWingsThree=ProcedureCodes.GetProcCodeStartsWith("D0273",hashProcedureCodes).Select(x => x.CodeNum).ToList();//bitewings - three films
				listProcCodeNums.AddRange(listBiteWingsThree);
				listIntraoralAndBiteWings.AddRange(listBiteWingsThree);
				List<long> listPerioOralEval=ProcedureCodes.GetProcCodeStartsWith("D0120",hashProcedureCodes).Select(x => x.CodeNum).ToList();//periodic oral evaluation - established patient
				listProcCodeNums.AddRange(listPerioOralEval);
				listOralEvals.AddRange(listPerioOralEval);
				List<long> listLimititedOralEval=ProcedureCodes.GetProcCodeStartsWith("D0140",hashProcedureCodes).Select(x => x.CodeNum).ToList();//limited oral evaluation - problem focused
				listProcCodeNums.AddRange(listLimititedOralEval);
				listOralEvals.AddRange(listLimititedOralEval);
				List<long> listCompOralEval=ProcedureCodes.GetProcCodeStartsWith("D0150",hashProcedureCodes).Select(x => x.CodeNum).ToList();//comprehensive oral evaluation - new or established patient
				listProcCodeNums.AddRange(listCompOralEval);
				listOralEvals.AddRange(listCompOralEval);
				List<long> listDetailedExtensiveOralEval=ProcedureCodes.GetProcCodeStartsWith("D0160",hashProcedureCodes).Select(x => x.CodeNum).ToList();//detailed and extensive oral evaluation - problem focused, by report
				listProcCodeNums.AddRange(listDetailedExtensiveOralEval);
				listOralEvals.AddRange(listDetailedExtensiveOralEval);
				List<long> listPerioMaintenance=ProcedureCodes.GetProcCodeStartsWith("D4910",hashProcedureCodes).Select(x => x.CodeNum).ToList();//Periodontal Maintenance
				listProcCodeNums.AddRange(listPerioMaintenance); 
				List<long> listPanoramicFilm=ProcedureCodes.GetProcCodeStartsWith("D0330",hashProcedureCodes).Select(x => x.CodeNum).ToList();//panoramic film
				listProcCodeNums.AddRange(listPanoramicFilm);
				List<long> listProphylaxisAdult=ProcedureCodes.GetProcCodeStartsWith("D1110",hashProcedureCodes).Select(x => x.CodeNum).ToList();//prophylaxis - adult
				listProcCodeNums.AddRange(listProphylaxisAdult);
				listProphy.AddRange(listProphylaxisAdult);
				List<long> listProhylaxisChild=ProcedureCodes.GetProcCodeStartsWith("D1120",hashProcedureCodes).Select(x => x.CodeNum).ToList();//prophylaxis - child
				listProcCodeNums.AddRange(listProhylaxisChild);
				listProphy.AddRange(listProhylaxisChild);
				List<long> listTopicalFluorideProphyChild=ProcedureCodes.GetProcCodeStartsWith("D1201",hashProcedureCodes).Select(x => x.CodeNum).ToList();//Topical Fluoride Including Prophy-Child
				listProcCodeNums.AddRange(listTopicalFluorideProphyChild);
				listProphy.AddRange(listTopicalFluorideProphyChild);
				List<long> listTopicalFluorideProphyAdult=ProcedureCodes.GetProcCodeStartsWith("D1205",hashProcedureCodes).Select(x => x.CodeNum).ToList();//Topical Fluoride Including Prophy-Adult
				...
				for(int i=0;i<fam.ListPats.Length;i++) {
					recall=Recalls.GetRecallProphyOrPerio(fam.ListPats[i].PatNum,true);
			...
			foreach(SheetField field in sheet.SheetFields) {
				switch(field.FieldName) {
					...
					case "returnAddress":
						...
						if(PrefC.HasClinicsEnabled && Clinics.ListLong.Count>0 //if using clinics
		...
		private static string[] totInsBalLabsHelper(Sheet sheet,Statement Stmt) {
			...
			return new string[] { sLine1,sLine2,sLine3,sLine4,sLine5 };
		}
		...
		private static string[] totInsBalValsHelper(Sheet sheet,Statement Stmt,DataSet stmtData) {
			...
			if(Stmt.IsInvoice) {
				...
				sLine5+=(paymentValue + procAmt + adjAmt).ToString("c");
			}
