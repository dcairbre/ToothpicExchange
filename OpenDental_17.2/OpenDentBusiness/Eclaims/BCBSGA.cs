...
namespace OpenDentBusiness.Eclaims {
	public class BCBSGA {
		...
		public static bool Retrieve(Clearinghouse clearinghouseClin,bool isAutomatic,ITerminalConnector terminalConnector) {
			bool retVal=true;
			try {
				terminalConnector.ShowForm();
				terminalConnector.OpenConnection(clearinghouseClin.ModemPort);
	...
	//Possible issue with Trans ID
				...
				while(response==(byte)'Y') {
					...
					if(response==(byte)'N') {
						if(!isAutomatic) {
							MessageBox.Show(terminalConnector.Receive(10000));
						}
						break;
					}
