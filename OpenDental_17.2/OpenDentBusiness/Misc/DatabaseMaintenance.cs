...
namespace OpenDentBusiness {
	public class DatabaseMaintenance {
		...
		public static string MySQLServerOptionsValidate(bool verbose,DbmMode modeCur) {
			...
			command="SHOW GLOBAL VARIABLES LIKE 'sql_mode'";
		...
		public static string MySQLTables(bool verbose,DbmMode modeCur) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetString(MethodBase.GetCurrentMethod(),verbose,modeCur);
			}
			string log="";
		...
		[DbmMethodAttr]
		public static string AppointmentNoteTooManyNewLines(bool verbose,DbmMode modeCur) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetString(MethodBase.GetCurrentMethod(),verbose,modeCur);
			}
			...
			switch(modeCur) {
				case DbmMode.Check:
					command="SELECT COUNT(*) FROM appointment WHERE Note LIKE '%"+POut.String(tooManyRN)+"%'";
					int numFound=PIn.Int(Db.GetCount(command));
					if(numFound>0 || verbose) {
					...
					break;
				case DbmMode.Fix:
					command="SELECT * FROM appointment WHERE Note LIKE '%"+POut.String(tooManyRN)+"%'";
					...
					if(tableAppts.Rows.Count>0 || verbose) {
						for(int i=0;i<tableAppts.Rows.Count;i++) {
							...
							Appointment apt=oldApt.Copy();
							apt.Note=Regex.Replace(oldApt.Note,POut.String(tooManyRN)+"[\r\n]*","\r\n");
		...
		public static string ClearinghouseInvalidFormat(bool verbose,DbmMode modeCur) {
			...
			command="SELECT clearinghouse.Description,COALESCE(clinic.Abbr,'"+POut.String(Lans.g("FormDatabaseMaintenance","Unassigned"))+"') Abbr "
				+"FROM clearinghouse "
		...
		public static string DepositsWithIncorrectAmount(bool verbose,DbmMode modeCur,long userNum=0) {
			...
			command=@"SELECT deposit.DepositNum,deposit.Amount,deposit.DateDeposit,SUM(payments.amt) _sum
				FROM deposit
				INNER JOIN (
		...
		public static string DiseaseWithInvalidDiseaseDef(bool verbose,DbmMode modeCur) {
			...
			string log="";
			command=@"SELECT DiseaseNum,DiseaseDefNum FROM disease WHERE DiseaseDefNum NOT IN(SELECT DiseaseDefNum FROM diseasedef)";
			DataTable table=Db.GetTable(command);
			int numFound=table.Rows.Count;
			switch(modeCur) {
				case DbmMode.Check:
		...
		public static string EbillDuplicates(bool verbose,DbmMode modeCur) {
			...
			command="SELECT MIN(EbillNum) EbillNum,COUNT(*) Count "
				+"FROM ebill "
				+"GROUP BY ClinicNum ";
		...
		public static string EbillMissingDefaultEntry(bool verbose,DbmMode modeCur) {
			...
			string log="";
			command="SELECT COUNT(*) FROM ebill WHERE ClinicNum=0";
			int numFound=PIn.Int(Db.GetCount(command));
			switch(modeCur){
			case DbmMode.Check:
				if(numFound==0) {
		...
		public static string EduResourceInvalidDiseaseDefNum(bool verbose,DbmMode modeCur) {
			...
			command="SELECT EduResourceNum FROM eduresource WHERE DiseaseDefNum != 0 AND DiseaseDefNum NOT IN (SELECT DiseaseDefNum FROM diseasedef)";
			table=Db.GetTable(command);
			string log="";
			switch(modeCur) {
				case DbmMode.Check:
		...
		public static string FeeDeleteForInvalidProc(bool verbose,DbmMode modeCur) {
			...
			command=@"SELECT FeeNum,FeeSched,fee.CodeNum AS 'CodeNum' FROM fee 
			...
			switch(modeCur) {
				...
				case DbmMode.Fix:
					...
					for(int i=0;i<count;i++) {
						...
						command="DELETE FROM fee WHERE FeeNum="+table.Rows[i]["FeeNum"].ToString();
						numberFixed+=Db.NonQ(command);
					}
					if(numberFixed>0 || verbose) {
		...
		public static string InsPlanInvalidNum(bool verbose,DbmMode modeCur,long userNum=0,long patNum=0) {
			...
			if(patNum > 0) {
				...
				if(listPatPlans.Count < 1) {
					...
					listInsSubNums=listPatPlans.Select(x => x.InsSubNum).ToList();
			...
			switch(modeCur) {
				...
				case DbmMode.Breakdown:
					...
					command="SELECT claim.PatNum, claim.PlanNum, claim.ClaimNum, claim.DateService FROM claim "
						...
						+"WHERE claim.PlanNum!=inssub.PlanNum";
					...
					command="SELECT claim.PatNum, claim.PlanNum2, claim.ClaimNum, claim.DateService FROM claim "
						+"LEFT JOIN inssub ON claim.InsSubNum=inssub.InsSubNum "
						+"WHERE claim.PlanNum2!=inssub.PlanNum AND claim.PlanNum2!=0";
					...
					command="SELECT claimproc.PatNum, claimproc.ClaimProcNum, claimproc.InsSubNum, claimproc.ProcNum, claimproc.ClaimNum FROM claimproc "
						+"LEFT JOIN inssub ON claimproc.InsSubNum=inssub.InsSubNum "
						+"WHERE claimproc.PlanNum!=inssub.PlanNum";
					...
					command="SELECT etrans.PatNum, etrans.PlanNum, etrans.EtransNum, etrans.DateTimeTrans FROM etrans "
						+"LEFT JOIN inssub ON etrans.InsSubNum=inssub.InsSubNum "
						+"WHERE etrans.PlanNum!=inssub.PlanNum AND etrans.PlanNum!=0";
					...
					command="SELECT payplan.PatNum, payplan.PlanNum, payplan.PayPlanNum FROM payplan "
						+"LEFT JOIN inssub ON payplan.InsSubNum=inssub.InsSubNum "
						+"WHERE payplan.PlanNum!=inssub.PlanNum";
			...
			switch(modeCur) {
				...
				case DbmMode.Fix:
					...
					command=@"SELECT *,SUM(SplitAmt) SplitAmt_ 
						...
						GROUP BY PayNum,PatNum,DatePay";
					DataTable table=Db.GetTable(command);
					if(table.Rows.Count>0 || verbose) {
						Def[][] arrayDefs=DefC.GetArrayShort();
						for(int i=0;i<table.Rows.Count;i++) {
							...
							long oldPayNum=payment.PayNum;
							Payments.Insert(payment,oldPayNum != 0);
							if(oldPayNum==0) {//If the paysplit's PayNum was 0, we need to update that to the newly created payment's PayNum.
		...
		public static string PaySplitWithInvalidPrePayNum(bool verbose,DbmMode modeCur) {
			...
			command="SELECT ps1.* FROM paysplit ps1 LEFT JOIN paysplit ps2 ON ps1.PrePaymentNum=ps2.SplitNum WHERE ps1.PrePaymentNum!=0 AND ps2.SplitNum IS NULL";
			DataTable table=Db.GetTable(command);
			if(table.Rows.Count==0 && !verbose) {
				return "";
			}
		...
		public static string ProgramPropertiesDuplicatesForHQ(bool verbose,DbmMode modeCur) {
			...
			command="SELECT MIN(ProgramPropertyNum) ProgramPropertyNum,COUNT(*) Count "
		...
		public static string ProgramPropertiesMissingForClinic(bool verbose,DbmMode modeCur) {
			...
			string progNumStr=POut.Long(Programs.GetProgramNum(ProgramName.Xcharge))+","+POut.Long(Programs.GetProgramNum(ProgramName.PayConnect));
			command="SELECT DISTINCT pphq.*,clinic.ClinicNum missingClinicNum "//Distinct in case there are duplicate prog props with a ClinicNum 0.
		...
		public static string QuickPasteNoteWithInvalidCatNum(bool verbose,DbmMode modeCur) {
			...
			command=@"SELECT COUNT(*) FROM quickpastenote WHERE QuickPasteCatNum=0";
			int numFound=PIn.Int(Db.GetCount(command));
			string log="";
			switch(modeCur) {
				case DbmMode.Check:
		...
		public static string TreatPlanAttachDuplicateProc(bool verbose,DbmMode modeCur) {
			...
			command="SELECT treatplanattach.TreatPlanNum, MIN(treatplanattach.TreatPlanAttachNum) AS OriginalTPANum, "
			+ " ProcNum, "
		...
		public static string TreatPlanOrphanedProcTps(bool verbose,DbmMode modeCur) {
			...
			command=@"SELECT proctp.PatNum,proctp.TreatPlanNum 
				FROM proctp
...
HAVING cnt>1";
		...
		public static List<MethodInfo> GetMethodsForDisplay(long clinicNum=0,bool hasOnlyPatNumMethods=false) {
			...
			MethodInfo[] arrayDbmMethodsAll=(typeof(DatabaseMaintenance)).GetMethods();
