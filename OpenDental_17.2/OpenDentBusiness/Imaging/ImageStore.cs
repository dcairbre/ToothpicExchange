	...
	public class ImageStore {
		...
		public static string GetPreferredAtoZpath() {
			if(PrefC.AtoZfolderUsed==DataStorageType.InDatabase) {
				...
				//Override path.  Becuase it overrides all other paths, we evaluate it first.
		...
		public static Document ImportForm(string form,long docCategory,Patient pat) {
			...
			try {
				if(CloudStorage.IsCloudStorage) {
					...
					ImageStore.LogDocument(Lans.g("ContrImages","Document Created")+": ",Permissions.ImageEdit,doc);
				}
				else {
		...
		///<summary>If using AtoZ folder, then patFolder must be fully qualified and valid.  If not usingAtoZ folder, this uploads to Cloud or fills the doc.RawBase64 which must then be updated to db.  The image format can be bmp, jpg, etc, but this overload does not allow specifying jpg compression quality.</summary>
