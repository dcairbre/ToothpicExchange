...
namespace OpenDentBusiness {
	public class DefC {
		...
		public static List<Def> GetListAll() {
			Def[][] arrayDefs = GetArrayLong();
			List<Def> listDefs = new List<Def>();
			foreach(DefCat cat in Enum.GetValues(typeof(DefCat))) {
				listDefs.AddRange(arrayDefs[(int)cat].ToList());
			}
			return listDefs;
		}
		...
		public static Def GetDef(DefCat myCat,long myDefNum,Def[][] arrayDefs=null) {
			if(arrayDefs==null) {
		...
		///<summary>Returns the ItemName for the def passed in or empty string if it cannot be found.
		///Pass in a specific list of defs to save from making deep copies of the cache if you are going to call this method repeatedly.
		///Otherwise uses a deep copy of the entire Defs cache (including hidden items) and searches for the corresponding def.</summary>
		public static string GetName(long defNum,List<Def> listDefs=null) {
			if(listDefs==null) {
				listDefs=GetListAll();
			}
			Def def=listDefs.FirstOrDefault(x => x.DefNum==defNum);
		...
		///<summary></summary>
		public static string GetValue(DefCat myCat,long myDefNum,Def[][] arrayDefs=null) {
			string retStr="";
			if(arrayDefs==null) {
				arrayDefs=GetArrayLong();
			}
		...
		///<summary></summary>
		public static Color GetColor(DefCat myCat,long myDefNum,Def[][] arrayDefs=null) {
		...
		public static List<Def> GetDiscountPlanAdjTypes() {
			...
			Def[][] arrayDefs=GetArrayShort();
			for(int i=0;i<arrayDefs[(int)DefCat.AdjTypes].Length;i++) {
				if(arrayDefs[(int)DefCat.AdjTypes][i].ItemValue=="dp") {
					retVal.Add(arrayDefs[(int)DefCat.AdjTypes][i]);
				}
			}
			return retVal;
