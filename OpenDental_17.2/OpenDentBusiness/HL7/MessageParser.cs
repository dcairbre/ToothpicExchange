		...
		public static void ProcessAIGorAIP(Patient pat,Appointment apt,HL7DefSegment segDef,SegmentHL7 seg) {
			...
			if(apt.Confirmed!=aptOld.Confirmed) {
				...
				SecurityLogs.MakeLogEntry(Permissions.ApptConfirmStatusEdit,apt.PatNum,
					...
					+DefC.GetName(DefCat.ApptConfirmed,aptOld.Confirmed)+" "+Lans.g("MessageParser","to")+" "+DefC.GetName(DefCat.ApptConfirmed,apt.Confirmed)
					+" "+Lans.g("MessageParser","due to an indound HL7 message")+".",apt.AptNum,LogSources.HL7);
			}
			_aptProcessed=apt;
			if(_isVerboseLogging) {
