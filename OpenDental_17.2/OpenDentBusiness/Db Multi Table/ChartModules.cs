		...
		public static DataTable GetProgNotes(long patNum,	bool isAuditMode,ChartModuleComponentsToLoad componentsToLoad) {
				...
				|| componentsToLoad.ShowConditions)
			{
				...
				foreach(DataRow rowProc in rawProcs.Rows) {
					ProcedureCode procCode=ProcedureCodes.GetProcCode(PIn.Long(rowProc["CodeNum"].ToString()),listProcedureCodes);
					row=table.NewRow();
					...
					row["dx"]=DefC.GetValue(DefCat.Diagnosis,PIn.Long(rowProc["Dx"].ToString()),arrayDefs);
					...
					if(componentsToLoad.ShowProcNotes) {
						...
						if(isAuditMode) {//we will include all notes for each proc.  We will concat and make readable.
							for(int n=0;n<rawNotes.Rows.Count;n++) {//loop through each note
								if(rowProc["ProcNum"].ToString() != rawNotes.Rows[n]["ProcNum"].ToString()) {
									continue;
								}
					...
					row["priority"]=DefC.GetName(DefCat.TxPriorities,PIn.Long(rowProc["Priority"].ToString()),arrayDefs);
					...
					row["prognosis"]=DefC.GetName(DefCat.Prognosis,PIn.Long(rowProc["Prognosis"].ToString()),arrayDefs);
					...
					if(procCode.TreatArea==TreatmentArea.Surf) {
						...
						row["surf"]=Tooth.GetSextant(rowProc["Surf"].ToString(),
							(ToothNumberingNomenclature)PrefC.GetInt(PrefName.UseInternationalToothNumbers,dictPrefs));
					}
					else {
						row["surf"]=rowProc["Surf"].ToString();
					}
			...
			if(componentsToLoad.ShowTasks) {
				...
				for(int i=0;i<rawTask.Rows.Count;i++) {
					...
					row["dateTP"]="";
					txt="";
					switch(PIn.Enum<TaskObjectType>(rawTask.Rows[i]["ObjectType"].ToString())) {
						...
						case TaskObjectType.Appointment:
							...
							if(rawTask.Rows[i]["PatNum"].ToString()!=patNum.ToString()) {
			...
			if(componentsToLoad.ShowSheets) {
				...
				if(PrefC.GetBool(PrefName.ShowFeaturePatientClone,dictPrefs)) {
				...
				for(int i=0;i<rawSheet.Rows.Count;i++) {
					...
					if(PrefC.GetBool(PrefName.ShowFeaturePatientClone,dictPrefs) && rawSheet.Rows[i]["PatNum"].ToString()!=patNum.ToString()) {
