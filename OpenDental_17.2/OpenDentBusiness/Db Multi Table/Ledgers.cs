...
namespace OpenDentBusiness{
	...
	public class Ledgers {
			...
			if(PrefC.GetBool(PrefName.AgingIsEnterprise)) {
				...
				if(guarantor==0) {
					command="TRUNCATE TABLE famaging;"
						...
						+"("
							+GetAgingQueryString(0,asOfDate)
					...
					FamAging famAgingCur=Crud.FamAgingCrud.SelectOne(GetAgingQueryString(guarantor,asOfDate));
				...
				command+=(isMySqlDb?"UPDATE patient p,":"MERGE INTO patient p USING ")
					+"("+GetAgingGuarTransQuery(guarantor,asOfDate,false,false)+") famSums "
		...
		public static string GetAgingQueryString(long guarantor,DateTime asOfDate,bool isHistoric=false,bool isInsPayWoCombined=true,
			bool hasDateLastPay=false,bool isGroupByGuar=true,bool isWoAged=false)
		{
			//No need to check RemotingRole; no call to db.
			...
			command+="ROUND(tSums.PayPlanDue,3) PayPlanDue"//PayPlanDue included for enterprise aging use
				...
				+"FROM ("+GetAgingGuarTransQuery(guarantor,asOfDate,hasDateLastPay,isHistoric,isGroupByGuar,isWoAged)+") tSums";
			return command;
		}

		///<summary>Returns a query string.</summary>
		private static string GetAgingGuarTransQuery(long guarantor,DateTime asOfDate,bool hasDateLastPay,bool isHistoric,bool isGroupByGuar=true,bool isWoAged=false) {
			//No need to check RemotingRole; no call to db.
			...
			command="SELECT "+(isGroupByGuar?"p.Guarantor PatNum,":"trans.PatNum,")
				...
				#region Derived Trans Table Aliased 'trans'
					...
					+"SELECT 'Proc' TranType,pl.PatNum,pl.ProcDate TranDate,pl.ProcFee*(pl.UnitQty+pl.BaseUnits) TranAmount,0 PayPlanAmount,0 InsWoEst,0 InsPayEst "
					...
					#region Insurance Payments and WriteOffs, PayPlan Ins Payments, and InsPayEst
						...
						+"SELECT 'Claimproc' TranType,cp.PatNum,cp.DateCP TranDate,"
						...
						+(payPlanVersionCur==2 ? "AND COALESCE(pp.IsClosed,0)=0 ":"") //ignore closed payment plans on v2
						...
						+"FROM claimproc cp "
						+(payPlanVersionCur==2 ? "LEFT JOIN payplan pp ON cp.PayPlanNum=pp.PayPlanNum ":"")
						...
						if(isWoAged) {
							command+="UNION ALL "
								+"SELECT 'WriteoffOrig' TranType,cp.PatNum,"
								+DbHelper.DtimeToDate("COALESCE(css.DateTEntry,cp.DateCP)")+" TranDate,"
								...
								+"0 PayPlanAmount,"
								//if no snapshot and claim is not received, include claimproc writeoff in InsWoEst column
								+"(CASE WHEN css.Writeoff IS NULL AND cp.Status=0 THEN cp.Writeoff ELSE 0 END) InsWoEst,"
								+"0 InsPayEst "
								+"FROM claimproc cp "
								...
								+"UNION ALL "
								+"SELECT 'Writeoff' TranType,cp.PatNum,cp.DateCP TranDate,"
					...
					command+="UNION ALL "
					#region Adjustments
					+"SELECT 'Adj' TranType,a.PatNum,a.AdjDate TranDate,a.AdjAmt TranAmount,0 PayPlanAmount,0 InsWoEst,0 InsPayEst "
					+"FROM adjustment a "
					+"WHERE a.AdjAmt != 0 "
					...
					+"SELECT 'PatPay' TranType,ps.PatNum,ps.DatePay TranDate,"
					...
					+"FROM paysplit ps "
					+(payPlanVersionCur==2 ? "LEFT JOIN payplan pp ON ps.PayPlanNum=pp.PayPlanNum ":"")
					+"WHERE ps.SplitAmt != 0 "
					...
					+"SELECT 'PPCharge' TranType,ppc.Guarantor PatNum,ppc.ChargeDate TranDate,0 TranAmount,";
					if(payPlanVersionCur==2) {
				  ...
				  command+="0 InsWoEst,0 InsPayEst "
					...
					+(payPlanVersionCur==2 ? "LEFT JOIN payplan pp ON ppc.PayPlanNum=pp.PayPlanNum " : "")
					...
					if(payPlanVersionCur==1) {//v1: aging the entire payment plan, not the payPlanCharges						
						command+="SELECT 'PPComplete' TranType,pp.PatNum,pp.PayPlanDate TranDate,-pp.CompletedAmt TranAmount,0 PayPlanAmount,0 InsWoEst,0 InsPayEst "
							+"FROM payplan pp "
					...
					#region PayPlan Version 2
						...
						command+="SELECT 'PPComplete' TranType,ppc.Guarantor,ppc.ChargeDate TranDate,"
					...
					if(!isAllPats) {
						...
						command+="UNION ALL "
							+"SELECT 'FamPatNums' TranType,PatNum,NULL TranDate,0 TranAmount,0 PayPlanAmount,0 InsWoEst,0 InsPayEst "
							+"FROM patient "
