...
namespace OpenDentBusiness {
	...
	public class RepeatCharges {
		...
		public static RepeatChargeResult RunRepeatingCharges(DateTime dateRun) {
			...
			Hashtable hashProcCodes=ProcedureCodeC.GetHList();
			...
			List<Procedure> listExistingProcs=Procedures.GetCompletedForDateRange(dateRun.AddMonths(-3),dateRun.AddDays(1),
				listRepeatingCharges.Select(x => x.ProcCode).Distinct().Select(x => ProcedureCodes.GetProcCode(x,hashProcCodes).CodeNum).ToList()
			...
			foreach(RepeatCharge repeatCharge in listRepeatingCharges) {
				...
				long codeNum=ProcedureCodes.GetCodeNum(repeatCharge.ProcCode,hashProcCodes);
				Procedure proc;
				...
				foreach(DateTime billingDate in listBillingDates) {
					Procedure procAdded=AddRepeatingChargeHelper(repeatCharge,billingDate,dateNow);
					List<Claim> listClaimsAdded=new List<Claim>();
					if(repeatCharge.CreatesClaim && !ProcedureCodes.GetProcCode(repeatCharge.ProcCode,hashProcCodes).NoBillIns) {
