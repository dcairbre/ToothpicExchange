...
namespace OpenDentBusiness{
	...
	public class Carriers{
		...
		public static void Update(Carrier carrier,Carrier oldCarrier, long userNum=0) {
		...
		public static void Combine(List<long> carrierNums,long pickedCarrierNum, long userNum=0) {
			...
			for(int i=0;i<carrierNums.Count;i++){
				if(carrierNums[i]==pickedCarrierNum)
					continue;
				command="SELECT PlanNum FROM insplan WHERE CarrierNum = "+POut.Long(carrierNums[i]);
				...
				Db.NonQ(command);
				listPlanNums.ForEach(x => { //Log InsPlan CarrierNum change.
					InsEditLogs.MakeLogEntry("CarrierNum",userNum,POut.Long(carrierNums[i]),POut.Long(pickedCarrierNum),InsEditLogType.InsPlan,x,0);
				});
