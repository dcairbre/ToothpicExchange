...
namespace OpenDentBusiness {
	public class DisplayFields {
		...
		private class DisplayFieldCache : CacheAbs<DisplayField> {
		...
		public static List<DisplayField> ListDeep {
			get {
				return _displayFieldCache.ListDeep;
			}
		}

		///<summary>A list of all DisplayFields.</summary>
		public static List<DisplayField> ListShallow {
			get {
				return _displayFieldCache.ListShallow;
			}
		}
		...
		public static List<DisplayField> GetForCategory(DisplayFieldCategory category){
			...
			for(int i=0;i<DisplayFields.ListShallow.Count;i++){
				if(DisplayFields.ListShallow[i].Category==category){
					retVal.Add(DisplayFields.ListShallow[i].Copy());
				}
			}
		...
		public static List<DisplayField> GetForChartView(long ChartViewNum) {
			...
			List<DisplayField> retVal=new List<DisplayField>();
			for(int i=0;i<DisplayFields.ListShallow.Count;i++) {
				if(DisplayFields.ListShallow[i].ChartViewNum==ChartViewNum && DisplayFields.ListShallow[i].Category==DisplayFieldCategory.None) {
					retVal.Add(DisplayFields.ListShallow[i].Copy());
				}
			}
			if(retVal.Count==0) {//default
