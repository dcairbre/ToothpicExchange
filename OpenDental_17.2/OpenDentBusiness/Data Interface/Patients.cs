...
namespace OpenDentBusiness {
	...
	public class Patients {
		...
		///<summary>Gets all potential clones and their corresponding specialty for the original patient passed in.
		///The returned dictionary will always contain the master patient so that it can be displayed to the user if desired.
		///Specialties are only important if clinics are enabled.  If clinics are disabled then the corresponding Def will be null.</summary>
		public static SerializableDictionary<Patient,Def> GetClonesAndSpecialtiesForPatients(List<long> listPatNums) {
			...
			List<Clinic> listClinics=Clinics.GetClinics(arrayPatientClones.Select(x => x.ClinicNum).Distinct().ToList());
			Def[][] arrayDefLong=DefC.GetArrayLong();
			foreach(Patient clone in arrayPatientClones) {
				Clinic clinic=listClinics.FirstOrDefault(x => x.ClinicNum==clone.ClinicNum);
				Def specialty=null;
				if(clinic!=null) {
					specialty=DefC.GetDef(DefCat.ClinicSpecialty,clinic.Specialty,arrayDefLong);//Can return null which is fine.
				}
		...
		private static PatientCloneDemographicChanges SynchCloneDemographics(Patient patient,Patient patientSynch) {
			...
			if(patientSynch.BillingType!=patient.BillingType) {
				...
				for(int i=0;i<DefC.Long[(int)DefCat.BillingTypes].Length;i++){
					if(patientSynch.BillingType==DefC.Long[(int)DefCat.BillingTypes][i].DefNum) {
						cloneBillType=DefC.Long[(int)DefCat.BillingTypes][i].ItemName;
					}
					if(patient.BillingType==DefC.Long[(int)DefCat.BillingTypes][i].DefNum) {
			...
			if(patientSynch.FeeSched!=patient.FeeSched) {
				...
				for(int i=0;i<FeeSchedC.ListLong.Count;i++) {
					if(patientSynch.FeeSched==FeeSchedC.ListLong[i].FeeSchedNum) {
						cloneFeeSched=FeeSchedC.ListLong[i].Description;
					}
					if(patient.FeeSched==FeeSchedC.ListLong[i].FeeSchedNum) {
			...
			bool excludeIfUnsentProcs,bool ignoreInPerson,List<long> clinicNums,bool isSuperStatements)
		{
			...
			listWhereAnds.Add("guar.PatNum=guar.Guarantor");//guarantees listWhereAnds will never be empty
			string strMinusIns="";
			...
			string command="";
			command="SELECT guar.PatNum,guar.FName,guar.MiddleI,guar.Preferred,guar.LName,guar.SuperFamily,guar.HasSuperBilling,guar.BillingType,"
				+"guar.Bal_0_30,guar.Bal_31_60,guar.Bal_61_90,guar.BalOver90,guar.BalTotal,guar.InsEst,guar.PayPlanDue,"
				+"COALESCE(MAX(guarstmt.DateSent),'0001-01-01') AS lastStatement "
				+"FROM patient guar "
				+"INNER JOIN patient pat ON guar.PatNum=pat.Guarantor AND pat.PatStatus NOT IN ("+string.Join(",",listPatStatusExclude)+") "
				+"LEFT JOIN statement guarstmt ON guar.PatNum=guarstmt.PatNum "
					+(ignoreInPerson?("AND guarstmt.Mode_!="+POut.Int((int)StatementMode.InPerson)+" "):"")
				+"WHERE "+string.Join(" AND ",listWhereAnds)+" "
				+"GROUP BY guar.PatNum "
			...
			if(includeChanged) {
				command="SELECT patient.Guarantor AS PatNum,MAX(procedurelog.ProcDate) maxDate "
					+"FROM procedurelog "
				...
				command="SELECT patient.Guarantor AS PatNum,MAX(claimproc.DateCP) maxDateCP "
					+"FROM claimproc "
				...
				command="SELECT patient.Guarantor AS PatNum,MAX(payplancharge.ChargeDate) maxDatePPC "
					+"FROM payplancharge "
			...
			if(excludeInsPending) {
				command="SELECT patient.Guarantor AS PatNum "
					+"FROM claim "
		...
		public static long GetProvNum(Patient pat) {
			...
			if(retval==0) {
				retval=ProviderC.GetListShort().FirstOrDefault()?.ProvNum??0;
			}
			return retval;
		}
		...
		public static List<Patient> GetPatientsByPartialName(string partialName,long clinicNum=0) {
			...
			string command="SELECT * FROM patient "
				+"WHERE (LName LIKE '%"+POut.String(partialName)+"%' "
				+"OR FName LIKE '%"+POut.String(partialName)+"%' "
				+"OR MiddleI LIKE '%"+POut.String(partialName)+"%' "
				+"OR Preferred LIKE '%"+POut.String(partialName)+"%') ";
			if(clinicNum > 0) {
		...
		public static DataTable GetTreatmentFinderList(bool noIns,bool patsWithAppts,int monthStart,DateTime dateSince,double aboveAmount,List<int> providerFilter,
			List<int> billingFilter,string code1,string code2,List<long> listClinicNums) 
		{
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetTable(MethodBase.GetCurrentMethod(),noIns,patsWithAppts,monthStart,dateSince,aboveAmount,providerFilter,billingFilter,code1,code2,listClinicNums);
			}
			DataTable table=new DataTable();
			DataRow row;
			...
			";
			joinUsedFam=@"
				SELECT claimproc.InsSubNum,
				SUM(COALESCE(claimproc.InsPayAmt,0)) AmtUsedFam
				FROM claimproc
				WHERE claimproc.Status IN (1, 3, 4) /*Received, Adjustment, Supplemental*/
				AND claimproc.ProcDate >= "+POut.Date(renewDate)+" "  //  MAKEDATE("+renewDate.Year+", "+renewDate.Month+") "
				+"AND claimproc.ProcDate < "+POut.Date(renewDate.AddYears(1))+" "   //MAKEDATE("+renewDate.Year+"+1, "+renewDate.Month+") "
			...
			joinPendingInd=@"
				SELECT patplan.PatPlanNum,
				SUM(COALESCE(claimproc.InsPayEst,0)) PendingAmtInd
				FROM claimproc
				LEFT JOIN patplan ON patplan.PatNum = claimproc.PatNum
				AND patplan.InsSubNum = claimproc.InsSubNum
			...
			joinPendingFam=@"
				SELECT claimproc.InsSubNum,
				SUM(COALESCE(claimproc.InsPayEst,0)) PendingAmtFam
				FROM claimproc
				WHERE claimproc.Status = 0 /*NotReceived*/
				AND claimproc.InsPayAmt = 0
				AND claimproc.ProcDate >= "+POut.Date(renewDate)+@" 
			...
			";
			joinPlanned=@"
				SELECT PatNum, SUM(ProcFee) AmtPlanned
				FROM procedurelog
				LEFT JOIN procedurecode ON procedurecode.CodeNum = procedurelog.CodeNum
				WHERE ProcStatus = 1 /*treatment planned*/ 
			";
			if(code1!="") {
				joinPlanned+=" AND procedurecode.ProcCode >= '"+POut.String(code1)+"' "
			...
			if(dateSince.Year>1880) {
				joinPlanned+="AND procedurelog.ProcDate>="+POut.DateT(dateSince)+" ";
			}
			joinPlanned+=" GROUP BY PatNum ";
			joinAnnualMax=@"
				SELECT insplan.PlanNum, MAX(CASE WHEN CoverageLevel!="+POut.Int((int)BenefitCoverageLevel.Family)+@" THEN MonetaryAmt ELSE -1 END) AnnualMaxInd/*for oracle in case there's more than one*/, 
				MAX(CASE WHEN CoverageLevel="+POut.Int((int)BenefitCoverageLevel.Family)+@" THEN MonetaryAmt ELSE -1 END) AnnualMaxFam/*for oracle in case there's more than one*/
				FROM benefit
				INNER JOIN insplan ON insplan.PlanNum=benefit.PlanNum 
				LEFT JOIN covcat ON benefit.CovCatNum=covcat.CovCatNum
				WHERE (covcat.EbenefitCat=1 OR ISNULL(covcat.EbenefitCat))
			...
			command=@"SELECT patient.PatNum, patient.LName, patient.FName,
				...
				usedInd.AmtUsedInd ""AmountUsedInd"",
				usedFam.AmtUsedFam ""AmountUsedFam"",
				pendingInd.PendingAmtInd ""AmountPendingInd"",
				pendingFam.PendingAmtFam ""AmountPendingFam"",
				COALESCE(annualMax.AnnualMaxInd,0)-COALESCE(usedInd.AmtUsedInd,0)-COALESCE(pendingInd.PendingAmtInd,0) AS ""$AmtRemainingInd"",
				COALESCE(annualMax.AnnualMaxFam,0)-COALESCE(usedFam.AmtUsedFam,0)-COALESCE(pendingFam.PendingAmtFam,0) AS ""$AmtRemainingFam"",
				planned.AmtPlanned ""$TreatmentPlan"", carrier.CarrierName,COALESCE(clinic.Abbr,'Unassigned') clinicAbbr
				FROM patient
				LEFT JOIN ("
					+joinPlanned
				+@") planned ON planned.PatNum=patient.PatNum
				...
				LEFT JOIN ("
					+joinUsedInd
				+@") usedInd ON usedInd.PatPlanNum=patplan.PatPlanNum
				LEFT JOIN ("
					+joinUsedFam
				+@") usedFam ON usedFam.InsSubNum=inssub.InsSubNum
				LEFT JOIN ("
					+joinPendingInd
				+@") pendingInd ON pendingInd.PatPlanNum=patplan.PatPlanNum
				LEFT JOIN ("
					+joinPendingFam
				+@") pendingFam ON pendingFam.InsSubNum=inssub.InsSubNum
				LEFT JOIN ("
					+joinAnnualMax
			...
			if(aboveAmount>0) {
				command+=@"AND (annualMax.PlanNum IS NULL OR ((annualMax.AnnualMaxInd=-1 OR annualMax.AnnualMaxInd-COALESCE(usedInd.AmtUsedInd,0)>"+POut.Double(aboveAmount)+") "
					+"AND (annualMax.AnnualMaxFam=-1 OR annualMax.AnnualMaxFam-COALESCE(usedFam.AmtUsedFam,0)>"+POut.Double(aboveAmount)+"))) ";
			}
			...
			command+=@"ORDER BY planned.AmtPlanned DESC";
		...
		///<summary>Gets completed procedures and adjustments for a superfamily, ordered by datetime.</summary>
