...
namespace OpenDentBusiness{
	...
	public class ClaimProcs{
		...
		///<summary>For a given PayPlan, returns a list of Claimprocs associated to that PayPlan.</summary>
		public static List<ClaimProc> GetForPayPlans(List<long> listPayPlanNums) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<ClaimProc>>(MethodBase.GetCurrentMethod(),listPayPlanNums);
			}
		...
		public static List<ClaimProc> RefreshForClaim(long claimNum,List<Procedure> listProcsForClaim=null) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<ClaimProc>>(MethodBase.GetCurrentMethod(),claimNum,listProcsForClaim);
			}
			string command=
				"SELECT * FROM claimproc "
			...
			if(CultureInfo.CurrentCulture.Name.EndsWith("CA") && listClaimProcs.Count>0) {
				if(listProcsForClaim==null) {
					listProcsForClaim=Procedures.Refresh(listClaimProcs[0].PatNum);
				}
				foreach(Procedure proc in listProcsForClaim) {
					if(proc.ProcNumLab==0) {
						continue;
					}
					listClaimProcs.RemoveAll(x => x.ProcNum==proc.ProcNum);
				}
			}
			...
			if(cp.Status==ClaimProcStatus.CapClaim
				...
				if(CultureInfo.CurrentCulture.Name.EndsWith("CA") && plan.PlanType=="" && cp.Status==ClaimProcStatus.Preauth) {//Canada and Category Percentage
					...
					foreach(Procedure procCur in listLabFees) {
						CanadianLabBaseEstHelper(cp,procCur,plan,cp.InsSubNum);
					}
				}
		...
		///<summary>This will update or create a claimProc for the given Canadian lab procedure (procLab).
		///The only claimProc fields that will be set are claimProc.BaseEst and claimProc.InsEstTotal.
		///Calculations for both fields are based on the percentage from the parent claim procs percentage (procParent).</summary>
		public static void CanadianLabBaseEstHelper(ClaimProc procParent,Procedure procLab,InsPlan plan,long insSubNum) {
			//No need to check RemotingRole; no call to db.
			double estAmt=procLab.ProcFee*(double)procParent.Percentage/100d;
			List<ClaimProc> listClaimProcs=ClaimProcs.RefreshForProc(procLab.ProcNum)//Get all claimProcs this lab.
				.FindAll(x => x.InsSubNum==procParent.InsSubNum 
					&& x.PlanNum==procParent.PlanNum && x.Status==procParent.Status);//Limit down to estimate claimProcs for current claim.
			if(listClaimProcs.Count > 0) {//There exist a estimate for the current lab proc.  Only 0 or 1 results possible.
				listClaimProcs.ForEach(x => { 
					x.BaseEst=estAmt;
					x.InsEstTotal=estAmt;
					x.InsPayEst=ClaimProcs.GetInsEstTotal(x);
					x.FeeBilled=procLab.ProcFee;
					x.Status=procParent.Status;
					if(x.Status.In(ClaimProcStatus.Received,ClaimProcStatus.Supplemental)) {
						x.DateEntry=procParent.DateEntry;
					}
					x.ClaimNum=procParent.ClaimNum;
					Update(x); 
				});
				return;
			}
			...
			claimProcLab.Status=procParent.Status;
			if(claimProcLab.Status.In(ClaimProcStatus.Received,ClaimProcStatus.Supplemental)) {
				claimProcLab.DateEntry=procParent.DateEntry;
			}
			claimProcLab.ClaimNum=procParent.ClaimNum;
		...
		public static double AttachAllOutstandingToPayment(long claimPaymentNum,long claimNum) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<double>(MethodBase.GetCurrentMethod(),claimPaymentNum,claimNum);
			}
			string command="UPDATE claimproc SET ClaimPaymentNum="+POut.Long(claimPaymentNum)+" "
				...
				+"AND (InsPayAmt != 0 OR ClaimNum="+POut.Long(claimNum)+")";
			Db.NonQ(command);
		...
		public static void DateInsFinalizedHelper(long claimPaymentNum,long claimNum) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),claimPaymentNum,claimNum);
				return;
			}
			string command="UPDATE claimproc SET "
				+"DateInsFinalized = (CASE DateInsFinalized WHEN '0001-01-01' THEN "+DbHelper.Now()+" ELSE DateInsFinalized END) "
