...
namespace OpenDentBusiness{
	...
	public class ApptViewItems{
		...
		public static List<long> GetOpsForView(long apptViewNum) {
			...
			if(apptViewNum==0) {
				...
				for(int i=0;i<listVisibleOps.Count;i++) {
					//Do not consider operatories that are not associated with the currently selected op.
					if(PrefC.HasClinicsEnabled && (Clinics.ClinicNum!=0 && Clinics.ClinicNum!=listVisibleOps[i].ClinicNum)) {
						continue;
					}
		...
		public static List<long> GetProvsForView(long apptViewNum) {
			...
			for(int i=0;i<ApptViewItemC.List.Length;i++) {
				if(ApptViewItemC.List[i].ApptViewNum==apptViewNum && ApptViewItemC.List[i].ProvNum!=0) {
					retVal.Add(ApptViewItemC.List[i].ProvNum);
				}
			}
			return retVal;
		}
