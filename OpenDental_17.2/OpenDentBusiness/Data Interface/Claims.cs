...
namespace OpenDentBusiness{
	...
	public class Claims{
		...
		public static List<ClaimPaySplit> GetOutstandingClaims(string carrierName,string LName,string FName,DateTime claimPayDate) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<ClaimPaySplit>>(MethodBase.GetCurrentMethod(),carrierName,LName,FName,claimPayDate);
			}
			string command="SELECT claim.DateService,claim.ProvTreat,'' AS patName_,carrierA.CarrierName,claim.ClaimFee feeBilled_,claim.ClaimStatus,"
				...
				+"WHERE (claim.ClaimStatus='S' OR (claim.ClaimStatus='R' AND (claimproc.InsPayAmt!=0 OR claim.DateService>"+POut.Date(claimPayDate)+"))) "
			...
			if(PrefC.HasClinicsEnabled && Clinics.ListLong.Count>0) {
				dictClinicNames=Clinics.ListLong.ToDictionary(x => x.ClinicNum,x => x.Description);
			}
			long patNumCur;
			long clinicNumCur;
		...
		public static List <long> GetClaimFromX12(List <X12ClaimMatch> listX12claims) {
			...
			double feeMin=listX12claims.Select(x => x.ClaimFee).Min();
			double feeMax=listX12claims.Select(x => x.ClaimFee).Max();
			...
			List <long> listClaimNums=new List<long>();
			foreach(X12ClaimMatch xclaim in listX12claims) {
				...
				for(DateTime d=xclaim.DateServiceStart;d<=xclaim.DateServiceEnd;d=d.AddDays(1)) {
					if(dictClaims.ContainsKey(d)) {
						listDbClaims.AddRange(dictClaims[d].FindAll(x => PIn.Double(x["ClaimFee"].ToString())==xclaim.ClaimFee));
					}
				}
				...
				if(listMatches.Count==0) {
					listClaimNums.Add(0);//No suitable matches.
				}
