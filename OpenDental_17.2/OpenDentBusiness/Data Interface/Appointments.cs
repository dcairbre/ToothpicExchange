	...
	public class Appointments {
			...
			if(raw.Rows.Count==0){
				...
				if(dateStart.Date<DateTime.Now.Date) {//Use the actual writeoff if looking at a date in the past, otherwise writeoff estimates will be used.
					...
					command+="SUM(CASE WHEN claimproc.Status IN("+POut.Int((int)ClaimProcStatus.CapComplete)+","
						...
						+"WHEN WriteOffEstOverride!=-1 THEN WriteOffEstOverride ELSE WriteOffEst END) writeoffPPO,"
						+"SUM(CASE WHEN claimproc.Status NOT IN("+POut.Int((int)ClaimProcStatus.CapComplete)+","
							+POut.Int((int)ClaimProcStatus.CapEstimate)+") THEN 0 "
						+"WHEN WriteOffEstOverride!=-1 THEN WriteOffEstOverride ELSE WriteOffEst END) writeoffCap,";
				}
		...
		public static void SetProcDescript(Appointment apt,List<Procedure> listProcs = null) {
			...
			foreach(Procedure proc in listProcs) {
				...
				string procDescOne = "";
				ProcedureCode procCode = ProcedureCodes.GetProcCode(listProcCodes,proc.CodeNum);
		...
		public static Appointment CreateApptForNewPatient(Patient patCur,Operatory operatory,DateTime dateTimeStart,DateTime dateTimeAskedToArrive
			,string pattern,List<Schedule> listSchedPeriod,string apptNote="") 
		{
			//No need to check RemotingRole; no call to db.
		...
		public static void SetAptStatus(long aptNum,ApptStatus newStatus,bool suppressHistory=false) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),aptNum,newStatus,suppressHistory);
				return;
			}
			...
			HistAppointments.CreateHistoryEntry(aptNum,HistAppointmentAction.Changed);
		}
			...
			bool doAllowFreqConflicts,bool doResetConfirmationStatus,bool doUpdatePatStatus,bool provChanged,bool hygChanged,bool timeWasMoved,bool isOpChanged,bool isOpUpdate=false,LogSources secLogSource=LogSources.None) 
		{
			...
			if(apt.Confirmed!=aptOld.Confirmed) {
				...
				SecurityLogs.MakeLogEntry(Permissions.ApptConfirmStatusEdit,apt.PatNum,
					...
					+DefC.GetName(DefCat.ApptConfirmed,aptOld.Confirmed)+" "+Lans.g("MoveAppointment","to")+" "+DefC.GetName(DefCat.ApptConfirmed,apt.Confirmed)
					+Lans.g("MoveAppointment","from the appointment module")+".",apt.AptNum,secLogSource);
			}
			#endregion
		...
		public static void UpsertApptFromWeb(Appointment appt,bool canUpdateApptPattern=false,LogSources secLogSource=LogSources.MobileWeb){
			...
			if(apptTypeCur!=null && apptTypeCur.AppointmentTypeNum!=apptTypeNumOld) {//Appointment type set and changed.
				...
				listProcsOnAppt=ApptTypeMissingProcHelper(appt,apptTypeCur,listProcsForApptEdit,pat,canUpdateApptPattern,listPatPlans,listInsSubs,listInsPlans,listProcCodes:listProcCodes);
			}
			else {
			...
			Procedures.ProcsAptNumHelper(listProcsForApptEdit,appt,listAppointments,listProcSelectedIndices,listProcNumsAttachedStart,listProcCodes,(appt.AptStatus==ApptStatus.Planned),secLogSource);
			Procedures.UpdateProcsInApptHelper(listProcsForApptEdit,pat,appt,apptOld,listInsPlans,listInsSubs,listProcSelectedIndices,false,secLogSource);
		...
		public static List<Procedure> ApptTypeMissingProcHelper(Appointment appt,AppointmentType apptType,List<Procedure> listProcsForAppt,Patient pat=null,bool canUpdateApptPattern=true,
			List<PatPlan> listPatPlans=null,List<InsSub> listInsSubs=null,List<InsPlan> listInsPlans=null,List<Provider> listProvs=null,List<ProcedureCode> listProcCodes=null,List<Benefit>listBenefits=null)
		{
			...
			if(listAptTypeProcs.Count>0) {//AppointmentType is associated to procs.
				...
				if(listProcCodes==null) {
					listProcCodes=ProcedureCodes.GetAllCodes();
				}
				...
				foreach(ProcedureCode procCodeCur in listAptTypeProcs) {
					...
					if(!existsInAppt) { //if the procedure doesn't already exist in the appointment
						Procedure proc=Procedures.ConstructProcedureForAppt(procCodeCur.CodeNum,appt,pat,listPatPlans,listInsPlans,listInsSubs,listProcCodes,listProvs);
