...
namespace OpenDentBusiness {
	public class Procedures {
		...
		public static DataTable GetDentalSealantMeasure(string year) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
			...
			string command=@"SET @ReportingDateStart = CONCAT("+year+@",'-','01','-','01'), @ReportingDateEnd = CONCAT("+year+@",'-','12','-','31');
				...
				FORMAT(ROUND((COUNT(ColC.NumeratorPat)/COUNT(ColA.DenominatorPat))*100,2),2) AS 'Percentage Met'
				FROM (	#exclusions/exceptions: any patient that should be excluded from the denominator.
					/*EXCLUSIONS/EXCEPTIONS*/
					SELECT denom.PatNum AS DenominatorPat,denom.ProvNum
		...
		///<summary>Gets a list of TP procedures that are attached to scheduled appointments that are not flagged as CPOE.
		///Will run for all provs when calling from OpenDentalService.</summary>
		public static List<Procedure> GetProcsNonCpoeAttachedToApptsForProv(long provNum,bool isFromODService=false) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<Procedure>>(MethodBase.GetCurrentMethod(),provNum,isFromODService);
			}
			if(provNum==0 && !isFromODService) {
				//The only time we want to allow provNum 0 is when called from OpenDentalService.
				return new List<Procedure>();
			}
			string command="SELECT procedurelog.* "
				...
				if(!isFromODService) {
					command+="AND procedurelog.ProvNum="+POut.Long(provNum)+" ";
				}
				command+="AND "+DbHelper.DtimeToDate("appointment.AptDateTime")+" >= "+DbHelper.Curdate()+" "
		...
		public static void SetProvidersInAppointment(Appointment apt,List<Procedure> listProcOrig,bool isUpdatingFees) {
			...
			for(int i=0;i<listProcOrig.Count;i++) {
				...
				if(changedProc.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC) 
					&& !Security.IsAuthorized(Permissions.ProcComplEdit,changedProc.ProcDate,true)) 
				{
					continue;
				}
			...
			if(isUpdatingFees) {
				foreach(Procedure procCur in listProcNew) {
					if(procCur.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC)
						&& !Security.IsAuthorized(Permissions.ProcComplEdit,procCur.ProcDate,true)) 
					{
						continue;
					}
					procCur.ProcFee=Fees.GetAmount0(procCur.CodeNum,Providers.GetProv(procCur.ProvNum,ProviderC.ListLong).FeeSched,procCur.ClinicNum,procCur.ProvNum);
				}
			}
		...
		public static Procedure UpdateProcInAppointment(Appointment apt,Procedure proc) {
			...
			if(proc.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC)
				&& !Security.IsAuthorized(Permissions.ProcComplEdit,proc.ProcDate,true)) 
			{
		...
		public static bool FeeUpdatePromptHelper(List<Procedure> listNewProcs,List<Procedure> listOldProcs,ref string promptText) {
			//No need to check RemotingRole; no call to db.
			switch(PrefC.GetInt(PrefName.ProcFeeUpdatePrompt)) {
				...
				case 2:
					...
					foreach(Procedure proc in listNewProcs) {
						...
						proc.ProcFee=Fees.GetAmount0(proc.CodeNum,Providers.GetProv(proc.ProvNum,ProviderC.ListLong).FeeSched,proc.ClinicNum,proc.ProvNum);
		...
		public static long GlobalUpdateFees(List<Fee> listFees, long clinicNumGlobal=-1) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetLong(MethodBase.GetCurrentMethod(),listFees,clinicNumGlobal);
			}
			if(_odThreadQueueData!=null) {
				throw new ApplicationException(Lans.g("FeeSchedTools","Global update fees tool is already running."));
			}
			ODEvent.Fire(new ODEventArgs("FeeSchedUpdate",new ProgressBarHelper(Lans.g("FeeSchedTools","Gathering data, please wait")+"...")));
			#if DEBUG
			...
			List<Provider> listAllProvs=ProviderC.GetListLong();
			long practDefaultProvNum=PrefC.GetLong(PrefName.PracticeDefaultProv);
			long practDefaultProvFeeSched=listAllProvs.FirstOrDefault(x => x.ProvNum==practDefaultProvNum)?.FeeSched??0;//default to 0 if prov is not found
			long firstNonHiddenProvFeeSched=listAllProvs.FirstOrDefault(x => !x.IsHidden)?.FeeSched??0;//default to 0 if all provs hidden (not likely to happen)
			Dictionary<long,long> dictProvFeeSched=listAllProvs.ToDictionary(x => x.ProvNum,x => x.FeeSched);
			...
			try {
				while(!_isQueueDataThreadDone || _queueBatchData.Count>0) {//if batch thread is done and queue is empty, loop is finished
					...
					//update progress bar with skipped count
					ODEvent.Fire(new ODEventArgs("FeeSchedUpdate",new ProgressBarHelper(
						Lans.g("FeeSchedTools","Procedure fees processed")+" "+rowSkippedCount+"/"+_totCount+" - "
							+Math.Floor(((double)rowSkippedCount/_totCount)*100).ToString()+"%",
						null,rowSkippedCount,_totCount,ProgBarStyle.Continuous)));
				}//end of while loop
				...
				List<Action> listActions=dictFeeListCodes.SelectMany(x => x.Value.Select(y => new Action(() => {
					...
					int countCur=procFeesUpdatedCount+rowSkippedCount;
					ODEvent.Fire(new ODEventArgs("FeeSchedUpdate",new ProgressBarHelper(
						Lans.g("FeeSchedTools","Procedure fees processed")+" "+countCur+"/"+_totCount+" - "
							+Math.Floor(((double)countCur/_totCount)*100).ToString()+"%",
						null,countCur,_totCount,ProgBarStyle.Continuous)));
				}))).ToList();
				#endregion Create List of Actions
				ODThread.RunParallel(listActions,TimeSpan.FromMinutes(30));//each group of actions gets X minutes.
			}//end of try
			catch(Exception ex) {
				ex.DoNothing();
			}
			...
			try {
				...
				for(int i=0;i<_listProcNumMaxPerGroup.Count;i++) {
					...
					string command="SELECT procedurelog.PatNum,procedurelog.ProcNum,procedurelog.CodeNum,procedurelog.ClinicNum,procedurelog.ProvNum,"
						+"procedurelog.ProcFee,patient.PriProv,patient.FeeSched patFeeSched,"
		...
		public static string CheckFrequency(List<Procedure> procList,long patNum,DateTime aptDateTime) {
			...
			foreach(Procedure proc in procListNew) {
				ComputeEstimates(proc,pat.PatNum,ref listClaimProcsForProcs,false,listInsPlans,listPatPlans,listBenefits,listClaimProcsHist,null,false,pat.Age,listInsSubs);
		...
		{
			ComputeEstimates(proc,patNum,ref claimProcs,isInitialEntry,planList,patPlans,benefitList,null,null,true,patientAge,subList,false,null);
		}
		...
		public static void ComputeEstimates(Procedure proc,long patNum,ref List<ClaimProc> claimProcs,bool isInitialEntry,List<InsPlan> planList,
			...
			List<InsSub> subList,bool useProcDateOnProc=false) 
		{
			ComputeEstimates(proc,patNum,ref claimProcs,isInitialEntry,planList,patPlans,benefitList,histList,loopList,saveToDb,patientAge,subList,false,
				null,useProcDateOnProc: useProcDateOnProc);
		}
		...
		public static void ComputeEstimates(Procedure proc,long patNum,ref List<ClaimProc> claimProcs,bool isInitialEntry,List<InsPlan> planList,
			List<PatPlan> patPlans,List<Benefit> benefitList,List<ClaimProcHist> histList,List<ClaimProcHist> loopList,bool saveToDb,int patientAge,
			List<InsSub> subList,bool isForOrtho,List<ClaimProc> listClaimProcsAll,bool isClaimProcRemoveNeeded=false,bool useProcDateOnProc=false) 
		{
			//No need to check RemotingRole; no call to db.
			...
			List<SubstitutionLink> listSubLinks=SubstitutionLinks.GetAllForPlans(planList);
			...
			List<InsSub> listInsSubs,List<SubstitutionLink> listSubLinks,bool useProcDateOnProc=false)
		{
			...
			List<long> listBWCodeNums=ProcedureCodeC.ListBWCodeNums;
			List<long> listPanoFMXCodeNums=ProcedureCodeC.ListPanoFMXCodeNums;
			List<long> listExamCodeNums=ProcedureCodeC.ListExamCodeNums;
			for(int i=0;i<claimProcs.Count;i++) {
				if(claimProcs[i].ProcNum!=proc.ProcNum) {
					continue;
				}
				...
				if(histList!=null && benefitList!=null && (PrefC.GetBool(PrefName.InsChecksFrequency)) && proc.ProcDate.Year>=1880) {
					...
					foreach(Benefit ben in listBensForGroup) {
						if(ben.QuantityQualifier==BenefitQuantity.Months || ben.QuantityQualifier==BenefitQuantity.Years) {
							...
							string benCodeStr=ProcedureCodes.GetStringProcCode(ben.CodeNum,listProcedureCodes);
						...
						if(ben.QuantityQualifier==BenefitQuantity.NumberOfServices) {
							...
							List<ClaimProcHist> claimProcHistList=histList.FindAll(x => x.Status==ClaimProcStatus.Received 
								...
								&& listBensForGroup.Exists(y => ProcedureCodes.GetStringProcCode(y.CodeNum,listProcedureCodes)==x.StrProcCode)
								&& x.PatNum==proc.PatNum
								&& x.InsSubNum==claimProcs[i].InsSubNum);
			...
			List<PatPlan> patPlans,List<Benefit> benefitList,int patientAge,List<InsSub> subList,List<ClaimProc> listClaimProcsAll=null,bool isClaimProcRemoveNeeded=false) 
		{
			...
			for(int i=0;i<procs.Count;i++) {
				ComputeEstimates(procs[i],patNum,ref claimProcs,false,planList,patPlans,benefitList,null,null,true,patientAge,subList,false,
					listClaimProcsAll,isClaimProcRemoveNeeded);
			}
		}
		...
		public static string GetClaimDescript(ClaimProc claimProcCur,ProcedureCode procCodeSent,Procedure procCur,ProcedureCode procCodeCur,InsPlan planCur=null) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetString(MethodBase.GetCurrentMethod(),claimProcCur,procCodeSent,procCur,procCodeCur,planCur);
			}
			...
			params ProcAttachTypes[] excludedTypes) 
		{
			...
			foreach(Procedure proc in listProcs) {
				...
				if(listSplitsCur!=null) {
					procE.AmountEnd-= listSplitsCur.Where(x => x.ProcNum == procE.Proc.ProcNum).Sum(x => x.SplitAmt);
				}
				retVal.Add(procE);
			}
			return retVal;
		...
		public static void ProcsAptNumHelper(List<Procedure> listProcs,Appointment AptCur,List<Appointment> listAppointments,
			List<int> listSelectedRows,List<long> listProcNumsAttachedStart,List<ProcedureCode> listProcCodes,bool isAptPlanned=false,LogSources logSource=LogSources.None)
		{
			for(int i=0;i<listProcs.Count;i++) {
				Procedure proc=listProcs[i];
				...
				if(isDetaching && isAptPlanned && proc.PlannedAptNum==AptCur.AptNum) {//Detatching from this planned appointment.
					...
					if(proc.PlannedAptNum!=0 && proc.PlannedAptNum != AptCur.AptNum) {//Currently attached to another planned appointment.
						...
						SecurityLogs.MakeLogEntry(Permissions.AppointmentEdit,AptCur.PatNum,Lans.g("AppointmentEdit","Procedure")+" "
							+ProcedureCodes.GetProcCode(listProcCodes,proc.CodeNum).AbbrDesc+" "+Lans.g("AppointmentEdit","moved from planned appointment created on")+" "
					...
					if(proc.AptNum!=0 && proc.AptNum != AptCur.AptNum) {//Currently attached to another appointment.
						...
						SecurityLogs.MakeLogEntry(Permissions.AppointmentEdit,AptCur.PatNum,Lans.g("AppointmentEdit","Procedure")+" "
							+ProcedureCodes.GetProcCode(listProcCodes,proc.CodeNum).AbbrDesc+" "+Lans.g("AppointmentEdit","moved from appointment on")+" "+apptOld.AptDateTime
		...
		public static bool UpdateProcsInApptHelper(List<Procedure> listProcsForAppt,Patient pat,Appointment AptCur,Appointment AptOld,
			List<InsPlan> PlanList,List<InsSub> SubList,List<int> listProcSelectedIndices,bool doUpdateProcFees=false,LogSources logSource=LogSources.None)
		{ 
			if(AptCur.AptStatus!=ApptStatus.Complete) {//appt is not set complete, just update necessary fields like ProvNum, ProcDate, and ClinicNum
				foreach(int index in listProcSelectedIndices) {
					...
					if(procOld.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC) && !Security.IsAuthorized(Permissions.ProcComplEdit,procOld.ProcDate,true)) {
						continue;
					}
				...
				SetCompleteInAppt(AptCur,PlanList,listPatPlans,pat.SiteNum,pat.Age,SubList);
		...
		public static Procedure ConstructProcedureForAppt(long codeNum,Appointment appt,Patient pat,List<PatPlan> listPatPlans,
			List<InsPlan> listInsPlans,List<InsSub> linstInsSubs,List<ProcedureCode> listProcCodes,List<Provider> listProvidersAll)
		{
			Procedure proc=new Procedure();
			proc.CodeNum=codeNum;
			...
			if(listPatPlans.Count>0) {
				primarySub=InsSubs.GetSub(listPatPlans[0].InsSubNum,linstInsSubs);
				primaryPlan=InsPlans.GetPlan(primarySub.PlanNum,listInsPlans);
			}
			//surf
			proc.Priority=0;
			proc.ProcStatus=ProcStat.TP;
			ProcedureCode procCodeCur=ProcedureCodes.GetProcCode(listProcCodes,proc.CodeNum);
			#region ProvNum
			...
			if(!string.IsNullOrEmpty(proc.MedicalCode) && PrefC.GetBool(PrefName.MedicalFeeUsedForNewProcs)) {//use medical fee
				long feeSch=Fees.GetMedFeeSched(pat,listInsPlans,listPatPlans,linstInsSubs,proc.ProvNum);
				proc.ProcFee=Fees.GetAmount0(ProcedureCodes.GetProcCode(proc.MedicalCode).CodeNum,feeSch,proc.ClinicNum,proc.ProvNum);
			}
			else {//use dental fee
				long feeSch=Fees.GetFeeSched(pat,listInsPlans,listPatPlans,linstInsSubs,proc.ProvNum);
			...
			if(primaryPlan!=null && primaryPlan.PlanType=="p") {//PPO
				double standardfee=Fees.GetAmount0(proc.CodeNum,Providers.GetProv(Patients.GetProvNum(pat),listProvidersAll).FeeSched,proc.ClinicNum,proc.ProvNum);
	...
	public class ProcExtended {
		private static long ProcExtendedAutoIncrementValue = 1;
		///<summary>No matter which constructor is used, the AccountEntryNum will be unique and automatically assigned.</summary>
		public long ProcExtendedEntryNum = (ProcExtendedAutoIncrementValue++);
