...
namespace OpenDentBusiness{
	...
	public class Schedules {
		#region Get Methods
		public static List<Schedule> GetSchedListForDateRange(long empNum,string startDate,string endDate) {
			...
			string command="SELECT * FROM schedule where schedule.EmployeeNum="+empNum+
				" AND schedule.SchedDate BETWEEN STR_TO_DATE('"+startDate+"','%m/%d/%Y') AND  STR_TO_DATE('"+endDate+"','%m/%d/%Y')"+
		...
		public static List<Schedule> GetSchedsForOp(Operatory op) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<Schedule>>(MethodBase.GetCurrentMethod(),op);
			}
			string command="SELECT schedule.* FROM schedule INNER JOIN scheduleop ON schedule.ScheduleNum=scheduleop.ScheduleNum "
				+"WHERE scheduleop.OperatoryNum="+POut.Long(op.OperatoryNum)+" AND schedule.SchedDate>="+POut.Date(DateTime.Today);
			List<Schedule> listScheds=Crud.ScheduleCrud.SelectMany(command);
			...
			,long clinicNum) 
		{
			...
			string command=@"-- First, get all schedules associated to operatories.
				(SELECT schedule.* FROM schedule 
					...
					AND operatory.ClinicNum="+POut.Long(clinicNum)+@"
		...
		public static DataTable GetPeriodEmployeeSchedTable(DateTime dateStart,DateTime dateEnd,long clinicNum) {
			...
			if(PrefC.IsODHQ) {
				command+="AND StartTime!=StopTime ";
			}
		...
		public static DataTable GetPeriodProviderSchedTable(DateTime dateStart,DateTime dateEnd,long clinicNum) {
			...
			List<Schedule> listScheds=ListSchedulesForDate.FindAll(x => listProvNums.Contains(x.ProvNum) && (PrefC.IsODHQ?x.StartTime!=x.StopTime:true));
