...
namespace OpenDentBusiness{
	...
	public class ClockEvents {
		#region Get Methods
		public static string GetEmployeeClockStatus(long empNum) {
			ClockEvent ce=ClockEvents.GetLastEvent(empNum);
			return ce.ClockStatus.ToString();
		}

		public static DataTable GetEmployeeClockEventsForDateRange(long empNum,string startDate,string stopDate) {
			...
			string command="SELECT (CASE WHEN ClockStatus=0 THEN 'Home' WHEN ClockStatus=1 THEN 'Lunch' ELSE 'Break' END) AS 'Status',"
				...
				+" FROM clockevent WHERE EmployeeNum="+empNum+" AND TimeEntered1 BETWEEN STR_TO_DATE('"+startDate+"','%m/%d/%Y')"
				+" AND STR_TO_DATE('"+stopDate+"','%m/%d/%Y') ORDER BY TimeDisplayed1";
		...
		public static DataTable GetTimeClockedInOnDate(long empNum,DateTime date) {
			...
			string command="SELECT DATE_FORMAT(TimeEntered1,'%r'), DATE_FORMAT(TimeEntered2,'%r'), TIMEDIFF(TimeEntered2,TimeEntered1) FROM clockevent"
				+" WHERE EmployeeNum="+empNum+" AND TimeEntered1 BETWEEN"
				+" STR_TO_DATE('"+date+"','%m/%d/%Y') AND STR_TO_DATE('"+date.AddDays(1)+"','%m/%d/%Y') GROUP BY EmployeeNum ORDER BY TimeEntered1 ASC";
			return Db.GetTable(command);
		}
		#endregion
