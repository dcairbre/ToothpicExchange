...
namespace OpenDentBusiness{
	...
	public class Fees {
		...
		public static List<Fee> GetByFeeSchedNumsDB(List<long> feeSchedNums) {
			//Doing this before remoting role check to reduce round trips to the server.
		...
		public static double GetFeeUCR(Procedure proc) {
			...
			double ppoFee=Fees.GetAmount0(proc.CodeNum,ProviderC.ListLong[Providers.GetIndexLong(provNum)].FeeSched,proc.ClinicNum,provNum);
		...
		///<summary>Inserts, updates, or deletes the passed in listNew against the stale listOld.
		...
		///<summary>Gets the appropriate fee from the passed-in dictionary. Returns null if not found.</summary>
		public static Fee GetFeeFromDict(Dictionary<long,List<Fee>> dictFeesForSched,long codeNum,long feeSchedNum,long clinicNum,long provNum) {
			...
			bool isGlobal=FeeScheds.IsGlobal(feeSchedNum);
			if(isGlobal) {
				return dictFeesForSched[codeNum].Where(x => x.ClinicNum==0 && x.ProvNum==0)
					.FirstOrDefault();
			}
			...
			//Ordering this list by provNum, then Clinic num, ensures the best match will always appear last in the list.
	...
	public class FeeCache {
		...
		///<summary>Returns an amount if a fee has been entered. Returns null if the fee is not found.</summary>
