	...
	public class PIn{
		...
		public static T Enum<T>(string value,bool isEnumAsString=false) where T : struct, IConvertible {
			...
			if(isEnumAsString) {
				...
				retVal=(T)(object)valueAsInt;
				//This is an alternative to double casting if there are any issues with the above.
				//System.Enum.TryParse(System.Enum.GetName(typeof(T),valueAsInt).ToString(),out retVal);
			}
			return retVal;
		}
