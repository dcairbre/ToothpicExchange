...
namespace OpenDentBusiness{
	...
	public class PhoneEmpDefaults{
		...
		public static int GetExtForComputer(string computerName) {
			...
			string command="SELECT PhoneExt FROM phoneempdefault WHERE ComputerName='"+POut.String(computerName)+"'";
		...
		public static void SetAvailable(int extension,long empNum,string computerName="") {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),extension,empNum,computerName);
				return;
			}
			...
			string command="UPDATE phoneempdefault "
				+"SET StatusOverride="+POut.Int((int)PhoneEmpStatusOverride.None)
					...
					+(string.IsNullOrEmpty(computerName) ? "" : ",ComputerName='"+POut.String(computerName.ToUpper())+"' ")
			...
			if(!string.IsNullOrEmpty(computerName)) {
				//Blank out all other phoneempdefault rows that currently have this computer name.  Each row requires a unique computer name.
				command="UPDATE phoneempdefault SET ComputerName='' "
					+"WHERE EmployeeNum!="+POut.Long(empNum)+" "
					+"AND ComputerName='"+POut.String(computerName.ToUpper())+"'";
				Db.NonQ(command);
			}
		}
