	...
	public partial class FormUnitTests:Form {
		...
		private void butNewDb_Click(object sender,EventArgs e) {
			...
			if(!_isOracle) {
				...
				if(!DatabaseTools.SetDbConnection("",serverAddr,serverPort,textUserName.Text,textPassword.Text,_isOracle)) {
		...
		private void butRun_Click(object sender,EventArgs e) {
			...
			try {
				Application.DoEvents();
				textResults.Text+=AllTests.TestSeventyFour(specificTest);
			}
			catch(Exception ex) {
				textResults.Text+="74: Failed. "+ex.Message;
			}
			try {
				Application.DoEvents();
				textResults.Text+=AllTests.TestSeventyFive(specificTest);
			}
			catch(Exception ex) {
				textResults.Text+="75: Failed. "+ex.Message;
			}
			try {
				Application.DoEvents();
				textResults.Text+=AllTests.TestSeventySix(specificTest);
			}
			catch(Exception ex) {
				textResults.Text+="76: Failed. "+ex.Message;
			}
			try {
				Application.DoEvents();
				textResults.Text+=AllTests.TestSeventySeven(specificTest);
			}
			catch(Exception ex) {
				textResults.Text+="77: Failed. "+ex.Message;
			}
			try {
				Application.DoEvents();
				textResults.Text+=AllTests.TestSeventyEight(specificTest);
			}
			catch(Exception ex) {
				textResults.Text+="78: Failed. "+ex.Message;
			}
			try {
				Application.DoEvents();
				textResults.Text+=AllTests.TestSeventyNine(specificTest);
			}
			catch(Exception ex) {
				textResults.Text+="79: Failed. "+ex.Message;
			}
			try {
				Application.DoEvents();
				textResults.Text+=AllTests.TestEighty(specificTest);
			}
			catch(Exception ex) {
				textResults.Text+="80: Failed. "+ex.Message;
			}
			try {
				Application.DoEvents();
				textResults.Text+=AllTests.TestEightyOne(specificTest);
			}
			catch(Exception ex) {
				textResults.Text+="81: Failed. "+ex.Message;
			}
			try {
				Application.DoEvents();
				textResults.Text+=AllTests.TestEightyTwo(specificTest);
			}
			catch(Exception ex) {
				textResults.Text+="82: Failed. "+ex.Message;
			}
			try {
				Application.DoEvents();
				textResults.Text+=AllTests.TestEightyThree(specificTest);
			}
			catch(Exception ex) {
				textResults.Text+="83: Failed. "+ex.Message;
			}
			try {
				Application.DoEvents();
				textResults.Text+=AllTests.TestEightyFour(specificTest);
			}
			catch(Exception ex) {
				textResults.Text+="84: Failed. "+ex.Message;
			}
			try {
				Application.DoEvents();
				textResults.Text+=AllTests.TestEightyFive(specificTest);
			}
			catch(Exception ex) {
				textResults.Text+="85: Failed. "+ex.Message;
			}
			textResults.Text+="Done";
			Cursor=Cursors.Default;
		}
