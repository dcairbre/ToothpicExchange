...
using System.Drawing;"+rn);
			...
			for(int f=0;f<fieldsInDb.Count;f++){
				...
				else if(specialType==CrudSpecialColType.TimeSpanLong) {
				...
				if(fieldsInDb[f].FieldType.Name=="Color" || fieldsInDb[f].FieldType.Name=="Interval" || specialType==CrudSpecialColType.TimeSpanLong) {
					strb.Append(")");
				}
				strb.Append(";");
			}
			...
			foreach(FieldInfo field in fieldsInDb) {
				...
				if(specialType==CrudSpecialColType.TimeSpanLong) {
