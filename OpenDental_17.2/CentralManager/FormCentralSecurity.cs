		...
		private void treePermissions_MouseDown(object sender,MouseEventArgs e) {
			...
			if(_clickedPermNode.ImageIndex==1) {//unchecked, so need to add a permission
				...
				if(GroupPermissions.PermTakesDates(perm.PermType)) {
					...
					FormCentralReportSetup FormCRS=new FormCentralReportSetup(_selectedGroupNum,true);
					if(FormCRS.ShowDialog()==DialogResult.Cancel) {
						treePermissions.EndUpdate();
						return;
					}
				...
				if((Permissions)_clickedPermNode.Tag==Permissions.ProcComplEditLimited
					&& GroupPermissions.HasPermission(_selectedGroupNum,Permissions.ProcComplEdit,0))
				{
					try {
