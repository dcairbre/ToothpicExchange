	...
	public static class CentralSyncHelper {
			...
			for(int i=0;i<listCentralUserData.Count;i++) {
				...
				for(int j=0;j<listRemoteCEMTUserGroups.Count;j++) {
					if(listCentralUserData[i].UserGroup.UserGroupNumCEMT==listRemoteCEMTUserGroups[j].UserGroupNumCEMT) {
						for(int k=0;k<listCentralUserData[i].ListUsers.Count;k++) {
							listCentralUserData[i].ListUsers[k].UserGroupNum=listRemoteCEMTUserGroups[j].UserGroupNum;//fixing primary keys to be what's in remote db
						}
		...
		public struct CentralUserData {
			...
			public CentralUserData Copy() {
				...
				return new CentralUserData(UserGroup.Copy(),listUsers,listGroupPermissions);//DEEP COPY OVERKILL BUT WE LIKE IT THAT WAY
			}
		}
